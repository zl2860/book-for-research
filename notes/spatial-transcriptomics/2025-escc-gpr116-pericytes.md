...
研究者可以把肿瘤拆成十万级单细胞，却也可能在解离组织时丢掉一个关键问题：哪些细胞正贴近侵袭前沿，并向肿瘤细胞发送信号。周细胞位于血管基底膜，数量少，过去常被当作一个整体；既往研究甚至同时把它描述为促肿瘤参与者和抗肿瘤“守门人”。

因此，这项研究问的不是“ESCC 里有哪些细胞”，而是：转移与非转移原发灶之间，哪一种微环境细胞的数量、位置和通信变化最突出；这种变化究竟只是伴随转移，还是能够推动转移并削弱免疫反应。

作者给出的答案是一类 **GPR116+ 周细胞**。人体样本先提供转移关联和空间线索，体外实验与小鼠模型再支持其功能；机制上，PRRX1 塑造这一周细胞状态，周细胞分泌的 EGFL6 与肿瘤细胞 integrin β1–NF-κB 及上皮—间质转化（EMT）信号相连，并伴随调节性 T 细胞（Treg）增加、CD8+ T 细胞效应下降和肿瘤 PD-L1 上调。血清 EGFL6 与 integrin β1 阻断由此成为两个转化出口，但前者仍是候选标志物，后者仍是临床前治疗证据。

## 01｜在转移相关微环境里，最先值得追的是哪类细胞？

作者先比较 12 位 ESCC 患者的原发灶：非转移组 6 位、转移组 6 位，另有 4 份来自转移患者的配对癌旁组织。scRNA-seq 质控后保留 117,169 个细胞，分成 14 类主要细胞；这一步的目的不是立即找一个“转移基因”，而是先判断哪类微环境细胞在两组之间变化最突出。

细胞通信推断把周细胞列为转移组中外向信号增幅最大的细胞，与上皮细胞的互动增幅也最大。随后，作者在 5 位患者的 Stereo-seq 数据中比较非转移 2 位与转移 3 位患者；结果显示，转移组的周细胞更靠近肿瘤细胞。这里的“通信”和“靠近”都只是候选筛选：前者来自表达模型，后者来自空间共定位，均不能单独证明因果。

这一步真正完成的是优先级排序。它把一个数量不多、过去常被合并分析的血管壁细胞群，推到了后续亚群拆分和功能验证的起点。

## 02｜这项研究到底用了多大规模、多少层证据？

这不是一项“十几万细胞等于十几万患者”的研究。高维观测量很大，患者层发现样本仍然有限，理解结论时必须同时看两个尺度。

- **人体发现层：**12 位患者、16 份组织；补充表 1 显示 12 人均进入 scRNA-seq、snRNA-seq 和 snATAC-seq，scRNA-seq 保留 117,169 个细胞，snATAC-seq 质控后为 109,183 个细胞。
- **空间层：**5 位患者、195,366 个 bin50 空间伪点位/细胞；bin50 直径约 25 μm，并不等同于严格单细胞。
- **跨数据集与组织验证层：**整合周细胞队列的分母并不自洽：主图图注写转移 29 位、非转移 25 位，Fig. 1h Source Data 实为 28 位与 25 位，Supplementary Note 又写“54 个肿瘤样本”却列出 26 位与 29 位（相加为 55）。组织微阵列比较癌旁正常 164 份、`N0` 肿瘤 96 份和 `N1–N3` 肿瘤 104 份；生存与 Cox 的 200 例正是后两组肿瘤队列。
- **血清与模型层：**ESCC 血清 ROC 队列为患者 135 位、健康对照 60 位；Fig. 4o 生存与补充表 4 的临床病理分析只使用其中 54 位。体外实验多数每组 3–5 个重复，小鼠治疗实验每组 4–7 只。

这些层次回答不同问题：人体队列说明“与什么相关”，空间数据说明“在哪里出现”，体外扰动说明“能否改变细胞表型”，小鼠模型说明“在所用模型中能否改变转移或治疗反应”。它们可以接成证据链，但不能互相替代。

## 03｜为什么锁定 GPR116+ 周细胞，而不是全部周细胞？

周细胞重聚类后分成 STEAP4+、SORBS2+ 和 GPR116+ 三个亚群。SORBS2+ 亚群偏向正常组织，STEAP4+ 分布较均匀，GPR116+ 亚群则在转移病例中富集；这一方向在作者数据与 4 个公开 ESCC scRNA-seq 数据集整合后仍然存在。不过人数必须连同冲突一起读：图注为转移 29 位/非转移 25 位，Fig. 1h Source Data 只有 28/25 个唯一样本，Supplementary Note 则同时出现总数 54 与 26+29=55，不能任选一个口径当成无争议分母。

要看清作者怎样从全景图谱收敛到一个周细胞状态，图 1 最关键。

<a id="reader-escc-fig1"></a>

![Fig. 1：从 ESCC 细胞图谱定位 GPR116+ 周细胞](../../assets/spatial-transcriptomics/2025-escc-gpr116-pericytes/fig1-cellular-landscape-gpr116-pericytes.png)

简明图注：图 1 从 12 位患者、117,169 个 scRNA-seq 细胞出发，经通信与空间筛选定位周细胞，再用整合单细胞数据、流式、组织微阵列和生存模型验证 GPR116+ 亚群；完整 panel、统计方法和来源 ID 见技术附录。

组织层验证比较了癌旁正常 164 份、非转移肿瘤 96 份和转移肿瘤 104 份，转移组的 GPR116+ 周细胞更多。在报告 n = 200 的多变量 Cox 模型中，GPR116+ 周细胞比例高组相对低组的 HR = 1.4（95% CI 1.0–2.0，P = 0.038）；这说明它在模型调整后仍与风险相关，但不能证明它导致患者死亡，也不等于已经形成可用的临床分层阈值。

GPR116 在这里首先是一个细胞状态标签。研究真正要验证的，是整类 GPR116+ 周细胞的功能，而不是预先假定 GPR116 这个表面分子本身就是促转移效应分子。

## 04｜它真的推动转移，还是只与转移同时出现？

人体数据先回答“同时出现”：GPR116+ 周细胞在转移原发灶中更多，也更靠近高 EMT 潜能肿瘤细胞。这里的 EMT 是基于上皮与间质基因特征计算的状态分数，空间细胞身份又依赖参考映射；所以这部分证据支持邻近和关联，还不能确定作用方向。

要判断是否存在功能作用，作者随后比较 GPR116− 与 GPR116+ 周细胞。体外 2D/3D 模型中，GPR116+ 周细胞更强地促进 ESCC 细胞侵袭和 EMT；在足垫模型中，GPR116− 周细胞组有 3/6 只小鼠发生腘窝淋巴结转移，GPR116+ 周细胞组则为 6/6。

图 3 把空间线索、体外功能和两个在体方向的验证放在了一起。

<a id="reader-escc-fig3"></a>

![Fig. 3：GPR116+ 周细胞的空间、体外与在体证据](../../assets/spatial-transcriptomics/2025-escc-gpr116-pericytes/fig3-escc-metastasis.png)

简明图注：图 3 显示 GPR116+ 周细胞靠近侵袭前沿，并在体外侵袭、尾静脉肺转移、足垫淋巴转移和 Prrx1 条件敲除模型中接受功能检验；尾静脉与足垫模型均不能等同于人类食管原位转移。

反向证据来自周细胞特异性 Prrx1 条件敲除。对照组 5/5 出现淋巴结转移，敲除组为 1/5；原文用了“消除”转移的措辞，但 1/5 并不是零，因此更准确的理解是转移率在这一小样本模型中明显下降。

这组结果把证据从人体关联推进到了模型内因果，但仍不能排除 Prrx1 敲除改变其他周细胞状态，也不能把足垫或尾静脉模型直接外推成人类自然转移过程。

## 05｜PRRX1–EGFL6–integrin β1–NF-κB 这条证据链怎样建立？

作者先找 GPR116+ 周细胞的上游。三个筛选条件分别得到 33 个在该亚群中活化的调控因子、282 个相对 GPR116− 周细胞高表达的基因，以及 66 个在转移患者周细胞中上调的基因；交集只剩 PRRX1、SOX4 和 HIF1A，PRRX1 与 GPR116+ 亚群标志的相关最强。
...
<a id="reader-escc-fig5"></a>

![Fig. 5：EGFL6–integrin β1–NF-κB 机制证据链](../../assets/spatial-transcriptomics/2025-escc-gpr116-pericytes/fig5-egfl6-integrin-nfkb.png)

简明图注：图 5 由通信推断提出 EGFL6–integrin β1，继而以 HEK293T 外源 co-IP、NF-κB/EMT 读出、两类抑制剂拯救及 volociximab 小鼠实验补强；外源 IP 不能证明 ESCC 细胞内源结合或直接界面，药理实验也不能完全排除靶外效应。

这条链之所以有解释力，是因为它同时连接了周细胞状态的形成和肿瘤细胞端的可干预输出。但论文没有完成 EGFL6 特异性功能缺失实验，也没有用 ITGB1 遗传扰动排除药理非特异性，因此“唯一且必需的通路”仍未被证明。

## 06｜为什么同一亚群还会压低抗肿瘤免疫？

多重免疫荧光图注把 Fig. 6a 的相关分析写成 `n=15` 份 ESCC 肿瘤组织，Extended Data Fig. 7m 又写转移 8 份、非转移 7 份；但两份官方 Source Data 都只有 15 个 ROI 行，且全部来自两个样本 ID：T21（M，8 个 ROI）与 T23（NM，7 个 ROI）。因此可说 GPR116+ 周细胞与 PD-1+CD8+ T 细胞、FOXP3+CD4+ Treg 在这两个样本的多个 ROI 中相关，不能把 15 个 ROI 当作 15 位独立患者，也不能单凭空间图像判断诱导方向。

共培养把证据向功能推进了一步。与 GPR116− 周细胞相比，GPR116+ 周细胞使 PD-1+CD8+ T 细胞和 FOXP3+CD4+ Treg 比例上升（每组 n=4）；其条件培养基降低 PBMC 杀伤，并降低 CD8+ T 细胞的 CD107a、GZMB 和 perforin（每组 n=3）。这些变化可以来自状态转变、增殖或选择性存活，实验并未做完整谱系追踪。
...
## 09｜对新的空间肿瘤研究，哪些设计可以直接迁移？

最值得迁移的不是某一个标志物，而是一条候选收敛路径：先在患者层比较中寻找通信变化突出的稀有细胞，再做亚群拆分；随后用空间侵袭前沿过滤候选，以组织成像确认位置，最后才进入体外扰动、遗传模型和药理验证。空间信息在这里是功能实验前的优先级过滤器，不是因果结论本身。

复用这一路线时，患者应始终作为生物学重复单位，并预先定义主要终点、样本量、多重检验、随机化和盲法。若目标是预测转移或免疫治疗反应，还需要转移前纵向样本、治疗前后配对组织和独立验证队列，而不只是横断面比较。

联合治疗也应携带机制读出。本文先观察 integrin β1 抑制可削弱 PD-L1 上调，再进入与 αPD-1 的两种小鼠模型；迁移到其他研究时，同样需要预先说明“为什么联合”、测量哪一个药效标志，以及什么结果会否定机制。

对胃癌等其他癌种，只能迁移设计，不能直接搬运机制。本文胃癌血清队列中 EGFL6 区分转移的 AUC 为 0.808，但没有证明其来源仍是 GPR116+ 周细胞，也没有复现 PRRX1–EGFL6–integrin β1 轴；新的癌种必须重新确认细胞来源、空间位置和通路依赖。

## 10｜这些结果仍需要冷静看待：哪些结论还不能下？

第一，患者层发现样本只有 12 位，空间队列只有 5 位，而且比较的是已经有或没有转移的原发灶。117,169 个细胞和 195,366 个空间伪点位提高的是细胞或空间单位层面的观测密度，不能替代患者层重复，也不能确定 GPR116+ 周细胞是在转移前出现还是转移后的反应。

第二，空间结论依赖约 25 μm 的 bin50、外部 GSE160269 参考去卷积和 SpaCET 恶性判别；CellChat 又由作者人工加入 21 个 EGFL6 受体候选。空间邻近和通信概率适合筛选机制，不等同于直接测量细胞接触、蛋白分泌或信号因果。

第三，作者没有用统计方法预先确定样本量，多处动物实验每组仅 4–7 只。尾静脉模型绕过原发灶侵袭和入血，足垫模型还把“淋巴结转移”和“局部肿瘤侵袭”放在同一操作性定义中；这些模型支持所用条件下的功能，却不能完整重现人类 ESCC 自然转移。

第四，临床转化仍缺独立验证。血清 EGFL6 的高 AUC 来自研究内队列，没有预设临床阈值与前瞻性验证；integrin β1 阻断和 αPD-1 联合只有小鼠证据，尚无人类疗效数据。

第五，原文和随文材料存在必须保留的冲突：Fig. 4 图注与队列构成把 ESCC 血清转移/非转移人数写反；主文与 Reporting Summary 对 CD146 门控、随机化和动物伦理机构的表述不一致；整合周细胞队列同时出现 53、54、55 三种可推得的总数；Fig. 6a/Extended Data Fig. 7m 的图注把 `n=15` 写成组织，而 Source Data 暴露为两个样本的 15 个 ROI。官方 Supplementary Information、Tables 1–7 与 20 个 Source Data 文件现已全部补回审计，Supplementary Figs. 1–2 只展示 T 细胞门控，不能裁决 CD146+/CD146− 的周细胞分选冲突。完整清单均保留在技术附录，不能用一种表述静默覆盖另一种。
...
以下来源标识从上方读者正文移入附录；正文事实与下方 Results/Methods 覆盖表对应，标识本身未删减。

- **开场：**`P001.S0002–P001.S0006`、`P001.S0018–P001.S0019`；`P001.S0020–P001.S0022`、`P003.S0009–P003.S0011`；`P006.S0017–P006.S0064`、`P009.S0028–P011.S0047`、`P011.S0048–P011.S0064`
- **01：**`P003.S0017–P003.S0033`、`P016.S0010–P016.S0013`、`P016.S0031`；`P003.S0034–P003.S0047`
- **02：**`P016.S0010–P016.S0013`、`P016.S0031`、`P016.S0055`；`P003.S0039`、`P016.S0041–P016.S0042`；`P003.S0009`、`P003.S0024–P003.S0026`、`P005.S0074–P006.S0015`；`P016.S0021–P016.S0023`、`P006.S0036–P006.S0038`、`P009.S0036–P009.S0039`、`P011.S0045–P011.S0064`
- **03：**`P003.S0048–P005.S0078`；`P002.S0060–P002.S0062`、`P003.S0025–P003.S0026`、`P006.S0009–P006.S0016`
- **04：**`P007.S0004–P009.S0009`、`P016.S0044–P016.S0049`、`P017.S0008–P017.S0010`；`P009.S0010–P009.S0021`；`P009.S0022–P009.S0027`
- **05：**`P006.S0017–P006.S0024`；`P006.S0040–P006.S0064`；`P009.S0028–P009.S0047`、`P009.S0066–P011.S0020`
- **06：**`P011.S0030–P011.S0035`；`P011.S0036–P011.S0040`；`P011.S0041–P011.S0047`
- **07：**`P009.S0055–P009.S0064`
- **08：**`P011.S0021–P011.S0029`、`P011.S0052–P011.S0064`
- **09：**`P009.S0062–P009.S0065`
- **10：**`P016.S0041–P017.S0007`；`P017.S0028–P017.S0035`、`P018.S0045`
...
- 在线原文与补充材料：[Nature 文章页](https://www.nature.com/articles/s41588-025-02341-9)、[Supplementary Information](https://media.springernature.com/original/springer-static/esm/art%3A10.1038%2Fs41588-025-02341-9/MediaObjects/41588_2025_2341_MOESM1_ESM.pdf)、[Supplementary Tables 1–7](https://media.springernature.com/original/springer-static/esm/art%3A10.1038%2Fs41588-025-02341-9/MediaObjects/41588_2025_2341_MOESM3_ESM.xlsx)。
...
- 另一处方法学内部冲突：正文 Methods 写明分选 `CD146+GPR116+` 周细胞（`P017.S0024–P017.S0027`），Reporting Summary 的 gating strategy 却写成保留 `CD146-negative, GPR116-positive` 群（`P039.S0001–P039.S0010`）。现已补回的 Supplementary Figs. 1–2 是 T 细胞 sequential gating，不是周细胞 CD146/GPR116 门控，仍不能替作者裁决。
...
- 主 PDF 不内嵌正文引用的 Supplementary Note、Tables 1–7、Figs. 1–2 或独立 Source Data；本次从出版社文章页补回全部 23 个 MOESM 文件。Supplementary Information 另生成 15 页、285 个 ID 的 pack，已 285/285 范围审计；补充表、109 张 source-data worksheets 与 6 份未裁剪 immunoblot 作为结构化文件逐项盘点，不混入主 PDF 的 Results/Methods 分母。（主文引用：`P016.S0013`、`P016.S0018`、`P016.S0025–P016.S0026`、`P016.S0038–P016.S0039`、`P016.S0050`、`P017.S0008–P017.S0011`、`P018.S0009`、`P009.S0071–P009.S0072`、`P018.S0062`）
...
#### 官方补充材料与 Source Data 审计

出版社文章页共提供 **23 个 MOESM 文件**：1 份 Supplementary Information PDF、1 份 Reporting Summary PDF、1 份含 Supplementary Tables 1–7 的工作簿，以及 Fig. 1–7/Extended Data Fig. 1–7 的 20 个 Source Data 文件（14 个统计工作簿、6 个未裁剪 immunoblot PDF）。14 个 Source Data 工作簿合计 **109 张 worksheets**；若连同补充表工作簿的 7 张表一起计算，则 15 个 XLSX 合计 116 张，两个口径不混用。

| 官方材料 | 已核内容 | 对正文证据边界的影响 |
|---|---|---|
| Supplementary Information（MOESM1，15 页） | 已生成 `tmp/2025-escc-gpr116-pericytes-supplementary-llm-pack.md` 与 manifest，共 285 个 ID（title 2、supplementary 283），285/285 已审计；补回 5′ scRNA、10x Multiome、Stereo-seq、流式、mIF、侵袭、ChIP、ELISA 等方案及 Supplementary Figs. 1–2 | 补齐不少建库与实验参数；文本抽取有明显双栏混排，技术细节须按版面连读。S Figs. 1–2 只展示 T 细胞门控，不能解决周细胞 CD146 门控冲突 |
| Supplementary Tables 1–7（MOESM3） | Table 1 给逐例模态；Tables 2–4 给组织/血清临床病理；Table 5 给各细胞簇 top 200 DEG；Table 6 给签名基因；Table 7 给引物与 siRNA 序列 | 使原先“不可见”的样本分配、签名和序列可核；同时暴露 `T_M/T_NM` 主要按淋巴结 N 分期操作化，不能等同远处转移 |
| Source Data（MOESM4–23） | 14 个 XLSX、109 张统计 worksheets；另有 6 个未裁剪 blot PDF。Fig. 1h 暴露整合队列分母冲突；[Fig. 6 Source Data](https://media.springernature.com/original/springer-static/esm/art%3A10.1038%2Fs41588-025-02341-9/MediaObjects/41588_2025_2341_MOESM12_ESM.xlsx) 暴露 15 个 ROI 只来自 2 个样本 | 可核原始作图数值与独立单位，不能因文件存在就假定患者级重复、分析代码或所有统计中间量已完整公开 |
...
| T_M / T_NM | 论文标注的“转移”/“非转移”原发肿瘤 | 发现队列按作者的 metastasis 标签分组；组织与血清验证表中，组数精确对应 N1–N3 与 N0 | 是横断面分组；验证队列几乎全部为 M0，故这里主要反映淋巴结状态，不能无条件译成远处转移 |
...
| snRNA-seq | 单核 RNA 测序 | 与 snATAC-seq 配对用于周细胞转录状态和染色质状态分析；补充表 1 显示 12 位患者均有 snRNA/snATAC。（来源：`P003.S0018`、`P006.S0046–P006.S0052`） | 配对多组学不增加患者层独立样本数 |
...
| TMA | 组织芯片 | 用于 GPR116+ 周细胞、EGFL6、预后和组织验证。（来源：`P016.S0018`、`P006.S0009–P006.S0015`、`P009.S0049–P009.S0054`） | Supplementary Tables 2–3 已核；GPR116 n=200、EGFL6 n=192，N 分期与总体分期相关最明确 |
...
| 公共单细胞整合 | 合并作者数据与 4 个公开 ESCC scRNA-seq 数据集 | 图注：M 29、NM 25；Fig. 1h Source Data：M 28、NM 25；Supplementary Note：总数 54，却列 M 26、NM 29（和为 55） | 亚群富集方向跨数据集存在，但精确患者分母三方冲突。（来源：`P003.S0009`、`P005.S0074–P005.S0078`；Supplementary Note `SI:P004.S0013–P005.S0003`） |
| 新鲜组织流式 | 邻近正常、非转移肿瘤、转移肿瘤 | N 7、T_NM 6、T_M 10 | 在新鲜样本中验证 GPR116+ 周细胞比例。（来源：`P003.S0024`） |
| GPR116+ 周细胞 TMA/IF | 邻近正常、N0 肿瘤、N1–N3 肿瘤；另作生存和 Cox | N 164、T_NM/N0 96、T_M/N1–N3 104，共 364；生存/Cox 队列为后两组 200。M stage 为 199 例 M0、1 例 M1 | 组织层富集、预后和独立关联；不能把 T_M 写成远处转移。（来源：`P003.S0024–P003.S0026`、`P006.S0009–P006.S0015`；Supplementary Table 2） |
| EGFL6 组织验证 | ESCC TMA/IHC 与生存 | N 111、T_NM/N0 86、T_M/N1–N3 106，共 303；生存 192。M stage 为 191 例 M0、1 例 M1 | 验证组织 EGFL6 与淋巴结状态、预后及 GPR116+ 周细胞相关。（来源：`P006.S0067`、`P009.S0049–P009.S0054`；Supplementary Table 3） |
| ESCC 血清 EGFL6 | 两中心 ESCC 血清与健康对照 | ROC：健康 60、ESCC NM 65、M 70，总 ESCC 135；Fig. 4o 生存/补充表 4 临床病理子集仅 54（N0 25、N1–N3 29） | 评估 ESCC 检出、作者标注的 M/NM 区分和生存关联；只有 54 例临床子集可确认组数对应 N 分期，不能外推给完整 ROC 队列。（来源：`P016.S0021–P016.S0023`、`P009.S0055–P009.S0061`；Fig. 4 图注的 NM/M 数字冲突见解析质量） |
| 跨癌种血清 EGFL6 | 肺癌与胃癌血清 | 图表行为肺癌 NM 31、M 60，胃癌 NM 46、M 47；Source Data 中肺癌 91 行只有 53 个唯一 Patient ID，胃癌 93 行只有 91 个唯一 ID | 探索性分类结果存在同一患者多行/重复 ID，独立患者数低于样本行数；不能称为 91 与 93 位独立患者。（来源：`P006.S0068`、`P016.S0023–P016.S0024`、`P009.S0062–P009.S0064`） |
| 组织免疫空间验证 | ESCC 多重 IF | 图注称总 n=15 份组织、Extended Data Fig. 7m 称 M 8/NM 7；两份 Source Data 实为相同 15 个 ROI，只来自 T21（M，8 ROI）和 T23（NM，7 ROI） | 相关分析的独立患者单位只有 2，ROI 嵌套不能当 15 位患者；图注还把无转移组误写成第二个 `T_M`。（来源：`P009.S0036`、`P032.S0017`、`P011.S0033–P011.S0035`） |
...
- Supplementary Table 1 已补齐逐例信息：12 位患者年龄 54–82 岁，男 8、女 4，stage I–III；12 人均有 scRNA、snRNA、snATAC，Stereo-seq 只覆盖 P21/P22（NM）与 P26/P27/P28（M）。Supplementary Information 却把队列概括为“operable locally advanced”，与表中 stage I 病例并不完全相容。（来源：`P016.S0010–P016.S0013`、Supplementary Table 1、`SI:P002.S0001–P002.S0006`）
- 整合周细胞队列的数字不能静默统一：图注 29+25=54，Fig. 1h Source Data 28+25=53，Supplementary Note 文字则把“54”与 26+29=55 写在同一句；Extended Data Fig. 2f 虽有 7,912 个周细胞和 54 个 sample IDs，却包含仅 10 个周细胞的 `P2_T`，又与“只纳入 >50 个周细胞的肿瘤样本”规则冲突。

#### 方法速览与分析框架

##### 1. 从原始测序到细胞图谱

- scRNA-seq 采用 10x Chromium Next GEM Single Cell 5′ v2/immune repertoire 体系，在 MGISEQ2000 上 PE100 测序；Cell Ranger 7.1.0 对 GRCh38 比对计数，再用 R 4.2.2/Seurat 4.3.0 整合。剔除基因数 <200 或 >6,000、UMI <500 或 >130,000、线粒体基因 >20% 的细胞，保留 117,169 个细胞；选择 2,000 个高变基因、Harmony 批次校正、前 20 个 PC、resolution 0.5 聚类，并以经典标志人工注释。（来源：`P016.S0026–P016.S0038`；`SI:P003.S0011–P003.S0014`）
- 周细胞从总图谱中再聚类；Supplementary Table 5 已提供相关细胞簇的 top 200 DEG、`avg_log2FC`、两组检出比例和 Bonferroni-adjusted P 值，而不是只给一个不可核的 marker 列表。（来源：`P016.S0037–P016.S0038`、Supplementary Table 5）

##### 2. 空间定位与侵袭前沿

- Stereo-seq 覆盖 5 份原发肿瘤，切片贴芯片后 37 °C 3 min，以甲醇在 −20 °C 固定 40 min；原位逆转录后用 0.8 μM cDNA-PCR primer 扩增，取 20 ng 产物以 Tn5 在 55 °C 处理 10 min，再经 0.6×与0.2× Ampure XP beads 纯化，在 DNBSEQ-Tx 上 PE100 测序。原始数据经 SAW 和 GRCh38 处理；把直径 220 nm 的 DNA nanoball 合并为 50×50 的 bin50，约 25 μm，得到 195,366 个 pseudo-spots/cells。（来源：`P016.S0039–P016.S0043`；`SI:P004.S0004–P004.S0013`）
...
- 单核配对组学采用 10x Chromium Next GEM Multiome ATAC+Gene Expression；MGISEQ2000 上 snRNA 为 PE100、snATAC 为 PE50。Cell Ranger ARC 2.0.2 联合处理表达和染色质信息，ArchR 1.0.2 进行下游分析；纳入标准为 TSS enrichment >6、unique fragments >1,000，并以 ArchR 函数移除 doublets，保留 109,183 个细胞。（来源：`P016.S0050–P016.S0055`；`SI:P003.S0014–P004.S0003`）
...
- 肿瘤细胞 EMT score 定义为 mesenchymal score 减 epithelial score，两部分均用 Seurat AddModuleScore 计算。Supplementary Table 6 已给出 14 个 epithelial genes（含 CDH1、CLDN3/4/7、OCLN、TJP1）和 7 个 mesenchymal genes（VIM、FN1、S100A4、ACTA2、COL1A1/2、COL3A1）。（来源：`P017.S0008–P017.S0010`、Supplementary Table 6）
- TCGA 中的 PD-1+CD8+（7 genes）、CD4+ Treg（6 genes）和 GPR116+ 周细胞（13 genes）signature 通过 GEPIA2 用于生存和相关分析。GPR116+ signature 本身包含 EGFL6，Fig. 4g 再把 EGFL6 与该分数相关会产生部分数学自相关；它还与 mesenchymal signature 共享 ACTA2、COL1A1、COL3A1，可能混入基质丰度。（来源：`P017.S0011`、Supplementary Table 6）
...
- Supplementary Information 已补回流式抗体 panel、PBMC 的 CD3/IL-2/CD28 48 h 激活、EGFL6 12.5/25 ng ml⁻¹ 侵袭条件、ChIP、ELISA、qPCR 和多重 IF 等步骤；仍未完整报告所有批次、独立重复定义、随机种子或周细胞分选门控图。Supplementary Figs. 1–2 是 Fig. 6 免疫 readout 的 T 细胞门控，不是 GPR116+ 周细胞分选门控。（来源：`P009.S0071–P009.S0072`；`SI:P006.S0008–P012.S0021`）
...
> 本节继续解读 Fig. 1 的 f–q panels；图像已置于[读者正文第 03 节](#reader-escc-fig1)，此处不重复嵌入。
...
- g：周细胞亚群标志物点图。
- h：图注称整合 ESCC 数据集中转移患者 n=29、非转移 n=25；Fig. 1h Source Data 实为 28/25 个唯一样本，Supplementary Note 又同时写总数 54 与 26+29=55，富集方向与精确分母须分开解释。
...
- k：图注称整合数据集中非转移 n=25、转移 n=29 的 SORBS2+、STEAP4+ 周细胞比例箱线图；同一 Source Data 的唯一样本实际为 25/28，t 检验，ns 表示不显著。
...
- g：TCGA-ESCC 队列中 EGFL6 与 GPR116+ 周细胞特征的相关性；该 13-gene signature 本身包含 EGFL6，相关存在部分自相关。
...
- k：不同组织中 EGFL6 的 IHC 染色与定量（N，n=111；T_NM/N0，n=86；T_M/N1–N3，n=106；该 192 例肿瘤队列仅 1 例 M1）。
...
- o：按血清 EGFL6 水平分层的 ESCC 患者生存分析（n=54，高/低各 27；log-rank 检验），不是 Fig. 4p ROC 的 135 例。
- p：血清 EGFL6 预测 ESCC 发生（左，n=195）及转移（右，n=135）的 ROC 曲线。
- q–r：左为健康个体及有/无转移癌症血清行的统计检验，右为 ROC；q，肺癌（HC 60；NM 31 行；M 60 行），但 91 个癌症行仅 53 个唯一患者 ID；r，胃癌（HC 60；NM 46 行；M 47 行），93 行仅 91 个唯一 ID。重复行代表何种取样及 ROC 是否按患者聚合均未说明。图注阈值：*P<0.05，**P<0.01，***P<0.001。

###### 原文结果梳理与逐句覆盖

EGFL6 是 GPR116+ 相对 GPR116− 周细胞中上调最强的基因，并主要由周细胞表达；Prrx1 条件敲除降低组织和血清 EGFL6。重组 EGFL6 以剂量依赖方式增强 ESCC 细胞侵袭和 EMT。组织及血清队列进一步显示 EGFL6 与作者定义的 M/NM 分组及较差生存相关；但完整 135 例 ROC 只有 M/NM 标签，只有 54 例临床子集可确认组数对应 N1–N3/N0，二者也不是同一分母。肺癌/胃癌 Source Data 还存在同一患者多行。诊断性能来自研究内 ROC，未见独立前瞻性外部验证、预设阈值、患者级聚合规则或校准信息，因此不能直接视为已可临床应用。
...
| `P009.S0053` | 在 TCGA 数据集和组织队列中，均观察到 EGFL6 表达与 GPR116+ 周细胞之间的强正相关（图 | 跨数据源支持 EGFL6 与该亚群相联；GPR116+ signature 本身包含 EGFL6，且与 mesenchymal signature 共享 3 个基因，相关可能部分来自数学自相关与基质丰度共同变化。 |
...
| `P009.S0060` | 血清 EGFL6 高水平患者的生存也更短（图 | 提供血清标志物的预后关联；Source Data 与 Supplementary Table 4 显示此处 n=54、高低各 27，不是 ROC 的 135 例，分界值和调整因素仍未充分报告。 |
| `P009.S0061` | 4o）。 | 仅补全图 4o 引用。 |
| `P009.S0062` | 血清 EGFL6 对肺癌转移也显示预测潜力（AUC=0.902；图 | 跨癌种探索性结果；Source Data 91 行仅 53 个唯一患者 ID，重复行未解释，不能直接证明同一机制或患者级性能。 |
| `P009.S0063` | 4q），对胃癌转移亦然（AUC=0.808；图 | 胃癌 Source Data 93 行仅 91 个唯一 ID；区分度低于肺癌/ESCC，仍为单队列探索。 |
...
- c：HEK293T 中双质粒共转染后的外源 IP，显示 EGFL6 与 integrin β1 可进入同一复合物；不是 ESCC 细胞内源 co-IP。
...
配体–受体分析将 EGFL6–integrin β1 指向 GPR116+ 周细胞与高 EMT 肿瘤细胞的通讯轴。HEK293T 外源双质粒 co-IP 支持二者可处于同一复合物，表达/富集分析和免疫印迹将 integrin β1 与 NF-κB 激活、EMT 和转移状态连接起来；integrin β1 或 NF-κB 抑制剂可逆转 EGFL6 诱导的侵袭/EMT。体内使用 volociximab 抑制 integrin β1 后，三级引流淋巴结转移明显下降。边界是外源 co-IP 不能证明 ESCC 内源结合或直接界面，药理抑制可能存在靶外效应，volociximab 结果来自小鼠模型；本节不提供人类疗效证据。
...
| `P011.S0003` | 由于 EGFL6 含有可与 integrin 结合的精氨酸–甘氨酸–天冬氨酸（RGD）基序，我们用 IP 实验证实 EGFL6 与 integrin β1 的物理结合（图 | 作者称“物理结合”；Supplementary Methods 显示这是 HEK293T 双质粒外源 co-IP，只能支持同一复合物，不能确认 ESCC 内源结合、直接界面或体内亲和力。 |
...
- a：图注称 ESCC 肿瘤组织相关分析 `n=15`；Source Data 实为 15 个 ROI、只来自 T21（M，8 ROI）和 T23（NM，7 ROI），不是 15 份独立组织。
...
- m：图注称有转移 `T_M, n=8` 与无转移组 `n=7`，但又把后者误写为 `T_M`；原图注 P 值为 0.0095、0.00021、0.011。对应 Source Data 与 Fig. 6a 完全相同，实际是 T21 的 8 个 ROI 与 T23 的 7 个 ROI，不能把 `n=8/7` 解释为独立组织或患者。

###### 原文结果梳理与逐句覆盖

单细胞相关和多重 IF 显示 GPR116+ 周细胞邻近耗竭 CD8+ T 细胞与 Treg，但 IF 定量只有两个样本的 15 个 ROI，患者层证据远弱于图注表面上的 `n=15`。共培养进一步使 PD-1+CD8+ 和 FOXP3+CD4+ 细胞比例上升，并降低 PBMC 杀瘤及 CD8+ 效应分子；GPR116+ 周细胞还通过可被 integrin β1 抑制剂逆转的方式上调癌细胞 PD-L1。证据支持模型内免疫抑制表型，但体外共培养不能完整还原体内抗原特异性、细胞迁移和其他免疫成分；pan-cancer TCGA 仍是 bulk 特征相关。
...
| EGFL6 通过 integrin β1–NF-κB 促进 EMT/侵袭 | CellChat 候选、HEK293T 外源 EGFL6–integrin β1 co-IP、ITGB1 状态、p-p65 剂量响应、ATN-161/BAY 11-7082 拯救。（来源：`P009.S0066–P011.S0020`；`SI:P011.S0009`） | 中（模型内机制链） | 外源 IP 不是 ESCC 内源结合或直接界面证据；未做 ITGB1 遗传扰动，不能证明患者体内的唯一或必要通路 |
| GPR116+ 周细胞呈现并支持免疫抑制表型 | 单细胞相关、两个组织样本的 15 个 ROI、多重 IF、T 细胞/肿瘤细胞共培养、PBMC 杀伤与效应分子读出。（来源：`P011.S0030–P011.S0047`） | 中低：有体外功能，患者空间独立单位极少 | ROI 伪重复不能证明患者内免疫抑制或免疫治疗耐药完全由该亚群造成 |
| 血清 EGFL6 在本研究合并横断面队列显示 ESCC/作者所标 M–NM 的区分能力 | ESCC ROC 135 例+健康 60，AUC 0.983/0.912；生存分析另为 54 例，只有该临床子集可确认 M/NM 组数对应 N1–N3/N0。（来源：`P009.S0055–P009.S0061`、Supplementary Table 4） | 中：研究内区分关联较强，但仍是候选标志物 | 不能把 54 例中的 N 分期对应关系或生存结果套给 135 例；未见独立测试集、预设阈值、校准或前瞻验证 |
| EGFL6 在 ESCC、肺癌和胃癌呈探索性跨癌种信号 | 肺癌 AUC 0.902、胃癌 AUC 0.808；肺癌 91 行/53 个唯一 ID，胃癌 93 行/91 个唯一 ID。（来源：`P009.S0062–P009.S0065`） | 探索性且受重复行影响 | ROC 的患者级聚合规则未说明；三个癌种不等于泛癌验证，也不证明同一细胞来源或机制 |
...
| `P016.S0013` | 单细胞测序队列的临床特征汇总于补充表 1。 | 表 1 已核：12 人均有 scRNA/snRNA/snATAC，Stereo 仅 P21/P22/P26/P27/P28；年龄、性别和 stage I–III 逐例可见。 |
...
| `P016.S0018` | **实验验证的人体样本队列。**采用含原发 ESCC 肿瘤及其配对癌旁正常组织的人体 ESCC 组织微阵列（Shanghai Outdo Biotech），通过免疫荧光和 IHC 分析 GPR116+ 周细胞比例及 EGFL6 表达；生存分析队列的临床病理特征见补充表 2、3。 | 表 2/3 已核：GPR116 n=200、EGFL6 n=192；两者均主要与 N stage 和 overall stage 相关，而各仅 1 例 M1。 |
...
| `P016.S0023` | 广州医科大学附属第五医院提供 60 例健康个体，以及 91 例肺癌患者，其中无转移 31 例、有转移 60 例。 | Source Data 的 91 个癌症行仅 53 个唯一 Patient ID（NM 19、M 34 人）；行级重复和聚合规则未说明。 |
| `P016.S0024` | 中国医科大学提供 93 例胃癌患者，其中无转移 46 例、有转移 47 例。 | Source Data 的 93 行仅 91 个唯一 Patient ID；两位 M 组 ID 各重复一次。 |
| `P016.S0025` | 各癌种血清检测队列的临床病理特征汇总于补充表 4。 | 表 4 已核，三癌种分组数均精确对应 N0/N1–N3；ESCC 临床/生存子集仅 54 例，高低 EGFL6 各 27。 |
| `P016.S0026` | **scRNA-seq 数据处理、聚类与注释。**scRNA-seq 的详细实验方案见 Supplementary Note。 | Supplementary Information 已补回 10x 5′ v2/immune repertoire 与 MGISEQ2000 PE100 等建库/测序信息；正文下游分析仍单独审计。 |
...
| `P016.S0038` | 随后重新聚类周细胞以进一步分析；3 个簇各自前 200 个 DEG 汇总于补充表 5。 | 表 5 已核，可见 gene、cluster、avg_log2FC、pct.1/pct.2 与 Bonferroni-adjusted P 值。 |
| `P016.S0039` | **Stereo-seq 数据处理与分析。**Stereo-seq 的详细实验方案见 Supplementary Note。 | Supplementary Information 已补回 OCT、固定、扩增、Tn5、bead purification 与 DNBSEQ-Tx PE100 参数。 |
...
| `P016.S0050` | **snATAC-seq 数据处理与聚类。**snATAC-seq 的详细实验方案见 Supplementary Note。 | Supplementary Information 已确认 10x Multiome 及 snRNA PE100/snATAC PE50；主文给下游 Cell Ranger ARC/ArchR。 |
...
| `P017.S0009` | 使用 Seurat（v.4.3.0）的 `AddModuleScore` 函数和特征基因（补充表 6），计算 `epithelial_score` 与 `mesenchymal_score`。 | 表 6 已核，14-gene epithelial 与 7-gene mesenchymal 列表可见；背景基因选择和随机种子仍需代码。 |
| `P017.S0010` | 每个肿瘤细胞的 EMT score 定义为 `mesenchymal_score − epithelial_score`。 | 明确评分公式；它是相对表达指标，不是迁移能力的直接实验测量。 |
| `P017.S0011` | 对 TCGA 数据集中的特征评分，将 PD-1+CD8+ T 细胞、CD4+ Treg 细胞和 GPR116+ 周细胞特征（补充表 6）输入 GEPIA2 网页服务器 68，进行生存和相关分析。 | 表 6 已给 7/6/13-gene 清单；GPR116+ signature 含 EGFL6，并与 mesenchymal signature 共享 3 个基因，须警惕自相关和基质丰度。 |
...
| `P018.S0009` | siRNA 序列列于补充表 7。 | 表 7 已核，列出 PRRX1 NC、#1、#2 的正反义链；仍未提供本研究内脱靶验证或转染效率的完整原始数据。 |
...
研究的核心测序材料来自 12 例病理确诊 ESCC：非转移和转移患者各 6 例，肿瘤均取自原发食管病灶；另外只在转移患者中取得 4 份、距肿瘤 5 cm 的配对癌旁正常组织。由此，转移状态是患者间分组，而癌旁正常并非两组均衡配对。Supplementary Table 1 显示 12 位患者均有 scRNA、snRNA 和 snATAC，Stereo-seq 只覆盖 P21、P22、P26、P27、P28；年龄 54–82 岁，男 8、女 4，stage I–III。Supplementary Information 却把全组写成 “operable locally advanced”，与表中的 stage I 不完全相容。即使模态分配已补齐，也不能把正常–肿瘤差异与转移–非转移差异视为完全正交。（`P016.S0010`–`P016.S0013`、`P034.S0040`–`P034.S0053`、Supplementary Table 1、`SI:P002.S0001–P002.S0006`）

采样后 30 min 内在冰上处理；组织在 RPMI 1640 中切碎、按厂家方案酶消化，再经 40-µm 滤网和死细胞去除。这保证了可进入单细胞流程的活细胞悬液，却也意味着消化耐受性、组织硬度和细胞大小可能影响回收组成。主文与补充材料仍没有给出消化时长、酶浓度或每例活率。（`P016.S0014`–`P016.S0017`、`P016.S0026`）

##### 2. 验证资源分为组织、血清和跨癌种队列

组织微阵列用 IF/IHC 测量 GPR116+ 周细胞比例与 EGFL6 表达，并承担生存验证。Supplementary Table 2（n=200）显示 GPR116+ 浸润与 N stage、overall stage 均 `P<0.001`，而年龄、性别、T/M stage 与 grade 未显著；Table 3（n=192）对组织 EGFL6 得到同一模式。血清 EGFL6 以 ELISA 检测，包括两个 ESCC ROC 队列〔上海 75 例（35/40）；汕头 60 例（30/30）〕、健康 60 例，以及肺癌 91 行和胃癌 93 行；但 Fig. 4o 生存只用 ESCC 54 例，肺癌 Source Data 的 91 行只有 53 个唯一患者 ID，胃癌 93 行只有 91 个唯一 ID。Supplementary Methods 补充了人/鼠 CUSABIO ELISA 货号，仍未给统一采血时点、治疗状态、批次或人口学匹配。（`P016.S0018`–`P016.S0025`、Supplementary Tables 2–4、`SI:P011.S0006–P011.S0008`）
...
分选时使用 PE-CD146（P1H12）和 488-GPR116（1055815）染色，并由 FACSAria Fusion 分离主文写明的 CD146+GPR116+ 周细胞；数据以 FlowJo v.10.8.1 分析。Reporting Summary 却写成保留 CD146−GPR116+ 群，并另报 4.7% 事件及 >94.5% 纯度，和主文标志组合直接冲突。Supplementary Information 补充了 viability dye、CD31/SMA 等抗体与 4 °C 暗孵育 30 min，但 S Figs. 1–2 只展示 T 细胞门控，仍没有周细胞完整 gating/补偿图。因此足以复述冲突，却不足以从零复现分选门。（`P017.S0024`–`P017.S0027`、`P038.S0066`–`P038.S0069`、`P039.S0004`–`P039.S0010`、`SI:P006.S0010–P007.S0007`）

##### 2. PRRX1/GPR116/EGFL6 的细胞扰动

PRRX1 siRNA 和对照质粒来自 Hanyi Biotechnology，siRNA 用 Lipofectamine 3000 按厂家说明转染；PRRX1、GPR116、EGFL6 质粒来自作者实验室。稳定株流程为：目标质粒与 Addgene 第三代包装质粒 12251/12253/12259 共转染 HEK 293T；48 h 后收上清，800g 5 min、0.45-μm 过滤后感染 ESCC 细胞；1 µg ml⁻¹ puromycin 筛 7 d，再以免疫印迹检查蛋白水平。Supplementary Table 7 给出 PRRX1 siRNA：NC `UUCUCCGAACGUGUCACGUTT`，#1 `GGCAUAUCAGAUUGAGCUATT`，#2 `AGCGAAGGAAUAGGACAACTT`（均为正义链 5′→3′；表内同时列反义链）。载体骨架、感染复数、筛选存活率和独立克隆数仍未报告。（`P018.S0001`–`P018.S0009`、Supplementary Table 7）

##### 3. 数据生成环节的已知与未知

主文把建库方案外引至 Supplementary Information；补回后可确认 scRNA 使用 10x Single Cell 5′ v2/immune repertoire、MGISEQ2000 PE100，配对 snRNA/snATAC 使用 10x Multiome、同机分别 PE100/PE50，Stereo-seq 使用 DNBSEQ-Tx PE100。主文下游仍分别由 Cell Ranger 7.1.0、SAW 和 Cell Ranger ARC 2.0.2 处理。Supplementary Information 未报告统一测序深度、建库批次或足以重现全部核提取的参数。（`P016.S0026`–`P016.S0028`、`P016.S0039`–`P016.S0040`、`P016.S0050`–`P016.S0052`；`SI:P003.S0011–P004.S0013`）
...
Seurat `AddModuleScore` 用 Supplementary Table 6 的 14-gene epithelial 与 7-gene mesenchymal signatures 计算 EMT score；TCGA 中则把 7-gene PD-1+CD8+、6-gene Treg 和 13-gene GPR116+ 周细胞 signatures 输入 GEPIA2。基因列表现已可重算，但背景基因抽样、随机种子与完整 GEPIA2 设置仍需从代码核对。还要注意 GPR116+ signature 本身含 EGFL6，和 EGFL6 做相关有部分数学耦合；其 ACTA2/COL1A1/COL3A1 又与 mesenchymal signature 重叠，可能混入基质丰度。（`P017.S0008`–`P017.S0011`、Supplementary Table 6）
...
| 模块 | 软件/版本 | 主文与官方补充材料已核关键参数 | 迁移时必须重新核对 |
|---|---|---|---|
| scRNA 原始计数 | 10x 5′ v2；MGISEQ2000；Cell Ranger 7.1.0 | PE100；GRCh38 | 命令行参数、测序深度、V(D)J 分析去向 |
| scRNA 分析 | R 4.2.2；Seurat 4.3.0；Harmony | 2,000 HVG；20 PC；resolution 0.5；t-SNE | 归一化调用、Harmony 批次变量、随机种子、marker 表 |
| Stereo-seq | DNBSEQ-Tx；SAW（版本未报）；Seurat 4.3.0 | PE100；37 °C 3 min；甲醇 −20 °C 40 min；0.8 μM primer；20 ng；Tn5 55 °C 10 min；0.6×/0.2× beads；bin50 | SAW 版本、spot QC、组织配准、建库批次 |
| 空间去卷积 | SPOTlight 0.1.7；SpaCET 1.2.0 | 参考 GSE160269；7 大类 | topic 构建、恶性判别阈值、参考集匹配 |
| 配对 ATAC+RNA | 10x Multiome；MGISEQ2000；Cell Ranger ARC 2.0.2；ArchR 1.0.2 | snRNA PE100、snATAC PE50；TSS>6；fragments>1,000；resolution 0.6 | doublet 阈值、批次变量、peak/motif 配置、默认参数 |
| regulon/CNV | SCENIC 1.3.1；RcisTarget 1.18.2；AUCell 1.20.2；inferCNV 1.18.1 | 278 regulons；519 个 high-EMT DEG，log2FC>0、adjusted P<0.05 | motif database、GRNBoost 参数、CNV 阈值与参考细胞 |
| 细胞通信 | CellChat 1.6.1 | 人工加入 21 个 EGFL6 受体 | 受体清单、置换次数、显著性/FDR |
| 签名评分 | Seurat 4.3.0 `AddModuleScore`；GEPIA2 | epithelial 14、mesenchymal 7、GPR116+ PC 13、PD-1+CD8 7、Treg 6 genes | 背景基因、随机种子、GEPIA2 设置、signature 重叠 |
| 流式 | FACSAria Fusion/LSRFortessa；FlowJo 10.8.1 | CD146、GPR116、CD31、SMA 及免疫 panel/孵育条件已核 | 周细胞补偿、完整 gating、CD146+/− 冲突、纯度 |
...
Supplementary Information、Tables 1–7 与 Source Data 已从官方文章页补回。现在可核：12 人逐例模态、10x 5′ v2/Multiome 与测序读长、Stereo-seq 建库条件、Tables 2–4 的临床病理分母、Table 5 的 DEG 字段、Table 6 的全部 signatures、Table 7 的引物/siRNA，以及多项流式、侵袭、ChIP、ELISA 和免疫印迹步骤。Supplementary Information 的 15 页、285/285 个 ID 已作范围审计；7 张表、109 张 Source Data worksheets 与 6 个 blot PDF 已逐文件盘点。

材料补回并未等于完全复现：仍缺 CellChat 人工加入的 21 个受体清单、统一测序深度/批次、若干默认参数与随机种子、周细胞完整 gating/补偿图，以及血清重复 Patient ID 的取样含义和患者级 ROC 聚合规则。更重要的是，新文件暴露了整合队列 53/54/55 三套分母、Fig. 6 的 ROI 伪重复、Supplementary Methods“局部晚期”与 Table 1 stage I 病例、以及 CD146+/− 等源内冲突；这些不能用“已有 Source Data”自动消解。
...
4. **CellChat 先验扩展会影响候选受体。** 作者手工加入 21 个 EGFL6 受体，再以表达和置换构建通讯概率；这适合筛候选，但不是物理作用证据。integrin β1 之后有 HEK293T 双质粒外源 co-IP 和抑制剂拯救，可信度高于纯计算候选，却仍缺 ESCC 内源 co-IP 与 ITGB1 遗传扰动。（来源：`P016.S0057–P017.S0007`、`P011.S0001–P011.S0017`、`SI:P011.S0009`）
...
6. **流式门控文字存在直接冲突。** 主文写 CD146+GPR116+，Reporting Summary 写 CD146−GPR116+；现可见 Supplementary Figs. 1–2 只展示 T 细胞/肿瘤细胞免疫 readout 的 sequential gating，不是周细胞分选门，无法裁决。（来源：`P017.S0024–P017.S0027`、`P009.S0071–P009.S0072`、`P039.S0001–P039.S0012`）
...
13. **血清 EGFL6 仍缺独立临床验证链。** ESCC ROC 使用作者标注的 NM 65、M 70，但只有 Fig. 4o 生存/临床子集的 n=54 可确认组数对应 N0 25、N1–N3 29，不能把该定义外推给完整 ROC 队列；研究也没有预设阈值、独立训练/测试、校准或前瞻决策曲线，Fig. 4 图注还把 NM/M 人数写反。（来源：`P016.S0021–P016.S0022`、`P006.S0067`、`P009.S0055–P009.S0061`、Supplementary Table 4）
14. **三个癌种的探索性信号不能称为泛癌验证。** 肺癌 Source Data 91 行仅 53 个唯一 Patient ID，胃癌 93 行仅 91 个，重复行含义和 ROC 聚合规则未说明；也没有证明肺癌/胃癌 EGFL6 来自 GPR116+ 周细胞或复现同一机制。（来源：`P009.S0062–P009.S0065`、`P013.S0028–P013.S0029`）
15. **组织免疫相关存在严重 ROI 伪重复风险。** Fig. 6a 和 Extended Data Fig. 7m 的 Source Data 是相同 15 个 ROI，只来自 T21 与 T23 两个样本；图注却把它们写成 n=15 份组织或 M 8/NM 7。患者层独立重复为 2，不能按 15 个独立组织解释相关和组间 P 值。
16. **整合周细胞队列分母无法闭合。** 图注为 M 29/NM 25，Fig. 1h Source Data 为 28/25，Supplementary Note 又在“54 samples”后写 26/29；Extended Data Fig. 2f 的 54 个 sample IDs 还包含只有 10 个周细胞的 P2_T，与 >50 纳入规则冲突。
17. **签名之间存在数学耦合。** GPR116+ 周细胞 signature 本身含 EGFL6，与 EGFL6 的相关分析部分自相关；它又与 mesenchymal signature 共享 ACTA2、COL1A1、COL3A1，相关可混入共同基质丰度，不能全归为特异生物学耦合。（Supplementary Table 6）
18. **官方材料已补回，但仍有明确复现缺口。** 23 个 MOESM 已盘点，Supplementary Information 285/285 个 ID、7 张表、109 张 source-data worksheets 和 6 个 blot PDF 均已审计；仍缺 21 个 EGFL6 受体清单、周细胞完整 gating、若干批次/随机参数与重复样本解释。（来源：`P016.S0013`、`P016.S0018`、`P016.S0025–P016.S0026`、`P016.S0038–P016.S0039`、`P017.S0008–P017.S0011`、`P018.S0009`、`P018.S0062`）
...
3. **配体–受体机制的闭环：** 分泌配体的细胞特异表达 → 受体候选 → 外源 co-IP 复合物 → 下游通路剂量反应 → 受体/通路抑制剂拯救 → 在体受体阻断；迁移时还应补内源结合和遗传依赖。（来源：`P009.S0028–P009.S0047`、`P009.S0066–P011.S0029`）
...
5. **机制闭环：** “候选配体–受体关系先由通讯分析提出，再由外源 co-IP、剂量响应和药理学拯救共同支持；内源结合与遗传依赖仍待验证。”（证据组织依据：`P009.S0066–P011.S0020`）
...
- **主 PDF 解析基础：**PDF 共 **39 页**，全文 pack 共 **2,012** 个句子 ID。
- **主 PDF Results 三分审计：**自动分类为 Results 的 **574** 个 ID，精确拆分为 **228 个真实叙事 ID + 39 个正式图注 ID + 307 个图内标签、坐标、单个数值、页眉页脚或跨栏错序 ID = 574**。其中 228/228 个真实叙事 ID 已逐句覆盖；307 个图内/错序 ID **不是原文叙事句**，不被伪装成正文翻译。
- **主 PDF Methods 审计：**真实 Methods 为 **183/183** 个 ID，均有独立逐句表行；manifest 的自动 Methods 计数 398 还包含 reporting summary 被误标的 **215** 个 ID，这 215 个 ID 不进入主文 Methods 分母。
- **官方补充材料审计：**Supplementary Information 共 15 页、285 个 ID，已 **285/285** 范围覆盖；Supplementary Tables 1–7 的 7 张表、14 个 Source Data XLSX 的 109 张 worksheets 与 6 个未裁剪 blot PDF 已逐文件盘点。结构化文件不混入主 PDF Results/Methods 的句子分母。
- **低置信和材料边界：**主 PDF Results 有 26 个混排或低置信 ID，Methods 有 69 个断句或低置信 ID，均保留 `EXTRACTION_CHECK`；Supplementary Information 也有双栏混排，新增信息以 `SI:` ID、表名或 Source Data 文件/panel 定位，仍不补造缺失参数。
...
- **补充材料边界：**Supplementary Information 285/285 个 ID 已范围审计，Tables 1–7 与 20 个 Source Data 文件已盘点；它们独立于主文 183/183 Methods 分母。仍缺的参数已在“复现信息与剩余缺口”中逐项列明。
...
- Extended Data Fig. 1–7 随主 PDF 可见并已按 panel 解读；官方 Supplementary Information、Tables 1–7、Supplementary Figs. 1–2 与 20 个 Source Data 文件本次均已补回。它们补齐模态分配、队列协变量、DEG/签名、siRNA、建库参数和部分原始数值，同时暴露 ROI 独立单位、重复 Patient ID、三套整合队列分母与周细胞门控等新边界；这些冲突均显式保留。

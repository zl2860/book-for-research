...
空间技术试图保住这层关系，但选择本身已经变成新的难题。综述列出的空间转录组尺度从早期约 55 μm 的捕获区域推进到 2 μm bin（空间像素单元）、约 500 nm 阵列；成像型平台一次检测的基因集合（panel）可从约 5,000 扩展到约 18,000，而多数商业空间蛋白组仍集中在约 20–60 个蛋白。分辨率、分子覆盖、组织面积、FFPE（福尔马林固定石蜡包埋）兼容性和成本不能同时最大化。

真正需要解决的因此不是“要不要做空间组学”，而是怎样从临床或生物学问题反推样本、平台和分析单位：该看一个细胞、一个生态位（niche），还是整个肿瘤生态？又该怎样把一张高维空间图谱变成能在独立队列验证的标志物？

这篇文章不是一项新的患者队列研究，而是一篇 26 页的叙述性综述。作者用 5 张主图、1 张平台比较表和 241 篇参考文献给出的答案是：先把形态、位置和分子放进同一坐标系，再从细胞逐级组织到生态位（niche）、群落（community）和生态系统（ecosystem）；临床转化则必须从预设终点出发，把高维发现蒸馏为可标准化、可质控、可部署的读数。

## 01｜单细胞已经能分清细胞，为什么还不够？

解离型单细胞测序擅长回答“有哪些细胞状态”，却会切断细胞原来的邻近关系、组织边界和区域梯度。两个肿瘤可以具有相似的 T 细胞比例，但一个肿瘤的 T 细胞与抗原呈递细胞形成免疫枢纽（hub），另一个肿瘤的 T 细胞被癌症相关成纤维细胞（CAF）和髓系细胞隔离；细胞清单相近，组织机制可能完全不同。

病理图像保留了结构，却通常缺少足够的分子维度；整体（bulk）组学覆盖广，却混合了不同细胞和区域。空间组学新增的不是一个坐标装饰，而是让细胞状态、邻近关系、肿瘤—基质边界、代谢梯度和临床结局有机会在同一组织框架下比较。

这层信息仍然以取样区域为边界。活检、感兴趣区域（ROI）或组织芯片没有覆盖到的生态位，任何空间算法都无法补回；模型重建出的高分辨率结构也不等于直接测量。
...
这里没有一个可汇总的患者样本量，也没有作者原创的效应量、P 值或合并比较（pooled comparison）。26 页、5 张图、1 张表和 241 篇参考文献描述的是领域覆盖范围，不是“1,598 个解析句子 ID 等于 1,598 个实验结果”，更不能把不同癌种、平台和队列的发现当作同一条件下的横向排名。

因此，阅读重点不是寻找一个最高曲线下面积（AUC），而是判断每一类技术和空间单位能回答什么问题、需要什么样本，以及其证据在哪一步仍停留在被引研究或作者路线图。
...
测序型平台更接近全转录组发现，却要在捕获效率、区域大小和单细胞边界之间取舍。组织阵列捕获（tissue-on-array）路线的名义尺度从约 55 μm 发展到约 500 nm，Visium HD 使用 2 μm bin；但 2 μm bin 通常只覆盖细胞的一部分，“亚细胞 bin”不能自动等同于准确分割出的单细胞。

其他模态有不同的取舍。多数商业空间蛋白组约覆盖 20–60 个蛋白，少数可以超过 100 种；基质辅助激光解吸电离质谱成像（MALDI-IMS）常见分辨率约 10–50 μm，二次离子质谱（SIMS）可以进入亚微米尺度，却通常牺牲分子覆盖。空间 CITE-seq（同切片 RNA—蛋白联合测量）约为 10–25 μm；相关 Stereo-CITE 方案可把全转录组与超过 100 个蛋白连接起来，但配准、组织消耗和批次也随之增加。
...
![Table 1：高分辨率商业空间转录组平台比较](../../assets/spatial-transcriptomics/2026-spatial-omics-review/table1-commercial-spatial-transcriptomics-platforms-p1.png)

简明图注：Table 1 应被当作平台选择矩阵。其规格来自 2025 年时点的厂商资料或网站，分辨率、panel、捕获面积、组织兼容性和成本并非同一实验条件下测得，后续也可能更新。

## 04｜拿到空间坐标后，怎样避免停在“漂亮地图”？

第一步不是聚类，而是确认坐标中的分子究竟属于哪个空间单位。成像型数据依赖细胞分割；捕获点（spot）或 bin 数据常需去卷积、从 bin 重建细胞或进行分辨率增强。分割边界一旦偏移，后面的细胞类型、邻近关系和配体—受体推断都会继承误差。

第二步才是归一化、批次校正和跨样本协调（harmonization），再把数据组织成细胞、亚细胞区室、生态位、群落或全肿瘤区域。一个算法输出的空间分区（domain）是模型构建的单位，需要用标志物、病理区域、独立样本或功能实验验证，不能因为颜色边界清楚就把它当成天然实体。

第三步是把 RNA、蛋白、代谢、DNA 变异和苏木精—伊红（H&E）图像配准到同一组织结构。多模态整合与空间基础模型（spatial foundation model）可以增强表示能力，也会引入训练集偏倚、模态缺失和跨平台泛化问题；增强后的图谱必须与原始测量并列保留。
...
Box 2 分别定义了细胞、邻域、生态位、群落、区域、分区梯度、生态型、原型和全肿瘤生态系统九个术语；Fig. 4 本身只画出“单细胞—多细胞生态位—空间群落—全肿瘤生态系统”四层。临床相关信号可能出现在较高层级：不是某个细胞独自决定治疗反应，而是肿瘤细胞、抗原呈递细胞、T 细胞和基质是否组成了可重复的结构。
...
作者提出的顺序是：先写清临床问题和结局（endpoint），再决定患者、时间点、部位、保存方式、平台和验证策略。癌前病变需要阶段性或纵向采样；转移研究需要原发灶、不同转移灶、正常组织和血液配对；微小残留病灶（MRD）与治疗反应则需要治疗前、治疗中、治疗后及复发前窗口。

样本保存直接限制可用技术。新鲜冷冻（fresh-frozen）更适合全转录组、代谢组和发现阶段多组学；FFPE 更容易获得大规模回顾队列、长期结局和病理配准，却通常依赖探针型或靶向检测。缺血时间、固定方式、切片厚度、区域选择和配对血液都应在研究方案中预先规定。

据此设计研究时，可以预设特定生态位密度、免疫排斥表型、靶点阳性肿瘤比例或肿瘤—基质界面状态等空间终点；这些是编者按综述路线整理的示例，不是原文逐项列出或已经验证的评分。若先生成海量特征再寻找最显著关联，平台批次、ROI 选择和多重比较很容易被误当成生物学发现。
...
临床部署通常不会原样报告成千上万个空间特征。更现实的路径是先用高多重度平台发现稳定结构，再把它蒸馏为低多重度免疫荧光、标准病理图像、有限标志物组合或可重复的组织评分，并在独立队列完成阈值、校准和结局验证。

组织芯片可以提高验证通量：综述讨论的组织芯片（TMA）取样芯常为 1.5–2 mm，可覆盖数千细胞并扩展到数百病例。但它只取肿瘤的一小块，恰好说明“扩大患者数”与“覆盖完整空间异质性”仍是一组取舍。

空间分析也可以提出药物靶点、治疗组合或风险模型假说，但被引研究启动试验、发现相关生态位，和试验证明疗效是三件不同的事。作者真正强调的是从发现到可部署检测的连续验证，而不是把高维图谱直接送入临床决策。

## 08｜对胃癌癌前病变研究，哪些设计可以迁移？

可以把 H. pylori 感染、慢性炎症、萎缩、肠化、异型增生和早癌当作阶段轴，但不要只做横断面“图谱比赛”。先预设可验证终点，例如肠化进展、异型增生发生、早癌检出或根除后的逆转，再决定每个阶段需要的病例数、随访和取样位置。

发现与验证可以分层：新鲜冷冻小规模队列用于全转录组或多模态发现，FFPE 回顾队列用于靶向空间转录组、空间蛋白组和病理图像验证。对同一患者尽量保留多区域黏膜、病灶邻近区和配对血液，并记录固定时间、切片厚度和病理区域。

最终输出应是可复核的空间终点，而不是一张更复杂的图。任何候选上皮状态、免疫生态位、基质边界或微生物界面都需要独立队列、平台重测、阈值校准和功能证据；这是从该综述迁移出的研究设计建议，不是原文已经证明的胃癌标志物。

## 09｜这些结果仍需要冷静看待

首先，这是一篇叙述性综述。原文没有报告系统检索、纳排标准、偏倚评价或 meta-analysis；被引研究跨癌种、平台、保存方式和分析流程，不能合并成一个统一效应，也没有可供本综述独立验证的作者 cohort。

其次，空间图谱仍受取样和算法共同限制。活检、感兴趣区域（ROI）与 1.5–2 mm TMA core 都可能遗漏全肿瘤生态；分割、去卷积、配准和分辨率增强会传播误差。细胞相邻、配体—受体共表达或 niche 与生存相关，只能支持机制假说，不能替代扰动实验和纵向验证。

再次，Table 1 是 2025 年时点的平台规格概览，不是统一条件下的 benchmark。名义分辨率、基因 panel、蛋白 plex、捕获面积和价格会随产品版本变化；跨平台复现必须报告版本、样本处理、深度、分割和空间零模型。

最后，high-plex 空间多组学仍受到成本、通量和标准化限制。空间 AI 还面临训练集偏倚、跨中心泛化、校准、可解释性和责任边界；癌前“molecular clock”、用液体活检推断组织空间结构等方向在本文中仍是路线图，不是完成的临床验证。本地主 PDF 的 Table 1 位于单页，但线性抽取因表格和双栏布局发生列错序；P001、P008–P010 与 P017 另有双栏/heading 错序，参考文献还被大面积误标为 Methods。具体来源边界见技术附录。
...
- [研究问题与科学背景](#研究问题与科学背景)
- [综述范围与证据结构](#综述范围与证据结构)
- [综述梳理的技术与分析框架](#综述梳理的技术与分析框架)
- [原文综述内容完整梳理](#原文综述内容完整梳理)
...
- [作者结论与证据强度](#作者结论与证据强度)
- [领域方法学与复现提示](#领域方法学与复现提示)
  - [综述型文章的证据结构](#综述型文章的证据结构)
  - [空间组学平台选择方法](#空间组学平台选择方法)
  - [空间数据处理流程](#空间数据处理流程)
  - [原综述点名的计算方法索引](#原综述点名的计算方法索引)
  - [编者补充的常用统计与分析框架](#编者补充的常用统计与分析框架)
...
- PDF 解析质量：
  ...
  - Table 1 完整位于 PDF 第 4 页（印刷页 27）；线性抽取因表格和双栏布局导致列值错序，本文以该页原表截图核对，不逐项重排未确认的规格。
  - 参考文献及页眉页脚区域共有 1,053 个 ID：前 40 个被正确标为 References，后 1,013 个被错分为 Methods；文末按综述真实结构完成语义校正审计。
- 图像截取说明：已截取主文 Fig. 1-5 和 Table 1 单页原表，图像位于 `assets/spatial-transcriptomics/2026-spatial-omics-review/`。旧文件名带 `p2` 的截图片实际是 PDF 下一页正文，不属于 Table 1，本次不再嵌入或作为表格证据。

### 本论文主图

| 原文图表 | 原文图题/核心信息 | 是否截取 | 图像文件 | 放置位置 |
|---|---|---|---|---|
| Fig. 1 | Overview of spatial omics technologies：空间转录组、蛋白组、代谢组、多组学和 3D/4D profiling 技术全景 | 是 | `assets/spatial-transcriptomics/2026-spatial-omics-review/fig1-overview-spatial-omics-technologies.png` | [03｜平台取舍](#03｜分辨率越高，平台就一定越好吗？) |
| Table 1 | Comparison of major commercial high-resolution spatial transcriptomic profiling platforms：高分辨率商业空间转录组平台比较 | 是 | `assets/spatial-transcriptomics/2026-spatial-omics-review/table1-commercial-spatial-transcriptomics-platforms-p1.png` | [03｜平台取舍](#03｜分辨率越高，平台就一定越好吗？) |
| Fig. 2 | Computational tools and analytical frameworks for spatial omics：空间组学数据处理、单细胞/亚细胞分析、空间组织、进化、多模态整合和 3D 重建 | 是 | `assets/spatial-transcriptomics/2026-spatial-omics-review/fig2-computational-tools-frameworks.png` | [04｜分析链](#04｜拿到空间坐标后，怎样避免停在漂亮地图？) |
| Fig. 3 | Spatial mapping of TME niches：肿瘤微环境中九类重要空间 niche | 是 | `assets/spatial-transcriptomics/2026-spatial-omics-review/fig3-spatial-mapping-tme-niches.png` | [05｜空间生态位](#05｜九类空间生态位，真正新增了什么信息？) |
| Fig. 4 | Hierarchy of spatial archetypes across scales：从细胞、niche、community 到 whole-tumor ecosystem 的空间层级 | 是 | `assets/spatial-transcriptomics/2026-spatial-omics-review/fig4-spatial-archetypes-hierarchy.png` | [05｜空间生态位](#05｜九类空间生态位，真正新增了什么信息？) |
| Fig. 5 | From spatial insights to clinical impact：空间组学从临床场景、研究设计、空间洞察到临床部署的路线图 | 是 | `assets/spatial-transcriptomics/2026-spatial-omics-review/fig5-spatial-insights-clinical-impact.png` | [06｜临床设计](#06｜怎样从临床问题反推样本和平台？) |
...
下文保留全部主图和 Table 1 的完整中文 panel 注释，不再重复嵌入读者正文已经展示的图像；图像文件和正文位置见“本论文主图”表。
...
中文图注（基于原文表格）：Table 1 比较六种商业高分辨率空间转录组平台，类别只有 cyclic decoding、cyclic in situ sequencing 和 spatially barcoded capture 三类，不含 barcode-to-cell。原表列为 Platform、Category、Resolution、RNA/Protein coverage、Tissue compatibility、Throughput/capture area 和 Run cost。它不是性能排名，也不是穷举；规格、通量和成本来自 2025 年时点的厂商或站点估计。不同研究问题仍需在分辨率、捕获面积、全转录组能力、组织兼容性、成本和多组学扩展之间取舍。

空间蛋白组和代谢组补充了 RNA 与功能状态并非完全等价的问题。蛋白组能直接读出受体、配体和免疫检查点等蛋白信号；代谢组能读出局部营养、脂质、药物分布和代谢微环境。空间多组学进一步把 DNA/RNA/蛋白/形态或 TCR/BCR 信息连接起来，但也带来配准、批次、模态尺度和组织消耗的困难。
...
下表只登记原综述正文实际点名的方法及其句子 ID；它说明作者综述了哪些计算路线，不表示作者亲自运行这些工具，也不构成统一数据集上的性能比较。

| 分析步骤 | 原文点名的方法或框架 | 来源句子 ID | 证据边界 |
|---|---|---|---|
| 细胞分割 | StarDist、DeepCell、Cellpose、Segger | `P006.S0014-P006.S0022` | 核/胞质信号与转录本辅助分割的路线概览 |
| 去噪、归一化与增强 | SpotClean、BayesTME、SPLIT、SpaNorm、SpatialPCA、SpaGCN、TESLA、iSTAR、UNI、KRONOS、Bin2cell、SMURF | `P006.S0023-P008.S0003` | 覆盖污染校正、空间保持、分辨率增强和 bin-to-cell 重建 |
| 细胞分型与去卷积 | Spatial-ID、STANN、BANKSY、RCTD、SPOTlight、cell2location、Tangram、Cytospace、Spotiphy、ScType、Celloscope | `P008.S0004-P008.S0011` | 同时包含有、无单细胞参考的方案 |
| 亚细胞分析 | TopACT、CellSP、Bento、PHOTON | `P008.S0012-P008.S0015` | 用于亚细胞转录本模式和基因模块，不代表功能因果 |
| 细胞通信 | CellPhoneDB、CellChat、Giotto、SOAPy、Squidpy（解析文本作 `Squipy`）、Spateo、CytoSignal、COMMOT、NCEMs、NEST、NicheNet、SpaTalk | `P008.S0016-P008.S0024` | 空间约束增强定位，仍不能证明真实接触或信号因果 |
| 生态位与群落 | nearest-neighbor、距离阈值、Delaunay triangulation、UTAG、CellCharter、scNiche、NicheCompass | `P008.S0025-P008.S0030` | 空间单位依赖邻接定义、输入特征和模型假设 |
| 轨迹、变异与克隆 | Monocle3、scVelo、Slingshot、CellRank、CytoTRACE、Spateo、stLearn/PSTS、TopoVelo；inferCNV、HoneyBADGER、CopyKAT、STARCH、SCmut、scSNV、SComatic、SpatialInferCNV、stSNV | `P008.S0031-P008.S0041`、`P009.S0009` | 表达推断的 CNV/SNV 与克隆结构仍需 DNA 或其他正交验证 |
| 3D 重建 | 光学/透明化体积成像、连续切片配准、CyCIF、CODA、连续 Visium 切片 | `P009.S0010-P009.S0017` | 破坏性与非破坏性路线的综述，不是同条件比较 |
| 空间基础模型 | UNI、LOKI、GigaTIME、ROSIE、CellPLM、scGPT-spatial、NicheFormer、KRONOS、Novae | `P009.S0018-P009.S0027` | 包含图像、图像—组学和组学中心模型；ROSIE 被原文明确称为非严格 FM |
| 多样本与多模态整合 | horizontal/vertical/diagonal integration、WNN、CCA、RPCA、Harmony、PASTE、STAligner、SpaMosaic、SpaMTP、MISO | `P009.S0001-P009.S0008`、`P009.S0028-P010.S0004` | 原文提醒分辨率、切片偏移、平台差异及过度校正风险 |

#### 编者补充的常用统计与分析框架

作为综述，本文没有作者原创的统计检验、P 值或回归模型结果。以下四类是编者为复现阅读补充的领域通用框架；其中 Ripley 类统计、permutation test、Cox/logistic 回归、ROC/AUC、cross-validation 和 calibration 均非原综述逐项列举或实际执行，不能反向归为作者 Methods。第一类是空间邻近和共定位分析：输入是细胞坐标、细胞类型或分子表达，目标是判断某些细胞类型是否比随机期望更常相邻或共定位。常见做法包括距离分布、nearest-neighbor enrichment、Ripley 类空间统计、permutation test 或 spatial autocorrelation。其局限是组织结构、细胞密度和采样区域会影响零分布。
...
样本物流是临床空间组学成败的前提。需要预先规定 ischemia time、固定方法、切片厚度、染色流程、区域选择、病理标注和 paired biospecimen。分析上，应在 protocol 中预设 primary spatial endpoint，而不是事后在海量空间特征中寻找最显著结果。TLS density、immune-excluded phenotype、target-positive tumor fraction、MRD niche score 或 spatial interaction signature 是编者根据综述主题整理的通用示例，并非原文逐项列出的终点。
...
自动解析把 `P017.S0009-P017.S0025` 的 17 个综述叙述 ID 标为 Results，又把 1,291 个 ID 标为 Methods。后者包含综述正文以及 1,013 个被误标的参考文献/页眉页脚 ID（`P018.S0062-P026.S0016`）；连同此前被正确标为 References 的 40 个 ID（`P018.S0022-P018.S0061`），整个参考文献及页眉页脚区域共有 1,053 个 ID。下表按综述真实结构闭合全部来源范围。
...
- Fig. 1 图注 A 部分来源为 `P002.S0004-P002.S0006`，B-E 续注为 `P003.S0042-P003.S0045`；Fig. 2 为 `P007.S0003-P007.S0007`；Fig. 3 为 `P010.S0017`；Fig. 4 为 `P013.S0017`；Fig. 5 为 `P015.S0003-P015.S0009`。完整 panel 解释和图像均在本文保留。
- Table 1 完整位于 PDF 第 4 页（印刷页 27），表体解析为 `P004.S0001-P004.S0035`，表注为 `P004.S0044-P004.S0046`；线性抽取因表格和双栏布局导致列值错序，本文只以该页原表截图核对，不根据错列文本补造规格。旧文件名带 `p2` 的截图片实际是 PDF 下一页正文，不属于 Table 1。
...
- TMA core 为 1.5–2 mm、包含数千细胞并可扩展到数百病例的表述来自 `P014.S0033`。原句支持的是保留中尺度结构的高通量相关性验证；它不代表覆盖全肿瘤异质性，也不证明因果机制或临床效用。

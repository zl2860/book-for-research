...
<!-- wechat-style-reviewed: 2026-08-15 -->
...
解离型单细胞测序擅长回答“有哪些细胞状态”，却会切断细胞原来的邻近关系、组织边界和区域梯度。两个肿瘤可以具有相似的 T 细胞比例，但一个肿瘤的 T 细胞与抗原呈递细胞形成免疫 hub，另一个肿瘤的 T 细胞被癌症相关成纤维细胞（CAF）和髓系细胞隔离；细胞清单相近，组织机制可能完全不同。
...
这里没有一个可汇总的患者样本量，也没有作者原创的效应量、P 值或 pooled comparison。26 页、5 张图、1 张表和 241 篇参考文献描述的是领域覆盖范围，不是“1,598 个解析句子 ID 等于 1,598 个实验结果”，更不能把不同癌种、平台和队列的发现当作同一条件下的横向排名。
...
作者提出的顺序是：先写清 clinical question 和 endpoint，再决定患者、时间点、部位、保存方式、平台和验证策略。癌前病变需要阶段性或纵向采样；转移研究需要原发灶、不同转移灶、正常组织和血液配对；微小残留病灶（MRD）与治疗反应则需要治疗前、治疗中、治疗后及复发前窗口。
...
- [领域方法学与复现提示](#领域方法学与复现提示)
  ...
  - [空间数据处理流程](#空间数据处理流程)
  - [被综述领域常用的统计与分析方法](#被综述领域常用的统计与分析方法)

...
功能分数的直接比较见 Supplementary Fig. S1：在 PCAWG 编码区，CADD 版 dNdS-Fun 与 dNdScv 的相关为 `r = 0.72`、`P = 6.4 × 10^-4`，高于 DANN 0.63、Eigen 0.33 和 FATHMM 0.37；TCGA 中 CADD 为 `r = 0.64`、`P = 1.2 × 10^-4`，其余三者为 0.55、0.13、0.31。这才是“CADD 与 dNdScv 最一致”的直接依据。

跨平台一致性是另一项检验：100kGP WGS 与 TCGA WES 的匹配癌种 dNdS-Fun `ω` 相关系数为 `r = 0.68`、`P = 3.7 × 10^-4`；PCAWG WGS 与 TCGA WES 为 `r = 0.53`、`P = 2.4 × 10^-3`。它支持队列层方向相近，但不能替代逐变异或逐患者复现。
...
相对五组既往非编码驱动候选，本文重现 Sherman 80%（8/10）、Shuai 71%（58/82）、Rheinbay 67%（28/42）、Dietlein 69%（20/49）和 Kumar 55%（130/238）。这些分母与 Table S2 的五组记录数一致，但部分研究与本文共享 PCAWG 等数据；它们衡量的是候选重叠，不能替代完全独立的灵敏度或特异度验证。原文把 Sherman、Rheinbay 后面的括号内引文对调，不能据错误引文重新分配数字。

功能分数本身也做了一个更直接的方向检查。在已知癌症调控区域中，高功能分突变占比超过 0.5 的 gene–tissue pairs 分别为启动子 93/110、5′UTR 41/54、3′UTR 3/5，合计 137/169。这个结果支持“高分突变在已知调控驱动区域富集”，但比较单位仍是既有癌症区域中的 gene–tissue pair，不能给新候选提供独立的假阳性率。
...
排除已知驱动基因突变后，多数癌种的编码区 `ω` 进一步降到 1 以下。这支持作者提出的解释：少数强正选择驱动可能遮盖背景中的负选择信号；但该分析没有直接测量克隆适应度。排除高突变负荷癌种、低可比对位点，并按三核苷酸、复制时序、染色质状态或逐癌种留一法分层后，总体方向仍保持。这里是全局负选择的敏感性分析，不沿用方法 benchmark 中“三类癌种、平均每人超过 35,000 个突变”的定义；两者口径见技术附录。
...
作者把局部分析概括为 503 个基因中的 1,271 个受正选择“元件”；官方 Table S1 实际是 1,271 条 element–tissue–cohort 记录，折叠后为 768 个“坐标＋区域”元件、730 个物理区间，不能把行数理解成 1,271 个唯一坐标。作者进一步把“在多个癌种重复出现”或“同时获 100kGP 与 PCAWG 支持”的 175 个基因定义为高置信候选，其中包括 TP53、PIK3CA、PTEN、CDKN2A 和 KRAS。
...
这里的“仅由非编码证据识别”是作者的分类标签：启动子、UTR 和剪接位点都被归入 `Noncoding`。但同一论文又把可能改变蛋白序列的剪接位点列为 Tier 1，把启动子和 UTR 列为 Tier 2。正式补齐的 Table S1 虽给出区域列，却没有标记 175 个高置信、69 个新候选或 30 个非编码独有候选的成员；正文点名的 RUBCN 和 DNAI3 也不在该表。现有材料因此仍不能把 30 个候选完整拆成“纯调控区域”与“依赖剪接位点”，更不能把它们一概解释为只受调控性非编码突变驱动。
...
主 Results 报告 120 个在至少两个癌种或两个队列中受正选择的内含子/基因间元件；EpiMap 与单细胞 ATAC 的 ABC 调控图谱为其中 64 个注释到靶基因，50 个具有组织特异性。在这 50 个元件中，原文称 25 个至少连到一个 CGC 基因、36 个至少连到一个转录因子、10 个至少连到一个激酶——这里计数的是**元件**，不是 25、36、10 个基因。官方 Table S5 又只列出 48 个坐标，其中相应三列非空的分别为 17、24、6 行；120→64→50 与表中 48 行无法闭合，不能拿补表数字静默替换正文数字。

不过，表达验证很弱。只有 PMM1 启动子和 MRPS33 5′UTR 的突变状态与表达差异在多重校正后显著；每个癌种同时具有 PCAWG 突变和 TCGA 表达的样本最多 51 例，多数元件的突变样本不足 6 例。

503 个候选共富集 168 个 GO 条目和 132 条 KEGG 通路，`FDR < 0.05`。主 Results 与 Fig. 4 在 FoxO 网络中突出 20 个受正选择基因，其中 14 个已在 CGC、6 个此前未报告；官方 Table S4 的 FoxO 行却列出 27 个基因，且把原始 P 值与校正后 P 值都写成 `4.60 × 10^-15`。这两个候选口径没有解释，不能合并成同一分母；通路富集本身也不能证明每个候选都通过 FoxO 驱动肿瘤。
...
免疫球蛋白位点是另一类特殊结果。主文称在淋巴瘤中识别到 20 个受正选择的 Ig 基因；官方 Table S7 实际列出 20 个元件、对应 19 个唯一最近基因符号，因为 IGLV3-1 占两行，而 Supplementary Fig. S29 的比较又是 16 个显著与 197 个不显著 Ig genes。S7 中 IGHJ6、IGLL5 两行的 tissues 还同时包含 Testicular，因此也不能把全部记录严格限定为淋巴瘤。它们分布于染色体 2 的 IGK、染色体 14 的 IGH 和染色体 22 的 IGL 位点；补充分析用于排查 BCR 重排和体细胞高突变造成的 CADD 错配。这组信号更可能主要反映淋巴系统肿瘤中的免疫受体演化，不能直接外推到其他实体瘤。
...
主 Results 报告 16 个基因呈现“同一癌种内全基因负选择、非编码元件正选择”的双向模式。Table S9 的 88 条区域记录虽也折叠为 16 个唯一基因，严格按同一 tissue 重建却只有 14 个符合该方向；CTNNB1 是反向的 Tier 1 正/Tier 2 负，TMEM102 的两种方向又出现在不同 tissue，因而不能拿“16 个唯一基因”直接确认正文的 16。

TERT 最典型。作者称启动子在 15 个癌种受正选择、编码区在 5 个癌种受负选择；S1 的 TERT 启动子记录去掉 pan-cancer 后是 15 个 cohort–tumor entries，却只有 13 个唯一癌种标签，S9 的负向记录则是 4 个非泛癌标签，加上 pan-cancer 才为 5。“15/5”因此依赖队列重复和 pan-cancer 是否计数，不能直接解释为 15/5 个唯一癌种。

正文另称 C228T 与 C250T 热点涉及 1,041 名患者，但 Table S10 两行分别记录 799 与 243 次、合计 1,042 次。该表没有患者 ID，无法判断是否有一名患者同时携带两种热点，也就不能用行数反推唯一患者数。作者据此提出，区域相反的选择方向或可解释为什么 TERT 驱动改变主要集中在启动子；但本文没有直接测量 TERT 表达、端粒酶活性或编码变异的细胞适应度，不能把“提高调控活性、保留蛋白功能”写成已验证机制。
...
作者在 Discussion 中提出四种可能解释：非编码区约占基因组 98%，因而提供了很大的潜在有害突变靶；剪接位点受到较强约束；关键调控元件被破坏后可能重排必需基因网络；异常剪接还可能产生新的开放阅读框（neoORF），继而触发免疫清除。

这四项解释来自既有文献与作者讨论，并不是本文逐项完成的机制实验。研究没有在这些候选上直接测量剪接、调控网络破坏、抗原呈递或免疫清除，因此当前数据能支持的是“高功能分突变相对缺失”，不能在四种机制之间排出主次。
...
作者还点名 PARP2、PARP12、PARP16 和 TIPARP 四个负选择候选，把它们作为合成致死筛选的入口。这只是候选生成：本文没有做合成致死、药敏或临床验证，也不能由 BRCA1/2–PARP 抑制剂这一既有范例反推出四个候选已经可用药。

证据等级也不能混在一起。除 175 个跨癌种或跨队列的高置信候选外，研究另有 328 个只在单一癌种或单一队列出现的正选择基因；后者值得复查，但不能共享前者的“高置信”标签。
...
局部分析的癌种范围也没有闭合。Results 和 Fig. 3 以 31 个癌种及泛癌为口径，图中包含结直肠癌；Methods 却说因为 100kGP 缺少 MSI 状态，局部分析排除了结直肠癌。Table S1 进一步列出 132 条 `Colorectal` 记录，涉及 103 个基因，其中 127 条来自 100kGP、5 条来自 PCAWG；它强化了冲突，却仍没有说明排除发生在哪个分析层或哪一版结果。不能把 Fig. 3 简单视为“31 个癌种全部使用同一纳入规则”。
...
- 期刊：Journal of Genetics and Genomics；出版社元数据显示 2026-06-17 在线发表，本地论文文件仍为 Journal Pre-proof
- 年份：2026；修订日期 2026-06-12，接收日期 2026-06-14
- DOI：10.1016/j.jgg.2026.06.008
- PubMed：PMID 42309349
...
- 补充材料：本地 PDF 内含 Supplementary Fig. S1–S30 及图注，但未嵌入 Supplementary Tables S1–S11。此次另从出版社正式附件逐表审计 S1–S11；它们补齐候选和队列汇总，却仍没有样本级 ID、测序深度或完整 QC/过滤明细。
...
### 官方 Supplementary Tables S1–S11 审计（不计入主文 ID）

- 出版社附件：[Supplementary Figures S1–S30](https://ars.els-cdn.com/content/image/1-s2.0-S1673852726002146-mmc1.pdf) 与 [Supplementary Tables S1–S11](https://ars.els-cdn.com/content/image/1-s2.0-S1673852726002146-mmc2.xlsx)。复审文件 SHA-256 分别为 `ad619ba51a48da223d3b9d61a928b6d2f2b1770c0dfcff5ee62a66537417a0eb` 和 `f3f601186e5ef66af5a864574af0b1c3a7c50bb81e0d27a89b2261a863d27957`。
- 工作簿有 11 个工作表。它独立于本地 73 页 PDF 和 1,141 个 `Pxxx.Sxxxx`，以下以表号和数据行定位，不给补表伪造正文句子 ID。

| 表 | 工作簿对象 | 能确认什么，以及仍未闭合什么 |
|---|---|---|
| S1 | 1,271 条 element–tissue–cohort 记录、503 个唯一基因 | 折叠后为 768 个“坐标＋区域”元件、730 个物理区间；区域记录为 whole-gene 349、promoter 269、CDS exon 266、5′UTR 184、3′UTR 120、splice site 83。表中没有 high-confidence、known/novel 或 noncoding-only 标志；按基因简单重建“至少两个非泛癌癌种或双队列”为 130 个，把 pan-cancer 计入则为 222 个，均非 175，说明作者还有未明示的筛选/聚合层。表中另有 132 条结直肠癌记录。 |
| S2 | 421 条既往非编码驱动记录 | 可审计五组文献候选重叠；共享队列和引文编号疑似互换的边界仍在。 |
| S3 | 168 条 GO 富集记录 | 条目数与 Results 一致，原始与 BH 校正 P 值分列。 |
| S4 | 132 条 KEGG 富集记录 | 条目数一致；FoxO 行为 27 个基因而主文/Fig. 4 写 20 个。132 行的 `P-value` 与 `Adjusted P-value` 又逐行完全相同，无法判断导出列是否重复或原 P 值已预先校正。 |
| S5 | 48 个内含子/基因间坐标 | 主文称 120→64→50；表仅 48 行，且 CGC、dbTF、kinase 三列分别只有 17、24、6 行非空，不能复现主文的 25、36、10 个元件。48 行展开为 81 个癌种标签实例，不等于 120 个唯一坐标；`chr8:56987069-56987150` 同列两组 cohort/ω/Q，却只给一个 mutations=25，无法逐队列对账。 |
| S6 | 21 个癌种/亚型加汇总行 | PCAWG 共 2,583 例，其中 736 例有相应表达数据；表下注称 430 例可用于验证。脚注把 `nGene_mut` 定义为至少 2 个突变样本，Summary 却称 126 个候选中 37 个有超过 2 个，实际枚举最小为 3；阈值是 `≥2` 还是 `>2` 没有闭合。逐癌种 PCAWG donor 数又与 S11 多处不一致，例如 meta_CNS 为 287 对 271，尽管总数同为 2,583。 |
| S7 | 20 个 Ig 元件、19 个唯一最近基因符号 | IGLV3-1 对应两个坐标；Supplementary Fig. S29 又按 16 个显著 Ig genes 作图，因此 20 genes、20 elements、19 symbols、16 significant genes 是四个未解释的对象口径。 |
| S8 | 10 类外部基因集及 CancerHMPath 的 21 条 hallmark pathways | 原始规模与 Fig. 5A–B 图示规模多处不同：dNdScv 369→343、EMOGI 791→753、IntOGen 564→526、PATHOGEN_GERM 407→389、EssentialGene 1,877→1,826、CancerHMPath 1,542→1,481；可能是与 20,185 个蛋白编码背景取交集，但 Methods 没有说明，不能自行认定。 |
| S9 | 88 条区域选择记录、16 个唯一基因 | 严格按同一 tissue 重建只有 14 个 Tier1−/Tier2+，另有 CTNNB1 反向、TMEM102 跨 tissue；既不能逐一闭合 Results 的 16，也不能解释 Fig. 5C 的 20。 |
| S10 | 12 条 TERT 非编码变异 | C228T 为 799 次、C250T 为 243 次，合计 1,042 条热点记录；表无患者 ID，无法与正文 1,041 名唯一患者直接对账。 |
| S11 | 41 条队列/癌种汇总记录 | 100kGP 为 12,303 donors、281,491,030 个突变；PCAWG 为 2,583 donors、45,304,280 个突变。它补齐癌种级样本量，但不含样本 ID、深度、逐样本 QC 或变异过滤。 |

补表使“表不存在”这一旧边界失效，却没有消除高置信 175、非编码独有 48/50、FoxO 20/27、内含子/基因间 120/48、Ig 20/19、TERT 1,041/1,042 等对象或分母冲突；以下正文与 Methods ID 审计仍按本地 pre-proof 原文保留。
...
| Fig. 4 | FoxO 及相关信号网络；橙色为有编码证据的 CGC，黄绿色为有非编码证据的 CGC，深绿为有编码证据的非 CGC，红色为有非编码证据的非 CGC；后三类突出此前未报告的选择证据。 | 通路富集基于 503 个候选；主文/Fig. 4 突出 20 个，Table S4 的 FoxO 行列 27 个；校正后 `P = 4.60 × 10^-15`。 | `fig4-foxo-pathway.png` | [06｜哪些非编码候选最值得关注](#06｜哪些非编码候选最值得关注) |
| Fig. 5 | A–B：负、正选择基因与癌症驱动、依赖和必需基因集的重叠。C：Tier 1（CDS、剪接、全基因）与 Tier 2（启动子、UTR）方向相反的基因及涉及癌种数。D：各癌种 TERT C228T/C250T 热点比例和人数。E：按样本数和区域长度标准化的 TERT 各区域突变负荷。F：参考基因组、100kGP 和 PCAWG 中 TERT CDS/启动子 CADD 分布。 | 图注称 A–B 用 Fisher’s exact test 并作 FDR；Results 称 5,000 组按数量和长度匹配，Methods 仅写从 20,185 个基因中等量重采样。C 图面为 20，而 Results/Table S9 为 16；D 正文 1,041 patients、Table S10 为 1,042 calls。E 用 Student’s t-test；F 给出分布检验 P 值。 | `fig5-negative-selection-tert.png` | [07｜负选择如何改写我们对 TERT 的理解](#07｜负选择如何改写我们对-tert-的理解) |
...
| `P010.S0013–P011.S0005` | 原文称 1,271 个元件、503 个基因、175 个高置信候选、69 个新候选；9 个非编码独有、12 个编码与非编码共同、7 个编码独有热点。 | [06](#06｜哪些非编码候选最值得关注) | S1 显示 1,271 实为 element–tissue–cohort 行，只有 768 个区域感知元件/730 个坐标；175 的成员与聚合规则仍需代码复核。 |
...
### Discussion 与读者正文边界索引

Discussion 不计入上面的 225 个 Results ID，也不计入后面的 148 个 Methods ID。读者正文中的机制解释、转化出口与限制来自以下独立范围：

| 原文句子 ID | Discussion 内容 | 正文对应位置 | 证据边界 |
|---|---|---|---|
| `P015.S0001–P016.S0008` | 175 个高置信候选、69 个既往未列驱动、非编码独有数量冲突、OncoKB 21/80，以及另有 328 个单癌种/单队列候选。 | [06](#06｜哪些非编码候选最值得关注)、[09](#09｜这项工作真正改变了什么)、[10](#10｜这些结果仍需要冷静看待) | 328 个候选不满足 175 个候选的跨癌种/跨队列门槛；可行动基因重叠不等于具体变异可用药。 |
| `P016.S0009–P017.S0016` | `ω` 的相对约束含义、负选择四项可能机制、TERT 的作者解释，以及 PARP2、PARP12、PARP16、TIPARP 的合成致死候选出口。 | [07](#07｜负选择如何改写我们对-tert-的理解)、[08](#08｜为什么会看到全基因组负选择信号)、[09](#09｜这项工作真正改变了什么)、[10](#10｜这些结果仍需要冷静看待) | 四项机制来自作者结合文献的解释；本文未直接检验 TERT 调控机制、合成致死或药物反应。 |
| `P017.S0017–P018.S0008` | 分数二分、单样本不可用、癌种样本不均衡与欧洲祖源等限制。 | [10](#10｜这些结果仍需要冷静看待) | 这些是作者 Discussion 的适用边界，不是新的独立验证。 |
...
- 本地 PDF 未包含 Table S11；此次官方工作簿已补齐癌种级 donor 与突变总数。它仍不含样本 ID、样本 QC、测序深度和变异过滤明细，且与 Table S6 的部分 PCAWG 癌种计数不一致。
...
- 作者在候选标签中把启动子、UTR 和剪接位点合并为 `Noncoding`（`P024.S0018`），但 Tier 定义又把剪接位点与 CDS、全基因归入可能改变蛋白序列的 Tier 1，而启动子和 UTR 属于 Tier 2（`P014.S0015–P014.S0017`）。Table S1 虽补齐区域列，却没有 high-confidence、known/novel 或 noncoding-only 标志，因此仍不能把 30 个候选的 `Noncoding` 标签等同于“纯调控非编码”。
...
- 原文未报告 dNdS-Fun 的 commit/tag、运行环境和多数依赖版本。截至 2026-09-15，官方仓库 `main` 为 `13f7736dfb81b44b8732906a75ac5bfbac883ad0`，另有 `v1.0.0` 标签 `308855916e4cb36b922fdd975c716ca2e8a02890`；二者都不能反推论文实际运行的代码快照。仓库 `DESCRIPTION` 写 R ≥4.0.5、包版本 1.00 和 GPL ≥3，README 另列 data.table、MASS、doParallel、foreach、GenomicRanges、Biostrings、tabix 等最低版本；迁移时仍应同时固定代码、参考基因组、功能分数、注释和二分方向。
...
| `P022.S0009–P022.S0017` | 100kGP、PCAWG、TCGA 获取、合并和纳入规则。 | 100kGP/PCAWG 是主 WGS，TCGA 是 WES 评估。 | S11 只补齐队列/癌种汇总；样本级 ID、QC、深度和过滤仍缺。 |
...
| `P026.S0007–P026.S0022` | 数据与代码资源。 | 提供主数据库、GitHub、在线门户和 Zenodo。 | PDF 的 URL 顺序破碎；官方补表已审计，但论文运行 commit 与环境锁定仍缺。 |
...
- 503 个基因有局部正选择证据；作者报告其中 175 个满足跨癌种或跨队列高置信规则，但 S1 没有成员标记，简单聚合也不能复现 175。
- 225 个负选择基因在必需、癌症依赖和群体约束指标上富集。

**合理但尚未直接证明：**

- 统计候选中的每个非编码突变都是肿瘤驱动。
- 非编码靶大小、剪接约束、调控网络破坏和 neoORF 免疫清除四项机制中的任一项主导了本文观察到的负选择。
- TERT 启动子正选择与编码区负选择已经通过表达、端粒酶活性或细胞适应度实验建立了机制因果链。
- 负选择基因就是可药用的合成致死靶点。
...
- 元件与高置信对象：Results 把 1,271 称为 elements，S1 实为 1,271 条 element–tissue–cohort 记录、768 个区域感知元件、730 个坐标。S1 又没有 175 个高置信成员标志；简单按“至少两个非泛癌癌种或双队列”聚合得到 130 个基因，把 pan-cancer 纳入则为 222 个，均不能复现 175。
- 内含子/基因间元件：Results 为 120 个，其中 64 个连到靶基因、50 个组织特异；S5 只有 48 个坐标，相应 CGC、TF、kinase 列为 17、24、6 个非空元件，而不是正文的 25、36、10。差异可能来自折叠/筛选层级，但附件没有映射表。
- FoxO 基因数与 P 值：Results/Fig. 4 为 20 个（14 CGC + 6 new），S4 的 FoxO 行列 27 个正式基因符号；S4 全部 132 行的原始 P 与 adjusted P 又完全相同，而 S3 并非如此。
- 双向选择基因数：正文写 16 个“全基因负选择、非编码正选择”基因，Fig. 5C 柱图却显示 `Tier1−Tier2+` 为 20 个、反向为 1 个；两者可能使用不同过滤层级，不能直接互换。
- `Noncoding` 分类：`P024.S0018` 把启动子、UTR 和剪接位点合并为非编码证据，`P014.S0015–P014.S0017` 却把剪接位点列入可能改变蛋白序列的 Tier 1；S1 没有 noncoding-only/novel 标志，且正文点名的新候选 RUBCN、DNAI3 不在 S1，30 个“非编码独有”候选仍无法完整拆分。
- Ig 对象数：Results 为 20 Ig genes，S7 为 20 elements/19 nearest-gene symbols，Fig. S29 为 16 significant/197 non-significant genes；S7 的两行还同时标注 Testicular。四种口径不能互换。
- TERT 热点分母：Results 为 1,041 patients，S10 的两条经典热点合计 1,042 calls；缺患者 ID，无法验证是否由一名双热点患者解释差 1。
- 局部分析范围：`P024.S0013` 称在 31 癌种及 pan-cancer 计算局部 `ω`，Fig. 3 图面含结直肠癌；`P024.S0017` 又称因 100kGP 无 MSI 状态而排除结直肠癌。S1 仍有 132 条 Colorectal 记录，其中 127 条来自 100kGP，原文未说明排除适用于哪个队列、分析层或结果版本。
- 队列计数：S6 与 S11 的 PCAWG 总数同为 2,583，但多个癌种不一致，例如 meta_CNS 为 287 对 271；两表可能使用不同映射/筛选层，脚注未说明。
- 膀胱癌负选择：`P009.S0010` 把膀胱癌内含子称为非编码负选择的“唯一例外”，`P009.S0015` 又报告膀胱癌基因间区 `ω = 1.03`；可能涉及显著性或区域口径差异，不能把“唯一例外”解释为所有非编码区域都低于 1。
- 既往候选复现：S2 的 10/82/42/49/238 条记录确认 Sherman/Shuai/Rheinbay/Dietlein/Kumar 的研究名—分母映射；错的是原文 Sherman、Rheinbay 后的括号内文献引文，而不是比例归属。
- 补图交叉引用：Supplementary Fig. S21 图注把既往 noncoding drivers 指向 Table S9，实际候选清单位于 Table S2；S9 是双向选择基因表。
...
- 高置信门槛：引言摘要写“超过两个癌种或两个队列”，详细 Results/Discussion 写“多个或至少两个癌种”，边界是 `>2` 还是 `≥2` 不够一致；S1 的简单重建又无法得到 175。
- 数值精度：S1 有 277 行、S9 有 35 行把 P 与 Q 都导出为精确的 `0`；统计概率不应解释为字面零，附件未给下溢阈值或额外有效位数。
...
- [x] 原文冲突、官方 S1–S11 附表冲突和解析低置信内容已保留；
- [x] `STYLE_REVIEW_LOG.md` 已更新，分类 README 与 `SUMMARY.md` 经核对无需改动；

...
<!-- wechat-style-reviewed: 2026-09-07 -->
...
以 acetylcarnitine 为例，叠加前单细胞信噪比为 3.3，600 个细胞叠加后为 157.6；100 个测试代谢离子的中位信噪比提高 **33 倍**。跨 100 个代谢物的平均掉点率由 **82% 降至 6%**；acetylcarnitine 由 83% 降至 1%，carnitine 由 13% 降至 0%。
...
MetCell 依次完成单细胞帧识别、细胞叠加、峰检测、逐细胞定量和多维注释。一次 120 分钟 HepG2 采集中，它识别 3,094 个单细胞帧、检出 5,718 个代谢峰；其中 1,356 个峰通过 MS1 与碰撞截面（CCS）匹配得到候选，1,350 个再从**另一批单细胞**定向采集 MS2，最终注释 770 个代谢物。换句话说，MS2 增强了峰身份的证据，却不是在前述每一个单细胞上同步完成的逐细胞结构确认。

按 Metabolomics Standards Initiative 分级，这 770 个注释包括 level 1、2、3、4 各 188、126、332、124 个。跨多类细胞共有 **389 个**独特代谢物以化学标准品验证为 level 1；其他常见细胞系每类检出 5,074–6,975 个峰、注释 533–622 个代谢物。比较对象是既有方法通常基于 MS1 给出的 80–300 个推定注释，而不是同一批样本上的统一头对头基准。
...
作者不是逐细胞另做一次 LC–MS，而是用群体 LC–MS 的量给单细胞质谱流式强度定标。主分析把 LC–MS 的每细胞中位量与 mass cytometry 的单细胞强度中位数对齐，再作线性换算；作者另用两平台均值对齐做敏感性分析，并称结果近似一致。Fig. 4 同时把 6 次 LC–MS 重复展示为带误差的群体值，形成术语口径张力。因此，下面的绝对量是依赖尺度锚定的**估计值**，不是每个细胞的独立绝对定量。
...
第二个验证使用能报告 NAD⁺ 的 mCherry-FiNad-293T 细胞。FK866 处理后，荧光流式测得相对丰度为对照的 0.727，离子迁移 mass cytometry 为 0.856；传统处理因大量低信噪比掉点给出 1.0，未能识别下降。两个平台的效应大小并不完全相同，但方向和分布分离一致，这比只报告一个显著 P 值更有信息。
...
在 38,536 个已注释细胞中，年轻组 20,042 个、老年组 18,494 个。HSC 比例从年轻组的 6.8% 升到老年组的 9.4%（每组 6 只小鼠，P=0.04）；这一结果与既往免疫组化方向一致，但效应估计仍来自较小的动物层样本。

作者把肝细胞按代谢谱排成四个 pseudotime 状态：状态 1、2、3、4 中老年细胞分别占 **16%、64%、65%、86%**。状态 3 的老年肝细胞富集 arginine、hypoxanthine、spermine 和多种 diacylglycerol；状态 4 则富集 adenosine、nicotinamide、glutathione 与多种寡糖。
...
第五，**生物学验证仍是横断面小鼠设计，而且细胞数不能替代动物数**。主要年龄比较只有 12 周与 90 周两个端点、每组 6 只雄鼠；60 周补充数据虽有 8,545 个肝细胞，却只来自 2 只雄鼠。state 3/4 的差异代谢检验以 1,832 对 6,989 个细胞呈现，Supplementary Methods 没有说明把小鼠作为分层或随机效应，存在伪重复风险。pseudotime 不能当作真实纵向轨迹，结果也不能外推到雌鼠。

第六，**组织解离会改变代谢物**。作者自己指出，临床组织消化可能引入代谢伪影；稀有细胞也可能在混合样本中被忽略。研究中的培养细胞和即时处理小鼠肝脏，不能替代复杂患者标本的系统稳定性评估。

第七，**主文句子 ID 与补充材料必须分层**。本地 29 页主文 PDF 不含补充文件；本次复审已从 Nature 官方 Supplementary Information 定点核验 Supplementary Methods、Figs. 1–26 和 Tables 1–6，补出了 60 周龄 `N=2`、PRM-PASEF 与若干下游参数，但碰撞能、阈值语义和统计模型仍未闭合。Supplementary Data 1–6、独立 Source Data 与视频本体也未纳入本地审计；补充 PDF 没有冒充主文的 `Pxxx.Sxxxx` 来源 ID。
...
| 10（检测先验） | 前 600 个细胞、稀有峰与批次边界 | `P013.S0001–P013.S0038`、`P018.S0002–P018.S0010` |
| 10（结构鉴定） | 分辨率、未注释 MS2 与谱图相似度边界 | `P009.S0035–P009.S0048` |
| 10（离子模式） | 正离子模式、膜破坏、离子抑制与溶解度限制 | `P009.S0010–P009.S0018`、`P009.S0042–P009.S0047` |
| 10（动物设计） | 两端年龄每组 6 只雄鼠、中间年龄 2 只雄鼠与细胞嵌套边界 | `P005.S0051–P005.S0055`、`P008.S0045–P009.S0001`；Reporting Summary 第 2–3 页；官方 Supplementary Information 内页 S5–S7 |
| 10（组织解离） | 临床组织消化伪影与稀有细胞遗漏 | `P009.S0054–P010.S0005` |

09 的价值判断综合 03–08 的结果链，11 的迁移建议综合主 Methods `P012.S0002–P014.S0028` 与 Discussion `P009.S0002–P010.S0005`；二者没有被伪装成新的原文结果。
...
- 本地主论文 PDF 不含 Supplementary Information、Supplementary Data 1–6、Supplementary Video 或独立 Source Data。此次复审另外下载并定点核对了官方 Supplementary Information；Data、Source Data 与视频仍未摄入，不能把“补充 PDF 已看”写成全部开放材料均已复现。

### 官方 Supplementary Information 定点复审（不计入主文 ID）

- 来源为 Nature 文章页链接的[官方 Supplementary Information](https://media.springernature.com/original/springer-static/esm/art%3A10.1038%2Fs41592-025-02970-2/MediaObjects/41592_2025_2970_MOESM1_ESM.pdf)。该文件共 44 个 PDF 页面，正文页码为 S1–S43；它独立于上述 29 页主论文和 1,038 个 `Pxxx.Sxxxx` ID，以下只用补充页码/表号定位，不伪造主文句子 ID。
- Supplementary Methods（S4–S5）确认 60 周龄队列为 **2 只雄鼠**、12 周和 90 周主队列各 6 只，并补出 140 mM ammonium formate（pH 7.3）及最终 `4×10^5 cells ml^-1` 的上机细胞密度。每只小鼠分 2–4 批采集、每个肝样本至少采 2,000 个细胞的批次信息另见 S7。
- PRM-PASEF（S5–S6）写明前体隔离窗 `m/z ±0.6 Da`、淌度窗 `±0.03 V s cm^-2`，每个离子采 1 min、约 10–50 张 MS2 谱，共识谱中位数为 31 张；但 Supplementary Methods S5 的文字写碰撞能 30 eV，Supplementary Table 5（S41）却把 low/high 两档都列为 10 eV。两者可能属于不同层级设定，也可能是报告冲突，原文未解释，本文不替作者二选一。
...
- LC–MS 与流式（S8–S9）补出 5.43×10^6 cells/plate、提取液 ACN/MeOH/water `2:2:1`、Vanquish UHPLC–Orbitrap Exploris 480、Skyline 22.2，以及 FK866 `10 nM, 24 h` 和 GFP/mCherry 比值等条件。标准曲线浓度点、回归权重、完整门控、电压与逐鼠 QC 仍不足。
- Supplementary Fig. 13（S23）的 Oil Red O/PAS 图像每个年龄组展示 3 只小鼠，未见图像层面的定量；Supplementary Fig. 14（S24）把 60 周龄 8,545 个细胞分成 2,856/2,931/2,758 三簇。两者都是方向性补强，不能把组织图像或 N=2 小鼠写成独立的大规模验证。
...
- d：1,356 个候选中 1,350 个从另一批单细胞定向取得 MS2，最终得到 770 个 level 1–4 注释；这不是在前述每个单细胞上同步获得的逐细胞 MS2。
...
- b–c：LC–MS 的 6 次重复以平均量展示，主换算把群体量锚定到 mass cytometry 单细胞分布中位数；另有 mean-to-mean 敏感性分析。b 为 adenine，c 为 nicotinamide。
- d：mCherry-FiNad-293T 细胞的 NAD⁺ 由荧光流式与 IM-resolved mass cytometry 交叉验证。
- e–f：对照与 FK866 组的 NAD⁺ 相对丰度；荧光流式为 control `N=10,594`、FK866 `N=10,770`、ratio 0.727，IM-resolved mass cytometry 为 control `N=1,610`、FK866 `N=1,544`、ratio 0.856，两者均 `P<2.2×10^-16`。图中 N 是单细胞事件数，P 值用双侧 Wilcoxon 检验；箱线图为中位数、Q1–Q3 与 1.5×IQR 须线。数值与组别由主图目视复核。
...
- h：两个状态的 maltopentaose、maltotetraose、maltotriose；主图图面依次列出 FC 1.52、1.25、1.11，`P=6.6×10^-117`、`2.9×10^-75`、`4.5×10^-19`。state 3 和 4 分别含 1,832 与 6,989 个老年肝细胞，箱线图为中位数、Q1–Q3 与 1.5×IQR 须线；图注只明确 f 的差异分析经 Bonferroni 校正，未说明 g/h 标签是校正前还是校正后 P 值。由于图注也未定义 FC 的分子/分母，本文保留图面数值而不据此另推方向。
...
- 120 min、3,094 单细胞帧、5,718 peaks；1,356 个以 MS1+CCS 初注释；其中 1,350 个从另一批单细胞取得定向 MS2，得到 770 个注释：level 1/2/3/4 = 188/126/332/124（`P003.S0024–P003.S0040`；批次关系见 Supplementary Methods S5）。
...
- 腺嘌呤 18.4±1.4 amol/cell，单细胞映射范围 7.5–32.4 amol，LOD 8.2 amol/cell；烟酰胺/精氨酸/丙氨酸均值、范围和 LOD 见 §3.4。主换算把 LC–MS 群体量锚到单细胞强度中位数，mean-to-mean 仅作为敏感性分析（`P004.S0039–P005.S0024`；Supplementary Methods S8–S9）。
- NAD+ 面板的文本包保留荧光两组 `N=10,594`、`N=10,770`，质谱流式两组 `N=1,610`、`N=1,544`（`P006.S0040–P006.S0043`），但二维顺序丢失；主图目视复核恢复为荧光 control/FK866 `10,594/10,770`、质谱流式 control/FK866 `1,610/1,544`，两平台 ratio 分别为 0.727/0.856。
...
- state 3 vs 4 差异代谢物使用双侧 Wilcoxon，Fig. 6f 的差异分析经 Bonferroni 校正（`P008.S0038–P008.S0040`）。文本包单独不能恢复图面映射；主图目视复核显示 DG(38:4)/DG(36:2)/DG(36:4) 的 FC 为 0.70/0.70/0.74、P 为 `5.2×10^-13`/`3.9×10^-35`/`1.6×10^-12`，三种寡糖的 FC 为 1.52/1.25/1.11、P 为 `6.6×10^-117`/`2.9×10^-75`/`4.5×10^-19`。图注未说明 g/h 标签是否为校正后 P 值，也未明示 FC 分子方向，不据数值另推方向。
...
| 单细胞量是否可信？ | 群体 LC–MS 锚点 vs 单细胞强度分布；FK866 下荧光 vs IM 平台 | 线性换算的绝对量范围、LOD、NAD+ 相对比 | amol–fmol 量级可估计，药理扰动方向与独立平台一致 | 主分析为群体量对单细胞中位数的尺度锚定，mean-to-mean 是敏感性分析；不是同一细胞配对；0.727 vs 0.856 非完全一致 |
...
- 绝对定量/LOD：扩展图说明用 LC–MS 标准曲线定量 adenine、nicotinamide、arginine、alanine；单细胞质谱流式 LOD 通过“强度—SNR”线性回归估计，并把 SNR=3 的强度转换为 amol/fmol（`P023.S0003–P023.S0010`）。Supplementary Methods S8–S9 补出提取体积/溶剂、UHPLC–Orbitrap 平台、Skyline 和内外标策略，但标准曲线浓度点、回归权重、残差诊断及完整反算公式仍未给出。
...
- **独立开放文件**：Supplementary Data 1–6、Source Data、Supplementary Video、原始 MS 数据和 MetCell 归档代码未在本次本地审计中摄入；峰表、候选库快照、逐样本 QC 与原始—最终细胞追踪仍不能核验。
- **细胞与动物元数据**：培养基/温度/汇合度和主要解离步骤已有补充说明，但传代号、试剂 lot、禁食/取材时点、细胞活率阈值、每鼠进入质谱的细胞数与丢失原因仍不完整。
- **上机与 LC–MS**：缓冲液、细胞密度、提取液、仪器和内/外标已有说明；上样体积/等待时间、LC 梯度、标准曲线点与权重、线性范围、QC、回收率和基质效应仍不足。
- **流式与染色**：FK866 剂量/时长、GFP/mCherry 比值以及固定/切片/染色步骤已补出；激光/滤光片、电压、补偿、双细胞门、代表性 gating 图、成像参数和定量规则仍欠缺。
- **MetCell 与下游分析**：Table 6 和 Supplementary Methods 已补出大量默认值、归一化与 integration 框架；仍缺 charge-separation line 的完整定义、每批 acquisition-time table、blank 配对、邻居数、cluster resolution、UMAP/Monocle 细参、随机种子，以及差异代谢的鼠层模型和 Bonferroni 分母。
...
17. **绝对量锚定的展示口径有张力。** 主分析和 Supplementary Methods 采用 median-to-median 映射，Fig. 4 却把 LC–MS 的 6 次重复展示为 mean±误差；mean-to-mean 只作为敏感性分析且未给一致性系数。它不是一个可判定真假的数值冲突，但要求把结果写成尺度锚定的估计量。
...
- Supplementary Information 已补出 Tables 4–6、相关方法与图；但它不属于主文 pack 的句子 ID。Supplementary Data、Source Data、视频、原始数据与归档代码仍未摄入，关键配置语义、门控、blank 配对和鼠层统计仍需回查。
...
- **低置信解析不被静默修复：**主图图注与双栏正文混排，CCS 和组合评分公式存在断裂/错序，`P004.S0016`、`P005.S0018`、`P008.S0017` 等同一 ID 混合图注与正文。孤立 P 值已通过主图目视定位并明确标注证据类型；仍无法恢复的公式符号与组别顺序继续标为 `EXTRACTION_CHECK`。
- **复现边界：**此次联网取得官方 Supplementary Information，补齐了选择性累积、PRM-PASEF、60 周龄 N、实验流程和部分下游参数；同时保留碰撞能与 Table 6 阈值冲突。Supplementary Data 1–6、Source Data、视频、原始数据和代码归档仍未摄入，因此 frame gating、blank、配置语义、逐鼠 QC 与外部比较尚未独立复现。

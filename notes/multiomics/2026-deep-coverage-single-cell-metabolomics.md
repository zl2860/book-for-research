...
<!-- wechat-style-reviewed: 2026-09-15 -->

一次质谱流式类单细胞代谢组采集通常只有约 **0.1 秒**。在这段时间里，仪器既要捕捉阿摩尔（amol，10⁻¹⁸ mol）级的小分子，又要分清真实代谢物、随机噪声和同质量干扰；任何一个信号稍弱，都可能在结果里变成“这个细胞没有该代谢物”。

这不是少测几个峰的问题。既有单细胞代谢组技术通常只能检出数百种代谢物，低信噪比还会制造大量掉点，最后看到的“细胞异质性”可能同时混着生物差异与技术缺失。

真正的矛盾是：延长单细胞采集时间有助于鉴定，却会牺牲通量；维持高速采集，又难以兼顾灵敏度、覆盖度和注释可信度。作者因此追问，能否在不放弃分钟级几十个细胞通量的前提下，把单细胞代谢物测得更深、更稳，也更能说明“它究竟是什么”。

这篇论文给出的答案，是把流式进样、离子迁移—质谱、选择性离子累积和计算上的“细胞叠加”接成一条流程，再由 MetCell 把群体层面发现的可靠峰逐个提取回单细胞。平台达到每分钟 **25–40 个细胞**，在不同数据集中检出超过 5,000 个代谢峰，并把方法应用到 **45,603 个**小鼠原代肝细胞。

## 01｜为什么单细胞代谢组最容易把技术噪声误当成生物差异？

单个细胞里的常见代谢物多处于阿摩尔量级，接近质谱检测极限。信号弱时，同一种代谢物会在部分细胞中出现、在另一些细胞中消失；如果直接把“未检出”解释成“未表达”，掉点就会伪造亚群和代谢差异。
...
方法学验证从培养细胞开始。作者在 HepG2、HEK293T、Jurkat、HeLa、MEF 等细胞中比较离子累积、信噪比、掉点、注释覆盖与漂移；绝对量估计用 6 次 LC–MS 重复给出群体锚定量，再与 1,675 个 HEK293T 单细胞的强度分布对照。NAD⁺ 验证则把荧光传感器流式结果与同一干预方向下的单细胞质谱结果比较。

生物学应用使用 12 周龄和 90 周龄雄性小鼠各 6 只，共采集 45,603 个肝细胞；另从 10 只年轻小鼠分离四类肝细胞，用于筛选代谢标志物。作者还分析了来自 **2 只** 60 周龄雄性小鼠的 8,545 个肝细胞，并以组织染色和既有单细胞转录组数据作方向性验证。
...
平台先用约 25 μm 内径的毛细管把活细胞依次送入 timsTOF Pro。高速录像记录的 88 个事件中，83 个是单细胞事件（94.3%）；实际通量为每分钟 25–40 个细胞。细胞脉冲进入离子阱后，在几十毫秒内完成累积、离子迁移分离和 MS 检测，因此仍可保持 100% 离子占空比。

离子迁移不是再加一个装饰性维度。作者缩短 TIMS 累积时间并调整 ramping voltage，使低质量代谢物相对富集：m/z<400 Da 和 <200 Da 离子的相对比例分别提高约 **4 倍**和 **12 倍**，常见代谢物灵敏度最高提高 **20 倍**，而背景噪声没有同步增加。

Supplementary Fig. 5 还把“稳定”落到了数值上：采集过程中真空、毛细管电流和“每 5 分钟分段的单细胞 frame 总离子计数均值”的 RSD 分别为 **0.18%**、**1.61%** 和 **7.63%**；5 个代谢物的 m/z 漂移为 **0.5–1.2 ppm**，CCS 漂移为 **0.03%–0.15%**。这些是一次技术稳定性评估，不等于跨日、跨仪器或跨实验室重复性。
...
作者不是把 600 个细胞平均成一个“超级细胞”。MetCell 先把同一批次中 600 个单细胞的相同 m/z—离子迁移位置叠加，用群体信息把可重复的弱峰从随机噪声中找出来；随后再以这些峰的位置为靶标，回到每一个单细胞帧逐一提取强度。群体层只负责建立检测清单，最终定量仍保留在单细胞层。

以 acetylcarnitine 为例，叠加前单细胞信噪比为 3.3，600 个细胞叠加后为 157.6；100 个测试代谢离子的中位信噪比提高 **33 倍**。Supplementary Methods S6 说明 conventional comparator 是 20 个单细胞 SNR 的均值，并把 `SNR < 3` 定义为 dropout；Supplementary Fig. 7（S17）另限定比较只使用每个代谢物的 M0 同位素体。跨 100 个代谢物的平均掉点率由 **82% 降至 6%**；acetylcarnitine 由 83% 降至 1%，carnitine 由 13% 降至 0%。
...
MetCell 依次完成单细胞帧识别、细胞叠加、峰检测、逐细胞定量和多维注释。一次 120 分钟 HepG2 采集中，它识别 3,094 个单细胞帧、检出 5,718 个代谢峰；其中 1,356 个峰通过 MS1 与碰撞截面（CCS）匹配得到候选。主 Results/Fig. 3 称其中 1,350 个再从**另一批单细胞**定向采集 MS2，官方 Supplementary Table 1 却记为 1,352 个，最终注释数均写成 770。MS2 数量相差 2 尚未解释；无论采用哪一个，它都不是在前述每一个单细胞上同步完成的逐细胞结构确认。

按 Metabolomics Standards Initiative 分级，这 770 个注释包括 level 1、2、3、4 各 188、126、332、124 个。跨多类细胞共有 **389 个**独特代谢物以化学标准品验证为 level 1；其他常见细胞系每类检出 5,074–6,975 个峰、注释 533–622 个代谢物。Supplementary Tables 1 与 2 对 Jurkat、MEF 的 detected peaks 和 M0 level-1 数字出现互换，范围仍成立，逐细胞系对应却不能据此静默确定。比较对象是既有方法通常基于 MS1 给出的 80–300 个推定注释，而不是同一批样本上的统一头对头基准。
...
作者不是逐细胞另做一次 LC–MS，而是用群体 LC–MS 的量给单细胞质谱流式强度定标。主分析把 LC–MS 的每细胞中位量与 mass cytometry 的单细胞强度中位数对齐，再作线性换算；作者另用两平台均值对齐做敏感性分析，并称结果近似一致。Source Data 显示，报告的 adenine 18.4、nicotinamide 95.0、arginine 1.4 和 alanine 5.4 都是 6 次重复的中位数，而 `±1.4`、`±11.0`、`±0.2`、`±0.8` 是样本标准差；它不是通常意义上的 mean±SD。因此，下面的绝对量是依赖尺度锚定的**估计值**，不是每个细胞的独立绝对定量。

主图中，HEK293T 的 adenine LC–MS 锚定量为 18.4±1.4 amol/细胞；转换后，1,675 个单细胞估计落在 7.5–32.4 amol，检出限估计为 8.2 amol。Nicotinamide、arginine、alanine 的 LC–MS 锚定量分别为 95.0±11.0 amol、1.4±0.2 fmol、5.4±0.8 fmol，而单细胞估计范围分别为 23.6–196.0 amol、0.2–2.9 fmol、2.5–9.4 fmol。

第二个验证使用能报告 NAD⁺ 的 mCherry-FiNad-293T 细胞。FK866 处理后，荧光流式图中报告的 0.727 是处理组中位相对值；Source Data 的对照中位数为 0.992，因此两组中位数实际比值约为 0.733。离子迁移 mass cytometry 则把对照归一为 1，处理/对照中位数比为 0.856；传统处理因大量低信噪比掉点给出 1.0，未能识别下降。两个平台的效应大小并不完全相同，但方向和分布分离一致，这比只报告一个显著 P 值更有信息。

![Fig. 4：单细胞定量灵敏度与准确性](../../assets/multiomics/2026-deep-single-cell-metabolomics/fig4-quantification.png)

简明图注：Fig. 4 以 LC–MS 群体锚定量换算 adenine 与 nicotinamide 的单细胞范围，再以 FK866–NAD⁺ 干预对照荧光流式和两种质谱数据处理方式。

<a id="reader-07-liver"></a>

## 07｜代谢物本身能否给 45,603 个肝细胞“认身份”？

在 12 周龄与 90 周龄雄性小鼠各 6 只的混合肝细胞中，平台采集 45,603 个细胞，检出 5,659 个峰并注释 809 个代谢物。基于另 10 只年轻小鼠分离细胞得到的标志物，作者注释出 25,370 个肝细胞、2,898 个肝星状细胞（HSC）、3,333 个 Kupffer 细胞和 6,935 个肝窦内皮细胞；四类合计 38,536 个，仍有 7,067 个落在“其他”类别。
...
HSC 内部还分出 retinol-high 864 个与 retinol-low 2,034 个细胞。Fig. 5 图面显示，前者 retinol 和 retinal 分别为后者的 **2.47 倍**和 **2.39 倍**，glutathione 为 **0.84 倍**，约占全部 HSC 的 30%。这些是以细胞为单位的 Wilcoxon 比较，与更高氧化应激的**代理特征**一致，但不是直接的活性氧、损伤或代谢通量测量；极端 P 值及其统计层级见技术附录。
...
在 38,536 个已注释细胞中，年轻组 20,042 个、老年组 18,494 个。HSC 比例从年轻组的 6.8% 升到老年组的 9.4%：Source Data 说明分母是每只小鼠已注释的细胞，先逐鼠计算、再取每组 6 只的中位数（精确值 6.838% 与 9.434%），双侧 Wilcoxon 秩和检验——等价于 Mann–Whitney——`P = 0.0411`；不是把两组细胞直接汇总后的比例。这个动物层结果与既往免疫组化方向一致，但生物学重复仍只有每组 6 只。

作者把肝细胞按代谢谱排成四个 pseudotime 状态：把 25,370 个肝细胞汇总后，状态 1、2、3、4 中老年细胞分别占 **16%、64%、65%、86%**。这四个数是 `P(old | state)` 的 pooled-cell 比例，不是逐鼠比例均值，Source Data 也显示各鼠状态构成差异较大。状态 3 的老年肝细胞富集 arginine、hypoxanthine、spermine 和多种 diacylglycerol；状态 4 则富集 adenosine、nicotinamide、glutathione 与多种寡糖。

Oil Red O 与 PAS 染色把两类状态分别联系到中央静脉附近脂滴积累和门静脉周围糖原储存；来自 **2 只** 60 周龄雄性小鼠的 8,545 个肝细胞也出现相似亚群。这是“空间染色与小规模中间年龄数据方向一致”的支持，不是同一细胞被纵向追踪出的两条命运；两端年龄和 pseudotime 仍不足以证明真实时间轨迹。
...
对研究设计更直接的价值，是代谢物开始能够承担类似单细胞转录组 marker 的角色，同时提供更接近功能状态的读出。HSC 的 retinol 与氧化应激代理特征、老年肝细胞的脂质—糖原分化说明，这套平台可以从“有什么细胞”继续追问“同类细胞正处于什么代谢状态”。

但目前价值仍主要落在研究工具。论文没有临床样本上的前瞻验证，也没有证明组织解离后的代谢谱可稳定代表患者体内状态；不能把肝脏小鼠实验直接外推成肿瘤临床检测能力。

## 10｜对肿瘤多组学研究，最值得迁移的是什么？
...
## 11｜这些结果仍需要冷静看待：哪些边界不能越过？

第一，**600 细胞叠加建立了一个检测先验**。它能救回跨细胞重复出现的弱峰，却可能漏掉只存在于极少数细胞的代谢物；默认取数据集前 600 个细胞，还要求进样顺序、批次和稀有亚群不造成系统偏差。

第二，**注释覆盖不等于每细胞完整覆盖**。摘要中的“约 800 个代谢物/细胞”与 Results 中“肝细胞数据集共注释 809 个代谢物”口径不同；掉点虽下降，但不能据此假定每个单细胞都测到全部 809 个代谢物。

第三，**结构鉴定仍受物理分辨率和谱库限制**。当前离子迁移分辨率约 40，maltotetraose 与 maltotriose 等异构体未必能分开；约 50% 已采集 MS2 仍未注释，level 1 与 level 2 中分别有 26% 和 21% 的谱图 dot-product 只在 0.6–0.8。

第四，**平台目前只有正离子模式**。活细胞 electrolaunching 可能因膜破坏不完全而漏检，蛋白/DNA 共电离会造成离子抑制和复杂多电荷谱，维持细胞完整性的等渗 ammonium formate 又可能降低非极性代谢物溶解度。

第五，**生物学验证仍是横断面小鼠设计，而且细胞数不能替代动物数**。主要年龄比较只有 12 周与 90 周两个端点、每组 6 只雄鼠；60 周补充数据虽有 8,545 个肝细胞，却只来自 2 只雄鼠。state 3/4 的差异代谢检验以 1,832 对 6,989 个细胞呈现，Source Data 证明 P 值来自细胞层 Wilcoxon 后乘以 4,541 个峰作 Bonferroni 校正，但没有把小鼠作为分层或随机效应，仍存在伪重复风险。pseudotime 不能当作真实纵向轨迹，结果也不能外推到雌鼠。

第六，**组织解离会改变代谢物**。作者自己指出，临床组织消化可能引入代谢伪影；稀有细胞也可能在混合样本中被忽略。研究中的培养细胞和即时处理小鼠肝脏，不能替代复杂患者标本的系统稳定性评估。

第七，**主文句子 ID 与补充材料必须分层**。本地 29 页主文 PDF 不含补充文件；本次复审已核验官方 44 页 Supplementary Information，并逐工作表审计 11 个 Source Data XLSX，补出稳定性、统计分母和逐鼠口径。它们不冒充主文的 `Pxxx.Sxxxx` 来源 ID。Supplementary Data 2/4 已定点核查版本差异，但 Data 1–6 尚未全部逐值审计；视频、原始 MS 数据和归档代码也未摄入，碰撞能、阈值语义、frame gating、blank 配对与鼠层差异模型仍未闭合。
...
下表收纳读者正文 24 类事实的来源标记；本次复审新增的补充页码与主图目视证据也在相应行单列。完整 Results、Methods、图注、解析质量和证据边界仍由后续审计展开。

| 正文位置 | 所支持的事实 | 原来源标记 |
|---|---|---|
| 开场第 1 段 | 0.1 秒采集、阿摩尔量级与弱信号掉点 | `P002.S0002–P002.S0008` |
| 开场第 2 段 | 既有覆盖度与技术缺失问题 | `P002.S0003–P002.S0009` |
| 开场核心答案 | 平台流程、25–40 个细胞/分钟、>5,000 个峰与 45,603 个肝细胞 | `P002.S0010–P002.S0022`、`P002.S0023–P003.S0019`、`P005.S0051–P005.S0060` |
| 01 | 阿摩尔量级、检测极限与 dropout 风险 | `P002.S0002–P002.S0008`、`P002.S0051–P002.S0057` |
| 02（方法验证） | 培养细胞、6 次 LC–MS、1,675 个单细胞与 NAD⁺ 验证 | `P003.S0024–P004.S0021`、`P004.S0036–P005.S0050` |
| 02（生物学应用） | 年龄队列、45,603 个细胞、10 只标志物队列与 2 只 60 周龄雄鼠的 8,545 个肝细胞 | `P005.S0051–P006.S0065`、`P007.S0017–P007.S0022`、`P008.S0042–P009.S0001`；官方 Supplementary Information 内页 S5 |
| 03（流路与通量） | 25 μm 毛细管、83/88 单细胞事件、通量与占空比 | `P002.S0023–P002.S0033`、`P012.S0016–P012.S0034` |
| 03（选择性累积） | 低质量离子相对比例与最高 20 倍灵敏度 | `P002.S0034–P002.S0050` |
| 04（算法） | 600 细胞峰发现与逐细胞回提 | `P002.S0051–P002.S0059`、`P012.S0044–P013.S0040` |
| 04（性能） | 信噪比、33 倍提升与 dropout 比较 | `P002.S0060–P003.S0019` |
| 05（流程） | 3,094 个帧、5,718 个峰、从另一批单细胞采集 MS2 与 770 个注释 | `P003.S0020–P003.S0043`；官方 Supplementary Information 内页 S5 |
| 05（注释等级） | 四级注释、389 个 level 1 代谢物与跨方法比较 | `P003.S0039–P004.S0020` |
| 06（绝对定量） | LC–MS 群体锚定量、1,675 个单细胞估计范围、检出限及 median-to-median 主分析与 mean-to-mean 敏感性分析 | `P004.S0036–P005.S0028`；官方 Supplementary Information 内页 S8–S9、Supplementary Fig. 10（S20） |
| 06（NAD⁺ 验证） | FK866 后荧光流式、离子迁移与传统处理比较 | `P005.S0029–P005.S0050` |
| 07（细胞注释） | 45,603 个细胞、四类已注释细胞与其他类别 | `P005.S0051–P006.S0020`、Fig. 5 |
| 07（HSC 亚群） | 864/2,034 个 HSC、retinol/retinal/glutathione 的倍数与图面 P 值 | `P006.S0021–P006.S0053`；Fig. 5j–l 页面视觉复核 |
| 08（年龄组成） | 20,042/18,494 个细胞与 HSC 6.8%→9.4% | `P006.S0054–P006.S0060`、Fig. 6b,c |
| 08（计算状态） | 四个 pseudotime 状态及老年细胞比例 | `P006.S0061–P008.S0024` |
| 08（正交验证） | Oil Red O、PAS 与 2 只 60 周龄雄鼠的 8,545 个肝细胞 | `P008.S0025–P009.S0001`；官方 Supplementary Information 内页 S5 |
| 11（检测先验） | 前 600 个细胞、稀有峰与批次边界 | `P013.S0001–P013.S0038`、`P018.S0002–P018.S0010` |
| 11（结构鉴定） | 分辨率、未注释 MS2 与谱图相似度边界 | `P009.S0035–P009.S0048` |
| 11（离子模式） | 正离子模式、膜破坏、离子抑制与溶解度限制 | `P009.S0010–P009.S0018`、`P009.S0042–P009.S0047` |
| 11（动物设计） | 两端年龄每组 6 只雄鼠、中间年龄 2 只雄鼠与细胞嵌套边界 | `P005.S0051–P005.S0055`、`P008.S0045–P009.S0001`；Reporting Summary 第 2–3 页；官方 Supplementary Information 内页 S5–S7 |
| 11（组织解离） | 临床组织消化伪影与稀有细胞遗漏 | `P009.S0054–P010.S0005` |

09 的价值判断综合 03–08 的结果链，10 的迁移建议综合主 Methods `P012.S0002–P014.S0028` 与 Discussion `P009.S0002–P010.S0005`；二者没有被伪装成新的原文结果。
...
- 原始数据：National Omics Data Encyclopedia `OEP00005636`；Supplementary Data 的 Zenodo concept DOI 为 10.5281/zenodo.15189109，2026-09-15 可见的当前版本记录为 10.5281/zenodo.16874744；Supplementary Video 为 10.5281/zenodo.15119433；复用的单细胞转录组为 GEO `GSE171904`。旧版本记录 15189110 缺 Data 6，Data 2/4 后续又增加 6 个证据字段，复现时必须锁定版本。（论文来源：`P014.S0030–P014.S0037`；Reporting Summary 第 1–2 页；版本差异来自 Zenodo 记录）
...
- 本地 PDF SHA-256：`ce0657b0916173be8dfe26b9bfaa91ed175f63abe6ccfe7dccf224c22bc4bd36`。

### PDF 解析质量

- 本次运行以 `scripts/build_pdf_llm_pack.py --engine pymupdf` 临时重建全文依据；解析包未作为版本库文件持久化。PDF 共 29 页、1,038 个句子 ID；自动分类为 title 30、other 22、results 36、supplementary 204、methods 506、discussion 28、references 212。
...
- Reporting Summary 的软件版本包括 R 4.2.1、Seurat 5.0.1、Monocle 2.26.0、FlowJo 10.10.0、MetCell 1.0.22 等；主文 Methods 主要报告仪器与 MetCell 算法。官方 Supplementary Information 已补出若干函数、参数和组织处理流程，但随机种子、部分聚类参数与鼠层统计模型仍不完整。
- 本地主论文 PDF 不含 Supplementary Information、Supplementary Data 1–6、Supplementary Video 或独立 Source Data。此次复审另外核对了官方 Supplementary Information，并逐工作表审计主图 1–6 与扩展图 2/4/7/8/9 的 11 个 Source Data XLSX；Supplementary Data 2/4 只做了版本差异定点核查，六份 Supplementary Data 尚未全部逐值审计，视频、原始数据和代码归档也未审计。不能把 Source Data 审计写成全部开放材料均已复现。

### 官方 Supplementary Information 定点复审（不计入主文 ID）

- 来源为 Nature 文章页链接的[官方 Supplementary Information](https://media.springernature.com/original/springer-static/esm/art%3A10.1038%2Fs41592-025-02970-2/MediaObjects/41592_2025_2970_MOESM1_ESM.pdf)，复审文件 SHA-256 为 `82f2827d40bd29d3125e8e4b1dd72fa622f30363ce97ccc279a9dc55fafe6e9a`。该文件共 44 个 PDF 页面，正文页码为 S1–S43；它独立于上述 29 页主论文和 1,038 个 `Pxxx.Sxxxx` ID，以下只用补充页码/表号定位，不伪造主文句子 ID。
- Supplementary Methods（S4–S5）确认 60 周龄队列为 **2 只雄鼠**、12 周和 90 周主队列各 6 只，并补出 140 mM ammonium formate（pH 7.3）及最终 `4×10^5 cells ml^-1` 的上机细胞密度。细胞在 37 °C/5% CO₂ 培养；贴壁细胞用 DMEM、Jurkat 用 RPMI，均含 10% FBS 和 1% glutamine/pyruvate/penicillin–streptomycin，48–72 h 换液、汇合度低于 80%、胰酶 2–4 min、100g 3 min/4 °C 收集。肝脏用 EGTA 5 ml/min、37 °C 灌流 5 min，再 collagenase 6 min、DNase 37 °C 10 min、70 μm 过滤和 580g 10 min/4 °C 离心；每鼠分 2–4 批、每个肝样本至少采 2,000 个细胞。
- PRM-PASEF（S5–S6）写明前体隔离窗 `m/z ±0.6 Da`、淌度窗 `±0.03 V s cm^-2`，low/high 质量窗为 50–300/300–1,300 Da，每次 scan 带 marker、每 scan 采 2–6 个离子；每个离子采 1 min、约 10–50 张 MS2 谱，共识谱中位数为 31 张。共识谱按 20 ppm 分组、强度求和并以最高峰归一；120 min HepG2 数据在 7 线程 i7-12700 上约运行 40 min、占 6–20 GB。但 Supplementary Methods S5 写碰撞能 30 eV，Table 5（S41）却把 low/high 两档都列为 10 eV；原文未解释，本文不替作者二选一。
- MetCell/下游流程（S6–S9）补出 MetCell 1.0.22、SCMeTA 0.2.7、Seurat 5.0.1 与 Monocle 2.26.0；肝脏数据以总离子强度做 LogNormalize、取 2,000 个高变特征和 25 个主成分，再做 CCA integration、邻接图、聚类及 UMAP/t-SNE。软件、函数和大框架由此可定位，但 cluster resolution、邻居数、随机种子和鼠层差异模型仍未报告。
- 跨批峰匹配（S7）把 31 个批次的单电荷峰都对齐到随机选取的 old mouse #3，形成 `4,541 singly-charged peaks × 45,603 cells` 的矩阵。这个参考样本选择可能影响跨批特征保留，补充材料没有给更换参考样本的敏感性分析。
- 分类时，DotPlot 以 cluster average marker expression >0 作为归入相应细胞类型的门槛；FindMarkers 使用双侧 Wilcoxon，并按 4,541 个峰作 Bonferroni。这个阈值没有独立测试集、held-out accuracy 或混淆矩阵支持。
- 标志物筛选（S8）实际使用 hepatocyte 2,386、LSEC 1,990、Kupffer cell 2,309、HSC 1,449 个细胞，先取各类 FC 前 10 名再人工复核，并要求 marker 为 level 1。人工筛选提高化学身份门槛，却没有提供独立测试集的分类性能。
- Supplementary Fig. 5（S15）报告真空、毛细管电流和 5 min 分段单细胞 frame 总离子计数均值的 RSD 为 0.18%、1.61%、7.63%。Phosphorylcholine、carnitine、spermine、lysoPC(16:0)、PC(34:1) 的 m/z 漂移依次为 1.1/1.2/0.5/0.8/0.7 ppm，CCS 漂移依次为 0.07%/0.03%/0.04%/0.15%/0.14%；这是单次采集稳定性，不是跨日重复性。
- Supplementary Methods S6 与 Supplementary Fig. 7（S17）说明 33× SNR 比较只用每个代谢物的 M0 同位素体：conventional SNR 是 20 个单细胞的均值，dropout 定义为 `SNR < 3`。Table 1 又写采得 1,352 条 MS2，而主 Results/Fig. 3 写 1,350；Tables 1/2 还把 Jurkat 与 MEF 的 detected peaks、M0 level-1 数字互换，相关分母不能静默择一。

Supplementary Table 1（S37）的列依次为 peaks/candidates/MS2/L1/L2/L3/L4/注释合计；Table 2（S38）的列依次为 detected/singly charged/M0 all-level/含同位素关联峰/M0 L1/含同位素 L1。逐行值如下：

| 数据集 | Table 1 | Table 2 |
|---|---|---|
| HepG2 | 5,718/1,356/1,352/188/126/332/124/770 | 5,718/4,154/770/1,136 (27.3%)/188/328 (7.9%) |
| HeLa | 5,074/971/965/186/110/199/104/599 | 5,074/3,120/599/726 (23.3%)/186/251 (8.0%) |
| HEK293T | 6,297/1,181/1,177/162/126/244/90/622 | 6,297/4,092/622/926 (22.7%)/162/290 (7.1%) |
| Jurkat | 6,331/973/956/135/81/192/125/533 | **6,975**/4,609/533/657 (14.3%)/ **176**/196 (4.3%) |
| MEF | 6,975/1,114/1,085/176/102/240/81/599 | **6,331**/4,159/599/746 (18.0%)/ **135**/245 (5.9%) |
| mixed liver | 5,659/1,576/1,572/215/182/290/122/809 | 5,659/4,541/809/966 (21.3%)/215/285 (6.3%) |

Table 2 的 Jurkat/MEF detected 与 M0 L1 正好反向对应 Table 1，singly charged 也随交换后的 detected 行变化；附件没有勘误或映射说明。
- Supplementary Table 4（S40）补出选择性累积的核心差异：TIMS ramp 从 `0.55–1.90` 改为 `0.45–1.45 V s cm^-2`，ramp/accumulation time 从 `100/100 ms` 改为 `50/50 ms`，low mass 从 150 Da 改为 60 Da。Supplementary Table 6（S42）给 HepG2 使用的可调 marker intensity 参数为 `intensity_abs_threshold_upper=60,000`、`intensity_abs_threshold_lower=600,000`；按标签看上下阈值似乎倒置，未经代码/配置文件核验前不能静默交换。
- LC–MS 与流式（S8–S9）补出 5.43×10^6 cells/plate、提取液 ACN/MeOH/water `2:2:1`、100 μl ACN/water `1:1` 重溶、BEH amide 色谱、Vanquish UHPLC–Orbitrap Exploris 480、Skyline 22.2，以及 FK866 `10 nM, 24 h` 和 GFP/mCherry 比值等条件。Source Data 还补齐四项标准曲线浓度点；回归权重、完整诊断、门控、电压与逐鼠 QC 仍不足。
- Supplementary Fig. 13（S23）的 Oil Red O/PAS 图像每个年龄组展示 3 只小鼠，未见图像层面的定量。Supplementary Fig. 14（S24）在 2 只 60 周龄雄鼠的 8,545 个细胞中给出 2,856/2,931/2,758 三簇，并以细胞层检验报告 DG 与寡糖方向一致；它是小规模方向性补强，不能写成独立的大样本动物验证。

### 官方 Source Data 逐表审计（不计入主文 ID）

- Nature 文章页提供 11 个 XLSX：MOESM3–8 对应主图 1–6，MOESM9–13 对应 Extended Data Fig. 2、4、7、8、9，共 44 个工作表。本次逐 sheet 读取；临时下载没有提交到仓库，持久定位使用“workbook＋sheet＋cell range”。

| 文件 | 图 | SHA-256 |
|---|---|---|
| MOESM3 | Fig. 1 | `67b789d8e5449484470e71814fa70b82c7fcbf333b0dc53d67995d0aef081a06` |
| MOESM4 | Fig. 2 | `02b8970cc1170a349bc002785b416e65e647872435ab7c79fff2c30709727678` |
| MOESM5 | Fig. 3 | `b0018cd3bde17d942d4637cc1c3e351919cd4df0b46a376e0654bc0ed57ad712` |
| MOESM6 | Fig. 4 | `080bbd64208637d2dbf925618228bf7f6222147ae3f4613401b8f4e464eebebd` |
| MOESM7 | Fig. 5 | `16b916543f6362e5b0d7b00b6693429160102132ee740aad85209affa83db6ae` |
| MOESM8 | Fig. 6 | `ed72098bf25db3719716e0d4234ff6af6f2c3a128f4e9628079852675b4f5138` |
| MOESM9 | Extended Data Fig. 2 | `6e8343bce44b0c41cd09801bd52cf44641c468638e56b76e9c8d3db6f46b408e` |
| MOESM10 | Extended Data Fig. 4 | `18ed738652787239ed9137d88a847c80963379a74b54b61206978bfc8051e7b0` |
| MOESM11 | Extended Data Fig. 7 | `4ce577776b21a83113face4291819a119526e875662e9d591b1b668cfe7148ae` |
| MOESM12 | Extended Data Fig. 8 | `73c52a4ba4de6580dc2764bb3743f151bc8fe46fe01f28d68ee26df131382861` |
| MOESM13 | Extended Data Fig. 9 | `a3a3a834fdd0d68d23f03791c9fb8655e5e7b492d896da5a3d16357dc2969952` |
- MOESM4 `figure2f/figure2i` 闭合 100 个离子的 SNR 与 dropout 底数；MOESM6 `figure4b/c` 证明 adenine、nicotinamide 锚点是 6 次重复中位数＋sample SD，MOESM12 `extended_fig_8e/8f` 对 arginine、alanine 给出同一口径。MOESM6 `figure4e/f` 另区分 flow 的 FK866 中位值 0.727 与实际中位数比 0.733、IM 的处理/对照比 0.856。
- MOESM7 `figure5j-l` 与 MOESM8 `figure6f/figure6g-h` 证明 Fig. 5j–l 和 Fig. 6g–h 的图示 P 值均为细胞层双侧 Wilcoxon 原始 P 乘以 **4,541** 后的 Bonferroni 值。这个分母现已闭合，细胞嵌套于小鼠的伪重复风险并未因此消失。
- MOESM8 `figure6b-c` 显示 HSC 6.838%→9.434% 是逐鼠已注释细胞比例的组中位数，双侧 Wilcoxon 秩和 `P=0.0411255`；若 pooled 为 6.36%/8.78%，组均值则为 6.494%/8.998%，三种摘要不可互换。
- MOESM8 `figure6d/e` 显示 16%/64%/65%/86% 是 pooled `P(old | state)`。逐鼠构成高度异质：老鼠 state 3 占各鼠 hepatocytes 的 1.35%–36.86%，state 4 为 14.71%–73.70%，年轻 4 号的 state 4 也达到 38.75%；它不是统一的个体级年龄轨迹。
- MOESM12 `extended_fig_8a/c/e/f` 补齐标准曲线：adenine 0.025–0.8 ppm、nicotinamide 0.05–1.6 ppm、arginine 2.5–20 ppm、alanine 5–40 ppm。它允许核对浓度点，仍不足以恢复未报告的回归权重和完整残差诊断。
...
- d：1,356 个候选中，主 Results/Fig. 3 写 1,350 个从另一批单细胞定向取得 MS2，Table 1 写 1,352 acquired，最终均报告 770 个 level 1–4 注释；差 2 条未解释。这不是在前述每个单细胞上同步获得的逐细胞 MS2。
...
- b–c：LC–MS 展示 6 次重复，主换算把其群体中位数锚定到 mass cytometry 单细胞分布中位数；Source Data 证明中心值为 median、误差条为 sample SD，另有 mean-to-mean 敏感性分析。b 为 adenine，c 为 nicotinamide。
- d：mCherry-FiNad-293T 细胞的 NAD⁺ 由荧光流式与 IM-resolved mass cytometry 交叉验证。
- e–f：对照与 FK866 组的 NAD⁺ 相对丰度；荧光流式为 control `N=10,594`、FK866 `N=10,770`，图示 FK866 中位值 0.727、Source Data 中位数比 0.733；IM-resolved mass cytometry 为 control `N=1,610`、FK866 `N=1,544`，中位数比 0.856。两者均 `P<2.2×10^-16`。图中 N 是单细胞事件数，P 值用双侧 Wilcoxon；箱线图为中位数、Q1–Q3 与 1.5×IQR 须线。
...
- j–l：两个 HSC 亚群的 retinol（FC 2.47，`P=3.1×10^-237`）、retinal（FC 2.39，`P=3.0×10^-228`）与 glutathione（FC 0.84，`P=2.6×10^-2`）；双侧 Wilcoxon 检验，箱线图为中位数、Q1–Q3 与 1.5×IQR 须线。数值由主图图面复核；a 由 BioRender.com 绘制。

#### Fig. 6｜Distinct metabolic states of hepatocytes during aging（完整图注来源：`P008.S0014–P008.S0017`、`P008.S0038–P008.S0041`；混合 ID 已复核）

- a：38,536 个已注释肝脏细胞的 UMAP，分别按细胞类型与年龄着色；其中 hepatocytes 为 25,370 个，其余为 HSC、Kupffer cell 与 LSEC。
- b：年轻与老年小鼠各类细胞比例（每组 N=6），误差条为 s.e.m.。
- c：以每鼠已注释细胞为分母，HSC 比例的组中位数由 6.838% 升至 9.434%；双侧 Wilcoxon 秩和 `P=0.0411255`，每组 N=6 只鼠。它不是 pooled-cell 比例。
...
- g：两个状态的 DG(38:4)、DG(36:2)、DG(36:4)；主图图面依次列出 FC 0.70、0.70、0.74，`P=5.2×10^-13`、`3.9×10^-35`、`1.6×10^-12`。
- h：两个状态的 maltopentaose、maltotetraose、maltotriose；主图图面依次列出 FC 1.52、1.25、1.11，`P=6.6×10^-117`、`2.9×10^-75`、`4.5×10^-19`。state 3 和 4 分别含 1,832 与 6,989 个老年肝细胞，箱线图为中位数、Q1–Q3 与 1.5×IQR 须线。Source Data 证明 g/h 六个 P 值均为细胞层双侧 Wilcoxon 原始 P 乘以 4,541 后的 Bonferroni 值；图注仍未定义 FC 的分子/分母，本文保留图面数值而不据此另推方向。
...
| `P002.S0048–P002.S0049` | IM 分辨质谱流式可稳定测量 m/z 和 CCS，同时保持信号强度一致。 | 正文没有 CV/漂移数值；Supplementary Fig. 5 补出单次采集的真空/电流/总离子计数 RSD 与 5 个代谢物的 m/z、CCS 漂移，不能外推为跨日重复性。 |
...
| `P003.S0031` | 1,350 条 MS2 谱最终“识别出 770……”；句尾混入 “Ion mobility separation”。 | `EXTRACTION_CHECK`：结果数量需与 `P003.S0039–S0040` 拼接理解；Supplementary Table 1 同一 HepG2 行写 1,352，原文存在 2 条谱的冲突，不能静默择一。 |
...
| `P003.S0052–P003.S0053` | 单细胞代谢组采集期间未观察到明显信号漂移。 | “明显”无正文阈值；Supplementary Fig. 5 给出单次采集 RSD 0.18%/1.61%/7.63%、m/z 漂移 0.5–1.2 ppm、CCS 漂移 0.03%–0.15%，仍没有跨日或跨仪器评估。 |
...
| `P004.S0039–P004.S0041` | LC–MS 报告 HEK293T 每细胞腺嘌呤为 18.4±1.4 amol；作者预期它对应单细胞质谱流式分布的中位数。 | Source Data 显示 18.4 是 6 次重复的中位数、1.4 是样本标准差；这是群体锚点，“对应中位数”是定标假设，不是逐细胞真值。 |
...
| `P005.S0018–P005.S0020` | ……原文报告三者每细胞量分别为 95.0±11.0 amol、1.4±0.2 fmol、5.4±0.8 fmol；本 ID 前部混入 Fig. 3 图注。 | Source Data 显示中心值均为 6 次重复中位数、误差为样本标准差；不是常规 mean±SD。`EXTRACTION_CHECK`。 |
...
| `P005.S0035–P005.S0036` | FK866 组中，两种方法都检测到 NAD+ 下降；正文分别报告 0.727（荧光）和 0.856（IM 质谱流式）。 | Source Data 显示 0.727 是荧光 FK866 组归一化中位值；因对照中位数为 0.992，实际中位数比为 0.733。方向一致但效应大小不同。 |
...
| `P005.S0050` | 常规方法给出的处理/对照 NAD+ 比值为 1，明显偏离荧光流式图示 0.727（Source Data 实际中位数比 0.733）。 | 同数据算法对照；证明常规检峰在此低 SNR 场景失效，不等于 IM 平台在所有分析物上无偏。 |
...
| `P006.S0057–P006.S0058` | HSC 比例从年轻组 6.8% 升至老年组 9.4%。 | Source Data 显示它是每只小鼠已注释细胞中 HSC 比例的组中位数（6.838%/9.434%），`N=6/组`、双侧 Wilcoxon 秩和 `P=0.0411255`；不是 pooled-cell 比例。 |
...
| `P006.S0063–P006.S0064` | 随伪时序推进，老年来源细胞比例从 state 1 的 16% 升到 state 3 的 65% 和 state 4 的 86%。 | 正文句子未列 state 2；主图 Fig. 6e 目视补出 state 2 为 64%。细胞仍嵌套于 12 只动物。 |
...
- 120 min、3,094 单细胞帧、5,718 peaks；1,356 个以 MS1+CCS 初注释；主 Results/Fig. 3 写其中 1,350 个从另一批单细胞取得定向 MS2，Supplementary Table 1 则写 1,352，二者最终都指向 770 个注释：level 1/2/3/4 = 188/126/332/124（`P003.S0024–P003.S0040`；批次关系见 Supplementary Methods S5）。
...
- 图注 ID：`P006.S0006–P006.S0008`, `P006.S0042–P006.S0045`。
- 图元提取显示 LC–MS `N=6 replicates`，单细胞质谱流式腺嘌呤面板 `N=1,675 single cells`（`P006.S0002–P006.S0003`）。这些 N 来自图内文字而非正文叙事。
- 腺嘌呤 18.4±1.4 amol/cell，单细胞映射范围 7.5–32.4 amol，LOD 8.2 amol/cell；烟酰胺/精氨酸/丙氨酸的锚点、范围和 LOD 见 §3.4。四个 LC–MS 中心值都是 6 次重复中位数，`±` 后为样本标准差；主换算把群体中位量锚到单细胞强度中位数，mean-to-mean 仅作为敏感性分析（`P004.S0039–P005.S0024`；Supplementary Methods S8–S9；MOESM6/MOESM12）。
- NAD+ 面板的文本包保留荧光两组 `N=10,594`、`N=10,770`，质谱流式两组 `N=1,610`、`N=1,544`（`P006.S0040–P006.S0043`），但二维顺序丢失；主图目视复核恢复为荧光 control/FK866 `10,594/10,770`、质谱流式 control/FK866 `1,610/1,544`。图示荧光 FK866 中位值为 0.727，Source Data 的实际中位数比为 0.733；IM 比为 0.856。
- Fig. 4e/f 使用双侧 Wilcoxon，两者均 `P<2.2×10^-16`；精确阈值来自主图目视复核，文本图注只保留检验名称（`P006.S0044`）。box plot 定义为中位数、Q1/Q3、1.5×IQR whisker（`P006.S0045`）。

##### Fig. 5｜肝细胞类型与 HSC 亚型

- 图注 ID：`P007.S0016–P007.S0022`, `P007.S0046–P007.S0052`。
...
- Fig. 5j–l 使用双侧 Wilcoxon（`P007.S0050`），box plot 定义同上（`P007.S0051`）。文本包把图面统计量拆散，单靠 `P007.S0039` 不能恢复映射；此次回看主图后可定位为 retinol FC 2.47、`P=3.1×10^-237`，retinal FC 2.39、`P=3.0×10^-228`，glutathione FC 0.84、`P=2.6×10^-2`。这些映射的证据类型是“主图目视复核”，不是补造句子 ID。

##### Fig. 6｜衰老肝细胞状态

- 图注 ID：`P008.S0014–P008.S0017`, `P008.S0038–P008.S0041`；`P008.S0017` 为图注/正文混合 ID。
- 38,536 个注释细胞；年轻 20,042、老年 18,494。HSC 6.8%→9.4% 是各鼠已注释细胞比例的组中位数（精确值 6.838%→9.434%），动物 `N=6/组`，双侧 Wilcoxon 秩和 `P=0.0411255`；pooled-cell 比例另为 6.36%→8.78%（`P006.S0055–P006.S0059`; `P008.S0015–P008.S0016`; MOESM8 `figure6b-c`）。
- 主图 Fig. 6e 目视复核显示 state 1/2/3/4 总细胞数为 12,585/1,851/2,840/8,094，其中老年细胞为 1,975/1,185/1,832/6,989，占 16%/64%/65%/86%；正文句子只明列 state 1/3/4（`P006.S0061–P006.S0065`）。
- state 3 vs 4 差异代谢物使用细胞层双侧 Wilcoxon；Source Data 证明 Fig. 6g/h 六个图示 P 值都是原始 P 乘以 4,541 后的 Bonferroni 值（`P008.S0038–P008.S0040`）。主图目视复核显示 DG(38:4)/DG(36:2)/DG(36:4) 的 FC 为 0.70/0.70/0.74、P 为 `5.2×10^-13`/`3.9×10^-35`/`1.6×10^-12`，三种寡糖的 FC 为 1.52/1.25/1.11、P 为 `6.6×10^-117`/`2.9×10^-75`/`4.5×10^-19`。图注未明示 FC 分子方向，不据数值另推方向。
- 60 周龄复现含 8,545 hepatocytes；动物数未出现在主文文本包（`P008.S0045–P008.S0049`），官方 Supplementary Methods S5 补出为 2 只雄鼠。
...
| 单细胞量是否可信？ | 群体 LC–MS 锚点 vs 单细胞强度分布；FK866 下荧光 vs IM 平台 | 线性换算的绝对量范围、LOD、NAD+ 相对比 | amol–fmol 量级可估计，药理扰动方向与独立平台一致 | 主分析为群体量对单细胞中位数的尺度锚定，mean-to-mean 是敏感性分析；不是同一细胞配对；荧光实际比 0.733 vs IM 0.856，非完全一致 |
...
| 年龄是否驱动两种肝细胞状态？ | 12 vs 90 周各 6 鼠；伪时序；组织染色；60 周龄 2 鼠补充队列 | 四状态、两老年分支、DG/寡糖差异、空间染色 | 年龄与两种代谢状态及肝区带图样相关 | 横断面、细胞嵌套于动物；未见混合模型/动物级差异检验；伪时序非真实时间；60 周龄 N=2 只作小规模方向性支持 |

统计上最需要冷静看待的是“细胞数很大但动物数不大”。Fig. 6 的 HSC 比例比较明确先逐鼠计算，再以 `N=6 mice/组` 做双侧 Wilcoxon，这是合适的动物级比较；但 state 3/4 的差异代谢物直接以 1,832/6,989 个细胞做 Wilcoxon，再乘 4,541 作 Bonferroni。Source Data 没有显示把小鼠作为分层或随机效应，因此仍可能把同一小鼠内细胞误作独立重复。技术性能部分的 600 cells 和 100 metabolites 同样不是生物学重复数。
...
- 坐标/图元/页眉页脚噪声：137/137，完整集合见 §2.2；单靠文本包不能无歧义排列的图面数值保留 `EXTRACTION_CHECK`，本次能由主图恢复者另标“目视复核”，不伪造句子 ID。
...
4. **文本包无法映射、但主图可目视定位的统计文字**：`P007.S0039` 的 `P=2.6×10^-2` 对应 Fig. 5l glutathione，`P008.S0037` 的 `P=6.6×10^-117` 对应 Fig. 6h maltopentaose；这种映射来自回看主图，不能冒充句子级解析。Fig. 4 的 control/FK866 组别、N、ratio 与 P 阈值也由主图目视恢复，文本包本身仍不足以安全排列。
5. **仍未提供的信息**：多个技术对照的独立批次/生物学重复、部分“无漂移/无背景增加”的量化判据、细胞类型分类性能，以及伪时序差异模型是否控制鼠内相关性；60 周龄动物数已由 Supplementary Methods 补为 N=2。
...
- 证据文件：本次以脚本临时重建的全文 pack 与 manifest（均未持久化）；为读取 Reporting Summary，又逐页查看原 PDF 第 26–29 页渲染图。
...
| `P012.S0035` | 选择性离子累积与默认离子累积的参数见补充表 4。 | 参数不在主 Methods 文本；此次已由官方 Supplementary Table 4（S40）补出 ramp、accumulation time、low mass 与电压设置，另层审计。 |
| `P012.S0036` | HepG2 的 IM-off 采集把仪器从 TIMS-MS 切换为仅 MS 模式以关闭淌度分离，其余选择性离子累积参数保持不变。 | 定义有/无 IM 的对照；“保持不变”的设定现可与 Supplementary Table 4（S40）对照，但仍缺运行级配置文件。 |
| `P012.S0037` | PRM-PASEF 靶向 MS2 采集参数见补充表 5。 | 官方 Supplementary Methods S5 与 Table 5（S41）已补出窗口/质量范围，却分别报告 30 eV 与 10 eV 碰撞能；参数不是继续缺失，而是出现未解释冲突。 |
...
| `P012.S0066` | PC(34:1) 的 EIC 峰强度还会按用户定义的强度标准过滤。 | Supplementary Table 6（S42）给出 HepG2 使用的可调值，但 `intensity_abs_threshold_upper=60,000`、`intensity_abs_threshold_lower=600,000` 按标签疑似倒置；其他数据集配置仍未知。 |
...
| `P012.S0075–P012.S0076` | 作者同步采集毛细管内细胞流动的高速视频与 MS 数据，进一步核实所见峰的细胞来源（补图 19）。 | Supplementary Fig. 19 已随官方补充 PDF 取得；独立视频文件仍未摄入，因此只能核对静态图，不能重放同步过程。 |
...
| `P012.S0081–P012.S0082` | 补图 20 展示了用替代 marker 组合 PC(34:1) 与 PC(32:1)（均为 `[M+H]+`）进行的稳健性验证。 | 说明至少验证过双 marker 组合；Supplementary Fig. 20 已可回看，但原始事件表未摄入，量化性能仍不能独立重算。 |
...
| `P013.S0022–P013.S0024` | EIM 峰的 CCS 由淌度峰顶按公式（1）计算：`CCS = convertor × z/K₀ × √(1/(T·M)) × √((M+m)/m)`；z 为电荷，1/K₀ 为淌度，T=305 K，M 与 m 分别为原文所称 “analyst ion” 和 N₂ 的精确质量。 | 全文 pack 把根号与分数拆散；本式由主 PDF 公式图面逐项复核恢复。“analyst ion”语义上疑为 “analyte ion”，但本文保留原文措辞，不静默校正。 |
...
| `P013.S0047–P013.S0048` | 分隔线上方的峰判为单电荷，下方判为多电荷（示例见补图 21a、22）。 | 给出决策方向；Supplementary Figs. 21–22 已可回看，但分隔线完整定义与运行配置仍不足。 |
...
| `P013.S0065–P013.S0066` | 所有数据集的电荷态判定和同位素注释结果见补图 21b、c。 | Supplementary Fig. 21 已随官方补充 PDF 取得；原始峰表未摄入，不能独立重算其汇总结果。 |
...
| `P013.S0072` | 随后用 PRM-PASEF 对这些推定候选进行靶向 MS2 采集。 | 身份验证依赖另一批单细胞的额外靶向采集；Supplementary Methods S5/Table 5 已补参数，同时暴露 30/10 eV 冲突。 |
...
| `P014.S0012` | 公式（5）把 `Score_CCS` 设为分段函数：`ΔCCS≤TOL_min` 时为 1；`TOL_min<ΔCCS≤TOL_max` 时为 `(TOL_max−ΔCCS)/(TOL_max−TOL_min)`；`ΔCCS>TOL_max` 时为 0。 | pack 中括号和中间项被版面拆散；此处按主 PDF 图面恢复。结合公式（6）和本文阈值，即偏差≤3% 满分、3%–6% 线性降至 0、>6% 为 0。 |
...
| 句子 ID | 忠实中文含义 | 方法/可复现性解释 |
|---|---|---|
| `P014.S0020–P014.S0021` | 不同置信等级中经同位素确认的代谢物数量见补图 21d。 | 具体数量不在主 Methods 文本；Supplementary Fig. 21d 已可回看，但原始注释表未摄入。 |
...
- 临时重建的 manifest 中，PDF 第 26、27、28、29 页均为 `lines: []`，所以自动 pack 完全漏掉表单内容。以下内容来自对这四页图像的逐项目视转录，**不属于 184 个主 Methods ID，也不能声称具有句子 ID**。
...
- 主 Methods 只直接出现 timsControl 4.0、TDF-SDK 2.8.7.1、Rdisop 1.58.0、AllCCS 1.10、Lipid4DAnalyzer 2.1、SIRIUS 6.0.4；Reporting Summary 另列 R/Seurat/Monocle/FlowJo/Xcalibur/Skyline/SCMeTA 与 MetCell 1.0.22，Supplementary Methods S6–S9 又补出部分版本、函数和输入处理。随机种子、若干聚类/轨迹参数与运行级配置仍没有说明。
...
| `P014.S0032–P014.S0033` | Supplementary Data 文件存于 Zenodo：`10.5281/zenodo.15189109`。 | 当前主 PDF 未附这些文件；本次仅定点核查 Data 2/4 的版本字段，六份文件尚未全部逐值审计，方法参数/输出表仍不能整体视为已复现。 |
...
- NAD⁺ 准确性验证使用 mCherry-Finad-293T，FK866 处理后分别用荧光流式与 IM-resolved mass cytometry 测量（`P005.S0029–P005.S0036`）。图中流式组细胞数为 10,594/10,770，质谱流式组为 1,610/1,544（`P006.S0040–P006.S0043`）；Supplementary Methods S8 补出 FK866 `10 nM, 24 h` 和 GFP/mCherry 读出，但随机分组和完整门控仍未报告。
...
- 60 周龄实验共分析 8,545 个肝细胞（`P008.S0045–P008.S0049`）；主文文本包没有报告小鼠只数，Supplementary Methods S5 补出为 **2 只雄鼠**。
- 全部动物为雄鼠这一限制见 Reporting Summary 与 Supplementary Methods S4–S5；主 Results 常仅写周龄与 N，正文结论不应外推到雌鼠。
...
- 绝对定量/LOD：扩展图说明用 LC–MS 标准曲线定量 adenine、nicotinamide、arginine、alanine；单细胞质谱流式 LOD 通过“强度—SNR”线性回归估计，并把 SNR=3 的强度转换为 amol/fmol（`P023.S0003–P023.S0010`）。Supplementary Methods S8–S9 补出提取体积/溶剂、UHPLC–Orbitrap 平台、Skyline 和内外标策略；MOESM12 又给出四条曲线的浓度范围 0.025–0.8、0.05–1.6、2.5–20、5–40 ppm，但回归权重、残差诊断及完整反算公式仍未给出。
- 主文只说“整合后消除批次效应”（`P005.S0059–P005.S0060`）、“无监督方法”（`P006.S0024–P006.S0026`）和“伪时间分析”（`P006.S0061–P006.S0065`）。Supplementary Methods S7 补出 LogNormalize、2,000 个高变特征、25 PCs、CCAIntegration、FindNeighbors/FindClusters、UMAP/t-SNE 和 Monocle 2.26.0；仍缺邻居数、cluster resolution、轨迹参数、随机种子和鼠层统计模型。
...
| `P002.S0036–P002.S0049`、`P017.S0003–P017.S0010` | accumulation time 由 100 ms 缩短至 50 ms以减轻 space-charge，并调整 IM ramp voltage 偏向低质量离子；扩展图给 N=600 和箱线图约定。 | Supplementary Table 4（S40）现已补出实际 ramp、accumulation time 与 low-mass 设置；不计入主 Methods 的 184 个 ID。 |
...
##### 9.1 已由 Supplementary Information 补出的内容

1. **仪器窗口与累积参数**：Supplementary Table 4（S40）给出默认/选择性 TIMS 的 ramp、accumulation time、low mass 和电压设置；Supplementary Table 5（S41）给出 low/high PRM-PASEF 的前体与采集质量范围。
2. **MetCell 配置**：Supplementary Table 6（S42）给出 marker m/z/mobility、峰宽、容差、叠加细胞数、LOESS 与注释参数；表下注明数值可调且所列为 HepG2 设置，其中 `intensity_abs_threshold_upper=60,000`、`intensity_abs_threshold_lower=600,000` 的标签/数值次序可疑，仍需用归档代码和真实配置核验。
3. **样本与实验流程**：Supplementary Methods（S4–S9）补出细胞培养、肝灌流/解离、140 mM ammonium formate（pH 7.3）、`4×10^5 cells ml^-1`、60 周龄 N=2、FK866 `10 nM, 24 h`、LC–MS 提取/仪器/内标，以及部分染色流程。
4. **软件与分析框架**：同一文件补出 MetCell 1.0.22、SCMeTA 0.2.7、Seurat 5.0.1、Monocle 2.26.0、2,000 个高变特征、25 PCs 和 CCA integration。Supplementary Figs. 1–26 与 Tables 1–6 均在该 44 页文件中，但本次只对支撑上述关键结论的页面定点复核，不把文件存在等同于逐图复现。
5. **Source Data**：官方 11 个 XLSX 的 44 个工作表已逐表核对，闭合 Fig. 1–6 与 Extended Data Fig. 2/4/7/8/9 的数值口径、Bonferroni 分母、逐鼠比例和标准曲线；文件哈希与关键发现见上方“官方 Source Data 逐表审计”。

##### 9.2 仍未摄入或仍未报告的关键材料

- **独立开放文件**：Supplementary Data 2/4 已定点核查版本差异，但 Data 1–6 尚未全部逐值审计；Supplementary Video、原始 MS 数据和 MetCell 归档代码也未摄入。Source Data 已审计，但它不能替代完整峰表、候选库快照、逐样本 QC 与原始—最终细胞追踪。
- **细胞与动物元数据**：培养基/温度/汇合度和主要解离步骤已有补充说明，但传代号、试剂 lot、禁食/取材时点、细胞活率阈值、每鼠进入质谱的细胞数与丢失原因仍不完整。
- **上机与 LC–MS**：缓冲液、细胞密度、提取液、仪器、内/外标和四条标准曲线浓度点已有说明；上样体积/等待时间、LC 梯度、回归权重/残差、QC、回收率和基质效应仍不足。
- **流式与染色**：FK866 剂量/时长、GFP/mCherry 比值以及固定/切片/染色步骤已补出；激光/滤光片、电压、补偿、双细胞门、代表性 gating 图、成像参数和定量规则仍欠缺。
- **MetCell 与下游分析**：Table 6 和 Supplementary Methods 已补出大量默认值、归一化与 integration 框架，Source Data 也闭合 Bonferroni 分母为 4,541；仍缺 charge-separation line 的完整定义、每批 acquisition-time table、blank 配对、邻居数、cluster resolution、UMAP/Monocle 细参、随机种子，以及差异代谢的鼠层模型。
- **研究执行记录**：随机化列表、盲法编码/解盲、批次布局、QC 插入、run order、停机/重跑规则及“所有尝试成功”的原始日志仍不可见。
...
15. **60 周龄验证规模很小。** Supplementary Methods S5 将 8,545 个 hepatocytes 定位到 2 只雄鼠；它可提供中间年龄的方向性支持，但单细胞数不能替代 2 个生物学重复。
16. **“无样本排除”不等于无选择性。** 事件、峰、低检出 feature、blank 不达标峰均被算法过滤；这与样本级排除声明并不冲突，但技术附录必须同时写清。
17. **绝对量锚定使用不常见的中心量＋误差组合。** 主分析采用 median-to-median 映射；Source Data 证明 Fig. 4 的四个 LC–MS 中心值是 6 次重复中位数、误差条却是样本标准差。mean-to-mean 只作为敏感性分析且未给一致性系数，因此结果应写成尺度锚定的估计量，而非逐细胞独立绝对定量。
18. **PRM 碰撞能有未解释冲突。** Supplementary Methods S5 写 30 eV，Supplementary Table 5（S41）写 low/high 均为 10 eV；可能是方法文字与仪器表对应不同层级，也可能是笔误，复现时不能任选其一。
19. **MetCell 上下阈值标签疑似倒置。** Supplementary Table 6（S42）为 HepG2 列出可调参数 `intensity_abs_threshold_upper=60,000`、`intensity_abs_threshold_lower=600,000`；在没有运行配置或代码语义核对前，应原样登记而非自动互换。
20. **HepG2 的 MS2 总数冲突。** 主 Results/Fig. 3 为 1,350，Supplementary Table 1 为 1,352；两者最终都报告 770 个注释，附件没有解释多出的 2 条谱。
21. **Jurkat/MEF 行发生交换。** Supplementary Tables 1/2 对 detected peaks 与 M0 level-1 的 Jurkat/MEF 归属相反，singly charged 计数也跟随交换后的 detected 行；本文保留两表原值，不代作者重排。
...
- `P013.S0022–P013.S0024` 的 CCS 公式（1）在 pack 中被严重拆散；本次已回看主 PDF 图面恢复根号、分数与转换常数位置，恢复内容与 pack 来源明确分层。
- `P014.S0011–P014.S0013` 的公式（4）–（6）在抽取顺序中错位、分段括号残缺；本次已按主 PDF 图面恢复组合式、三段 CCS 得分与相对偏差定义。
...
- 公式（1）的 PDF 原文写 “analyst ion”，语义上疑为 “analyte ion”；本报告在恢复公式时仍保留原词，不静默改写为确定事实。
...
- `P013.S0022–P013.S0024`：CCS 公式（1）抽取破损，已由主 PDF 图面恢复；“analyst ion”原词仍存疑。
- `P014.S0011–P014.S0013`：组合评分公式版面错序/分段符号破损，已由主 PDF 图面恢复。
- PDF pages 26–29：manifest 0 行、0 句，Reporting Summary 为视觉转录。
- Supplementary Information 已补出 Tables 1–6、相关方法与图，11 个 Source Data XLSX 也已逐工作表审计；它们均不属于主文 pack 的句子 ID。Supplementary Data 2/4 已定点核查版本差异，但六份文件尚未全部逐值审计；视频、原始数据与归档代码仍未摄入，关键配置语义、门控、blank 配对和鼠层统计仍需回查。
...
- **低置信解析不被静默修复：**主图图注与双栏正文混排，`P004.S0016`、`P005.S0018`、`P008.S0017` 等同一 ID 混合图注与正文。孤立 P 值与破损公式已通过主 PDF 图面定位并明确标注证据类型；组别顺序等仍无法恢复之处继续标为 `EXTRACTION_CHECK`。
- **复现边界：**此次取得官方 Supplementary Information，并逐工作表审计 11 个 Source Data XLSX，补齐选择性累积、PRM-PASEF、60 周龄 N、实验流程、统计分母与逐鼠/锚定口径，同时保留碰撞能与 Table 6 阈值冲突。Supplementary Data 2/4 只做了版本差异定点核查，六份文件尚未全部逐值审计；视频、原始数据和代码归档也未摄入，因此 frame gating、blank、配置语义、逐鼠 QC 与完整外部比较尚未独立复现。

...
## 01｜为什么固定的 GWAS 效应会漏掉真实差异

常规 GWAS 估计的是边际效应：把不同年龄、性别和生活方式的人放在一起，求一个平均的基因型–表型关联。若一个等位基因只在某类人群中起作用，或者在两类人群中方向相反，平均后可能变小甚至接近零。
...
发现阶段包括 UKB1 的最多 273,453 人和 BBJ1 的最多 166,757 人，共 440,210 人；队列内复现使用 UKB2 的最多 38,149 人和 BBJ2 的最多 65,373 人。外部复现又纳入 All of Us 的欧洲、非洲和美洲人群，以及日本 J-MICC/HERPACC、JPHC 和以色列 HPP，连同队列内复现合计 539,794 人。
...
局部位点能提供线索，也最容易被过度解释。ABCG2 位点的效应在低“肉类和奶酪”模式人群中更明显，作者用 ABCG2 参与肠道尿酸外排、肉类嘌呤可能遮蔽遗传效应来解释。可是正文把结局写成 eGFR，Fig. 2a,b 和 Extended Data Fig. 6 的纵轴、图注却都是尿酸；这个图文冲突使机制叙事只能作为候选解释，不能静默选定一个结局。
...
更具警示性的例子是 PITX2、纳豆与心律失常。BBJ1 的总体 G×E 为 \(P=2.8\times10^{-12}\)，但同一批房颤/房扑患者在开始华法林后明显减少纳豆摄入；分开分析房颤/房扑和其他心律失常后，G×纳豆分别为 \(P=0.84\) 和 \(2.6\times10^{-4}\)，作者仍判定两者都没有达到研究的多重校正门槛。BBJ2 的总体信号也未复现（\(P=0.40\)）。BBJ2 多数人在 DOAC 已替代华法林的年代入组，而 DOAC 不要求限制纳豆。
...
把各环境合并估计后，UKB1 有 7 个性状、BBJ1 有 11 个性状呈显著 G×E 遗传度，重叠的是身高、BMI、HDL-C 和舒张压。G×E 与边际遗传度之比在 BMI 明显高于身高：UKB1 为 0.100（95% CI 0.057–0.142）对 0.028（0.007–0.048），BBJ1 为 0.245（0.130–0.360）对 0.062（0.009–0.115）。
...
作者以脉压为例：UKB1 的 4 个脉压 G×E 位点都主要由年龄驱动，英国和日本队列都被平分为年轻、年长两组。跨人群 MAGMA 富集提示，年轻组更偏血管平滑肌收缩通路，年长组更偏细胞衰老通路。

将遗传效应投影到 Tabula Sapiens 后，两组都富集血管细胞，但相对强度不同。猴动脉单细胞图谱进一步显示，年轻组关联两个平滑肌细胞亚型（\(P=8.3\times10^{-3}\) 和 \(9.4\times10^{-3}\)），年长组关联一个冠状动脉内皮细胞亚群（\(P=2.3\times10^{-3}\)）。
...
CETP 是最需要克制的案例。CETP 位点对 LDL 中甘油三酯占比（LDL_TG_pct）的 G×E 为 \(P=1.8\times10^{-12}\)，边际关联却为 \(P=0.98\)，且男女效应方向相反。LDL_TG_pct 每升高 1 个标准差，与全因死亡和冠心病死亡的 HR 分别为 1.23（95% CI 1.21–1.25）和 1.25（1.18–1.32）。

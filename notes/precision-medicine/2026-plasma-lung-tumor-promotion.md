...
CANTOS 随机心血管预防试验的探索性分析曾提示，抗炎药 canakinumab 可能降低肺癌发生率；但另外针对转移性一线或辅助治疗场景的非小细胞肺癌试验未显示疗效。两类证据共同提示 IL-1β 抑制可能存在一个很早的干预窗口，真正的问题是：怎样在临床可见肿瘤出现以前，找到更可能受益的人？
...
图 1 的关键不是单独某一种蛋白，而是三层一致性：汇总 8 个外部数据集的随机效应分析后，14 种蛋白各自的合并关联方向均为正；UKCTOCS 中 WFDC2、CXCL17 和 CEACAM5 在 98 例未来病例里于诊断前约 2 年开始高于 150 名对照；基因表达又主要定位于肺上皮、髓系细胞和成纤维细胞。

需要区分两种“验证”。外部数据复现的是 14 个单蛋白的合并关联方向，不是完整 XGBoost 模型在 8 个数据集中的校准、灵敏度和临床净获益；完整模型的性能比较仍主要来自同一 UK Biobank 的留出集。
...
这个按中位数划分的高低组并非完全随时间漂移：安慰剂组从基线到 12 个月有 82%（n=1,314）仍处于同一组，Cohen's \(\kappa=0.64\)。它支持一年内的中等稳定性，却不等于已经建立跨平台绝对阈值。
...
| References | `P017.S0004–P020.S0139` | 532/532 | 识别为文献条目，不当作 Results |
...
| 内嵌补充图 | `P032.S0002–P046.S0007` | 232/232 | 检查 Fig. S1–S9 图注、样本和统计说明 |

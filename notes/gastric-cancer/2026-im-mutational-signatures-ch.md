...
<!-- wechat-style-reviewed: 2026-09-08 -->

发现胃肠化生以后，临床上最难回答的问题往往不是“它是不是癌前病变”，而是“这个人究竟有多大概率会继续进展”。

大多数胃肠化生长期稳定，文中给出的年进展风险约为 0.18%–0.25%（`P012.S0041–P012.S0043`）。如果所有患者都接受同样密集的胃镜随访，成本和负担很高；如果只依赖年龄、幽门螺杆菌和胃肠化生评估系统（OLGIM）分期，又可能漏掉已经开始克隆演化的病灶。

地区差异让问题更复杂。日本、韩国等胃癌高风险地区，与新加坡、北美等中低风险地区相比，不仅发病率不同，幽门螺杆菌菌株、上皮突变过程和宿主炎症背景也可能不同。

这项研究分析了 1,582 个胃肠化生样本。作者给出的答案是：高深度测序可以找到低频上皮克隆，SBS17 描绘了不同于年龄累积的突变过程；真正进入早期胃肿瘤关联或模型的候选变量，则是 ARID1A 截短突变、mutation count 和高水平克隆性造血。把分子变量加入临床模型后，受试者工作特征曲线下面积（AUC）从 0.72 升至 0.773，但模型只有 26 例早期胃肿瘤事件，尚不能直接改变随访方案。
...
简明图注：Fig. 1 从六地样本分布进入 Hp 菌株系统发育，再用结构模型和四次独立 co-IP 比较 CagA–ASPP2 结合。地区差异与体外结合结果相互支持，但尚不能估计某个 CagA variant 对个体进展风险的独立贡献。
...
近期招募的胃窦胃肠化生来自新加坡、韩国、香港、美国、日本和台湾；作者同时加入既往新加坡 TransGCEP1000 队列。除此之外，作者从突变 VAF 较可观的样本中选取 20 对 IM–normal 做平均 60.5× 全基因组测序（WGS）。另一个韩国跨阶段队列包含 14 人、38 份正常胃、异型增生或早期胃癌样本，用于酶促甲基化测序（EM-seq）；它不是同一批 IM 的配对甲基化组。
...
这不是单纯的技术细节。如果测序深度不够，体量最小的异常克隆会直接从数据里消失，研究者可能误以为癌前组织没有 driver；低 VAF 本身却不能证明这些克隆出现得最早。

高深度 panel 最终识别到 47 个显著突变基因和 2,100 个驱动突变（driver mutations），其中 25 个基因此前未在胃肠化生中报告。

## 04｜哪些上皮 driver 真正关联早期胃肿瘤

在 Fig. 2 汇总的 1,095 个胃窦 IM 中，日本/韩国受试者的每样本体细胞突变数中位数为 28.5，华人受试者为 20（Wilcoxon P = 4.1 × 10^-11）；人群频率部分比较 909 份华人样本（新加坡 836、香港 52、美国 13、台湾 8）与 142 份日本/韩国样本。跨地区合并的 EGN 分析比较 ARID1A truncating mutation 携带者与未携带者：携带者发生同期或后续 early gastric neoplasia（EGN，高级别异型增生或早期胃癌）的优势比为 6.2（P = 1.5 × 10^-3）。

![Fig. 2：跨地区 driver landscape 与 ARID1A 风险](../../assets/gastric-cancer/2026-im-mutational-signatures-ch/fig2-page6.png)

简明图注：Fig. 2 的 driver 图谱覆盖 1,095 个胃窦 IM，人群频率比较包括 909 份跨四地华人样本和 142 份日本/韩国样本；跨地区合并后，ARID1A 截短突变携带者相对未携带者更常见同期或后续 EGN（OR = 6.2，P = 1.5 × 10^-3）。这是队列内关联，不是已经外部纵向验证的预测标志。
...
![Fig. 3：KRAS–MAPK 改变、细胞状态与类器官实验](../../assets/gastric-cancer/2026-im-mutational-signatures-ch/fig3-page8.png)
...
20 个经 VAF 富集选择的 IM WGS 样本中，作者识别到 SBS1、SBS5/40、SBS17 和 SBS18；“正常胃中不典型”来自 79 例已发表外部正常胃数据，胃癌比较来自 122 例 ICGC 数据，并非直接在这 20 个 matched normal 上重做 signature。映射到 1,095 个胃窦 IM 后，287 个样本（26.2%）可检测到 SBS17，而选择性 WGS 集合为 60%，两者不能当作直接患病率复现。

SBS17 在最晚复制区域富集 14.5 倍，而且与吸烟相关，却没有随年龄显著增加。类器官中的 OXPHOS、氧耗和 8-oxo-dG 结果把它与氧化损伤联系起来，但这些实验仍不能证明 OXPHOS 直接制造了 SBS17。

![Fig. 4：SBS17、复制时序与氧化损伤](../../assets/gastric-cancer/2026-im-mutational-signatures-ch/fig4-page10.png)
...
甲基化数据给出了另一层背景。在独立的韩国跨阶段队列中，14 人、38 份正常胃、异型增生和早期胃癌样本的 CIN/MSI 相关病灶出现广泛低甲基化：受检 CpG 中 6,741,487 个（35.7%）低甲基化，而 326,646 个（1.7%）高甲基化。IM 中的 SBS17 mutations 与该队列后续病变的低甲基化区域重叠，原文将其 specificity 报为 75.7%；这是跨队列、跨阶段的区域重叠，既没有 IM methylome，也不能证明时间顺序。

## 07｜血液里的克隆能增加多少风险分层信息

作者在 1,067 名受试者中识别到 286 个已报道的克隆性造血（CH）相关基因突变，涉及 225 人，主要 driver 为 DNMT3A、TET2、ASXL1 和 PPM1D。

临床关联分析纳入 765 名 OLGIM II–IV 期受试者，其中 43 人有异型增生、26 人有 EGN。当 CH 变异等位基因频率超过 5% 时，high CH 在多变量分析中仍与 EGN 相关（OR = 4.19，95% CI 1.50–11.74；P = 6.4 × 10^-3）；ARID1A 截短突变的多变量 OR 为 4.54（95% CI 1.52–13.55；P = 6.7 × 10^-3）。只使用 OLGIM、年龄和性别的模型 AUC 为 0.72；加入 mutation count、ARID1A mutation 和 high CH 后，AUC 提高到 0.773。

在随访至少 5 年的 GCEP1000 子集中，312 人只有 9 例 EGN；high CH 与 ARID1A 截短突变的多变量 OR 分别为 10.62（95% CI 2.43–46.45）和 11.51（95% CI 2.28–58.20），区间很宽，AUC 则从 0.671 提高到 0.811。Methods 只说明用 `glm` 并经单变量筛选后进入多变量，没有报告独立 train/test、交叉验证、bootstrap、校准或决策曲线；这些很可能是表观 AUC，不能当作泛化性能。

![Fig. 5：CH driver、临床关联与风险模型](../../assets/gastric-cancer/2026-im-mutational-signatures-ch/fig5-page13.png)
...
合并各突变类型后，PIGR mutations 整体与 high CH 相关（OR = 2.3，P = 0.031），信号主要来自 truncating mutations（OR = 2.8，P = 0.017），而 missense/in-frame mutations 不显著（OR = 1.7，P = 0.43）。PIGR mutation status 与 PIGR expression 也不相关（P = 0.43）；这些未调整的 Fisher tests 没有控制年龄、总突变负荷或多重检验，因此没有直接证明功能丧失，更没有证明 CH 先造成 PIGR 突变。

数据方向与这个模型一致。scRNA-seq 比较 3 个 CH-high 与 10 个 CH-low IM，总 immune cells 的 P = 0.049；bulk RNA-seq 比较 20 个 high CH 与 94 个 non/low CH IM，IgA+ plasma cells 和 mature T cells 的 P 分别为 0.0069 和 0.0034。平衡下采样 1,000 次中，99.4% 保持同方向（比例检验 P = 0.006）。IgA+ plasma-cell abundance 与 bacterial reads 相关（r = 0.43，P = 1.1 × 10^-5），与 human reads 则不相关（r = 0.056，P = 0.59）。

补表中的菌属差异仍属探索性：high CH 组相对 low CH 组的 Streptococcus、Neisseria、Fusobacterium 和 Gemella 中位丰度分别为 4.1、15.6、9.7 和 6.1 倍（P = 0.0082、0.012、0.0079、0.0011），未见多重比较校正说明。PIGR truncation 与 Streptococcus 丰度相关（单侧 Wilcoxon P = 0.034），与 Helicobacter 则不相关（P = 0.80），这项阴性对照让结论更具体，也仍不足以闭合因果链。

FISH 和 Stereo-seq 使用的不是 IM，而是胃癌组织。正文没有报告 FISH 的病例分母；Stereo-seq 才是 4 例，其中 2 例检出 Streptococcus 空间簇：1 例与 CXCL8 高表达区域重叠，另 1 例与肠型上皮和 IgA plasma-cell signatures 重叠。预选 oral-microbiome reads 较高的 15 例胃癌切片中，4 例见局灶 Gram-positive cocci。这些结果只能为机制模型提供探索性支持。

![Fig. 6：CH、PIGR、IgA 与口腔菌的关联模型](../../assets/gastric-cancer/2026-im-mutational-signatures-ch/fig6-page14.png)

简明图注：Fig. 6 的单细胞比较为 3 个 CH-high 对 10 个 CH-low IM，bulk 比较为 20 个 high CH 对 94 个 non/low CH IM；FISH 的病例分母未报告，Stereo-seq 来自 4 例胃癌，只有 2 例检出 Streptococcus 空间簇。这些结果支持横断面关联和机制线索，不能证明 CH 先造成黏膜屏障与微生物改变。
...
简明图注：Fig. 7 把 Hp 菌株、上皮 driver/SBS、CH、免疫和微生物放回 Correa cascade。蓝框是作者提出的风险分层或干预机会，不是新增实验，也都尚未在患者中验证。

## 10｜对我们的研究有什么可借鉴

如果要迁移到胃癌精准预防队列，可以把病理分层、高深度上皮 mutation panel、外周血 CH、吸烟与 Hp 暴露、口腔/胃微生物组和长期 EGN 终点放在同一设计中。

风险模型必须保留 OLGIM 等临床基线，再检验 ARID1A truncation、mutation burden 和 high CH 能带来多少增量预测。SBS17 可作为过程或暴露候选，但本研究没有检验 SBS17–EGN 关联，不能预先把它当作进展预测变量。

机制研究则可以从 CH driver 分层开始。TET2、DNMT3A 和 ASXL1 不一定对应同一种黏膜炎症程序，需要用配套单细胞、空间转录组、IgA repertoire 和微生物组逐一验证。

## 11｜这些结果仍需要冷静看待

首先，跨国家样本很多，但长期进展事件主要依赖新加坡队列。其他地区不少结局只是基线观察；总体模型只有 26 例 EGN，GCEP1000 子集只有 9 例。变量又经同一数据的单变量筛选，没有重采样验证、校准或外部验证，因此“预测未来进展”的证据没有样本总数和表观 AUC 看起来那么强。

其次，高深度靶向 panel 以深度换广度。它适合捕捉低 VAF 克隆，却会遗漏 panel 外 driver、结构变异和非编码调控改变。

第三，SBS17、低甲基化与 OXPHOS 的关系仍以相关和跨队列区域重叠为主；用于 signature discovery 的 20 对 WGS 又是 VAF 富集选择，约 60× 深度仍可能漏掉小亚克隆。pyrvinium 依赖药理抑制，Supplementary Fig. S1 只给每条件 3 次技术重复且未说明独立生物学重复；DCA 实验也没有 SBS17 长期累积读出。
...
最后，官方补充包现可取得 Supplementary Figures S1–S7 和 Tables S1–S14，但仍没有本文可直接复算的 Source Data。MSKCC 对照不是可比健康对照：56 种非血液肿瘤中 76.3% 为白人，有治疗史资料者中 58.9% 接受过治疗；且其总人数在 Results/Fig. 5 中为 24,146、Methods 中为 24,126。Fig. 5A 按 1,067 名受试者，Fig. 5C 按 1,095 份样本（234 + 861）。模型又限定 OLGIM II–IV，不能外推 OLGIM I 或 all-comer；跨地区频率还混合采样地、祖源、年代和 Hp 根除，组间 909:142 失衡，不能据此解释国家发病率因果。

---

## 技术附录

以下内容保留论文基本信息、完整主图说明、Results/Methods 证据、复现参数和证据边界。读者正文已经展示 7 张主图各自的真实图页，附录不重复嵌入；完整 panel 图注、图像路径和正文位置均保留。
...
- 图像截取说明：已将主文 Fig. 1–7 的真实图页渲染为图片，位于 `assets/gastric-cancer/2026-im-mutational-signatures-ch/`；相邻的正文/续图注页不再误作主图展示。
...
- 补充材料：本地 PDF 不含 supplements；本次另从[官方 PMC 补充包](https://www.ebi.ac.uk/europepmc/webservices/rest/PMC13056296/supplementaryFiles)取得 Supplementary Figures S1–S7（DOCX，SHA-256 `ce6d65fa27014bba9d2bab6d2f6ed87ae0827913b8b2adf8605da2baebc02721`）和 Tables S1–S14（XLSX，SHA-256 `76893d7a6c097131cc534954f77fa13719988fa51356fb594fb1386a8aef429a`）并做定向复核。补充材料不分配主 PDF 句子 ID；Source Data 仍未包含在该包中。
- 低置信点：图表 OCR 中有拆词与希腊字母噪音；Methods 的 `P019.S0010–P019.S0012` 写作 “Student t test (two-way)”，原意可能是 two-tailed，但不能静默修正；MSKCC 对照总人数在 Results 为 24,146、Methods 为 24,126，见下文冲突记录。`fig5-page14.png` 与 `fig6-page14.png` 内容哈希完全相同，是保留未改的历史别名；前者不再嵌入。

### 补充材料定向复核

- Supplementary Table S10（`Sheet 10`, B8:O11、B51:O54）给出正文第 07 节采用的多变量效应量：765 人分析中 high CH 的 EGN OR 4.19（95% CI 1.50–11.74），ARID1A truncation OR 4.54（1.52–13.55）；312 人 GCEP1000 子集中两者分别为 10.62（2.43–46.45）和 11.51（2.28–58.20）。后者仅 9 个 EGN，宽区间提示估计不稳定。
- Supplementary Table S12（`Sheet 12`, C45:F60）列出 high/low CH 的菌属中位丰度倍数与 P 值：Neisseria 15.6/0.012、Streptococcus 4.1/0.0082、Porphyromonas 8.4/0.026、Actinomyces 3.2/0.028、Fusobacterium 9.7/0.0079、Capnocytophaga 11.9/0.042、Leptotrichia 7.0/0.040、Gemella 6.1/0.0011、Haemophilus 13.7/0.031；表内未说明多重比较校正，均按探索性结果解读。
- Supplementary Table S11 支持 PIGR truncation 与 Streptococcus abundance 的单侧 Wilcoxon P = 0.034，同时给出 Helicobacter 阴性结果 P = 0.80。Supplementary Fig. S5 展示 ROC/PR curves，但没有置信区间、校准或外部验证。
- Supplementary Fig. S1 的相关药物 panel 每条件只有 3 次技术重复并标为 representative results，未说明独立生物学重复。Supplementary Fig. S2 中 DCA 5 mmol/L、24 小时的 basal/maximal respiration P = 0.0166/0.0386（单侧 Welch）；2 周处理增加 ROS/γH2AX，但图注未给重复数，也没有 SBS17 accumulation 终点。
...
| Fig. 2 | A：1,095 个胃窦 IM 的 36-gene oncoprint 与 EGN forest plot；B：SOX9、ARID1A 等在不同人群的 lollipop/frequency；C：国际队列和 TransGCEP1000 中 ARID1A truncation 与 EGN。 | Chinese-ancestry `n = 909`（新加坡 836、香港 52、美国 13、台湾 8）对 Japanese/Korean `n = 142`；ARID1A 合并 OR 6.2、P = 1.5 × 10^-3。来源 `P005.S0019–P006.S0030`。 | `fig2-page6.png` | [04](#04｜哪些上皮-driver-真正关联早期胃肿瘤) |
| Fig. 3 | A：KRAS、BRAF、MAP2K1 gain-of-function 与 MAP2K4、MAP3K1、NF1 loss-of-function；B：KRAS/MAPK-mutant bulk RNA GSEA；C：scRNA epithelial type、cell cycle 与 KRAS–ERK score；D：CDX2；E：胃/肠谱系表达；F：severe 对 mild IM organoid；G：pyrvinium 活力。 | Bulk 为 10 个 mutant 对 94 个无可检出改变；down-sampling 1,000 次；organoid 为 6 IM 对 4 normal，Fig. 3G 为 1 μmol/L、72 h、每组 4 个 biological replicates，linear mixed-effects P < 0.0005。来源 `P007.S0001–P009.S0007`、图注 `P008.S0002–P008.S0008` 与 `P009.S0037–P009.S0046`。 | `fig3-page8.png` | [05](#05｜kras–mapk-改变能否提供干预线索) |
| Fig. 4 | A：20 对 IM–normal WGS 的四类 SBS；B：79 个正常胃、20 个 IM、122 个胃癌；C：各 signature VAF；D：复制时序；E：6 IM 对 4 normal organoid 的 OXPHOS；F：OCR；G：8-oxo-dG；H：年龄和吸烟。 | SBS17 在最晚复制区 14.5×；OCR 的 basal/maximal P = 0.035/0.015；8-oxo-dG 每个 organoid 六次 technical replicates，按个体内 mean ± 1.5 SD 去异常；targeted 映射为 287/1,095。来源 `P009.S0008–P011.S0019`、图注 `P010.S0047–P010.S0057` 与 `P011.S0043–P011.S0052`。 | `fig4-page10.png` | [06](#06｜sbs17-为什么不像普通的年龄累积噪音) |
| Fig. 5 | A：1,067 名受试者的 CH oncoprint；B：DNMT3A、TET2、ASXL1、PPM1D 与 MSKCC 对照；C：CH 与年龄/IM mutation count；D：CH gene 与吸烟；E：dysplasia/EGN logistic regression。 | 风险模型 `n = 765`（43 dysplasia、26 EGN）；GCEP1000 `n = 312`（20 dysplasia、9 EGN）。Fig. 5C 用 234 个 CHIP-positive 与 861 个 CHIP-negative IM samples，与 1,067 名受试者口径不同。来源 `P012.S0013–P015.S0002`、图注 `P013.S0031–P013.S0037`。 | `fig5-page13.png` | [07](#07｜血液里的克隆能增加多少风险分层信息) |
| Fig. 6 | A：high CH 关联的 IM drivers；B：PIGR mutation type；C：PIGR 与 CH genes；D：作者提出的屏障—微生物—炎症模型；E：scRNA immune composition；F：bulk deconvolution 与 bacterial/human reads；G：菌属差异；H：Streptococcus/S. anginosus FISH；I：Streptococcus 与 CXCL8 空间重叠。 | scRNA 为 3 high 对 10 low CH；bulk 为 20 对 94，另做 1,000 次平衡下采样；114 份 bulk 中排除 10 份 bacterial reads <0.02% 的样本后，微生物分析 `n = 104`，主文未报排除后的 high/low 分组数；FISH 分母未报告，Stereo-seq 4 例中 2 例检出空间簇。来源 `P015.S0003–P016.S0015`。 | `fig6-page14.png` | [08](#08｜血液里的克隆为什么会连接到胃黏膜) |
...
| SBS | single-base substitution signature | 单碱基替换突变签名 | SBS17 是本文重点关注的 IM-associated signature；见于 IM/胃癌，外部正常胃中不典型 |
...
| PIGR | polymeric immunoglobulin receptor | 黏膜上皮转运 IgA 的受体，truncating mutation 与高 CH 共现 | 作者据此提出屏障受损假说；本文只观察到共现和菌群关联，未证实功能丧失 |
...
作者还加入多个数据模态：从突变 VAF 较可观的样本中选 20 个 IM-normal 配对做 WGS，平均覆盖 60.5x；独立的韩国跨阶段队列用 14 人的 normal、dysplasia、early gastric cancer 共 38 个样本做 EM-seq；部分样本另有 bulk RNA-seq、scRNA-seq、IM microbiome、saliva microbiome、organoid 和 Stereo-seq 数据。整体设计以靶向深测序为主轴，再用不同样本集的 WGS、转录组、表观组、类器官和微生物空间证据补强机制解释。
...
driver gene 发现用 IntOGen pipeline，整合 7 个互补算法并做组合 q 值。突变签名分析先在 20 个经 VAF 富集选择的 WGS IM 样本上用 SigProfilerAssignment 推断 SBS1、SBS5/40、SBS17、SBS18，再把签名映射到 1,095 个 antrum IM 靶向数据中。CH 调用采用“反向 tumor-normal”思路：把 blood/saliva 当作待检测对象，IM 当作对照，筛选血液/唾液 VAF 更高的造血克隆突变。
...
中文图注（基于原文图注）：Fig. 2 展示 36 个代表性 driver genes 的 oncoprint、不同人群/地区的关键 driver mutation rate，以及 ARID1A truncating mutations 与 EGN 的关联。图中重点是跨四地华人样本中 SOX9 truncation 频率高，而日本/韩国高风险人群中 ARID1A、ARID2、ERBB3 等更常见。

IntOGen 在 IM 中识别 47 个显著突变基因，共 2,100 个 driver mutations，其中 25 个此前未在 IM 中报告。独立 48 个 IM 仅再次检出其中 35/47（74.5%），不是重跑显著性、频率或结局验证；另有 37/47（78.7%）至少出现在正常胃、TCGA pan-GI cancer 或 IntOGen 胃癌 driver 资源之一。

antrum IM 的平均 somatic mutation count 为 23，平均 VAF 2.9%，中位 VAF 1.7%。日本/韩国样本突变率高于跨四地的华人样本。SOX9 mutation 尤其在 Chinese-ancestry populations 中更常见，909 个样本中 114 个有 SOX9 mutations，其中 94 个是 truncating mutations；而高风险日本/韩国人群中 ARID1A、ARID2、ERBB3、KMT2D、KDM6A、CREBBP、PREX2 更常见。
...
中文图注（基于原文图注）：Fig. 3 展示 KRAS、BRAF、MAP2K1 的 gain-of-function 以及 MAP2K4、MAP3K1、NF1 的 loss-of-function 改变；bulk RNA-seq 显示 KRAS-ERK signatures 富集，支持通路活性升高；scRNA-seq 显示 cycling intestinal stem/TA cells KRAS-ERK 分数更高；IM organoid 通过 CDX2 和转录组确认，并对 pyrvinium 更敏感。
...
20 个经突变 VAF 富集选择的 IM WGS 样本中识别 4 类主导 signature：SBS1、SBS5/40、SBS17、SBS18。正常胃的比较来自已发表的 79 例外部数据，胃癌比较来自 122 例 ICGC 数据，并非直接在这 20 个 matched normal 上重做 signature；SBS17 在 IM 和胃癌中出现、在外部正常胃中不典型。SBS17 mutations 的 VAF 低于其他 SBS mutations，支持其更多代表晚期或 subclonal events；选择性入组与约 60× 深度也限制人群代表性和小克隆检出率。
...
映射到 1,095 个 antrum IM 靶向数据后，SBS5/40 和 SBS1 几乎普遍存在，SBS18 和 SBS17 分别在约 28.9% 和 26.2% 样本中可检测；WGS 选择集中 SBS17 为 60%，两者不能直接当患病率复现。高风险国家 IM 的所有 signature mutation rate 均更高，其中 SBS17 enrichment 更明显。SBS1、SBS5/40、SBS18 与年龄相关，SBS17 不随年龄显著相关，但与吸烟显著相关。它可视为突变过程或暴露候选；本文没有做 SBS17–EGN 关联，不能称为进展风险 biomarker。
...
作者进一步问 CH 如何影响 IM 进展。合并各突变类型后，PIGR mutations 整体与 high CH 相关（OR = 2.3，P = 0.031），特别是 truncating mutations（OR = 2.8，P = 0.017）；missense/in-frame 不显著（OR = 1.7，P = 0.43），PIGR expression 也不随 mutation status 改变（P = 0.43）。PIGR truncation 还与 TET2 CH mutation 共现（OR = 2.9，P = 0.044）。这些是未调整的 Fisher tests，没有控制年龄、总突变负荷或多重检验，提示屏障线索而非已证实的功能丧失。

微生物结果支持“屏障-口腔菌-炎症”的模型。PIGR truncation 与 Streptococcus abundance 升高有关（P = 0.034），与 Helicobacter 不相关（P = 0.80）。scRNA-seq 中 high CH IM 的 total immune cells 增多（3 对 10，P = 0.049）；bulk RNA-seq 在 20 对 94 中复现 IgA+ plasma cells（P = 0.0069）和 mature T cells（P = 0.0034）的升高。IgA+ plasma cell abundance 与 bacterial reads 相关（r = 0.43，P = 1.1 × 10^-5），与 human reads 不相关（r = 0.056，P = 0.59）。

CH-high IM 中多个口腔来源菌属升高，包括 Streptococcus、Neisseria、Gemella、Actinomyces、Haemophilus、Porphyromonas、Fusobacterium；补表给出倍数但未说明多重校正。美国配对唾液为 32 份，新加坡为 173 份、来自 154 人；胃与唾液富集菌中 13/20 genera 重合，其中 9/13 显著。FISH 在胃癌组织中的病例分母未报告；Stereo-seq 只做 4 例胃癌，2 例检出 Streptococcus 空间簇。预选 oral-microbiome reads 较高的 15 例胃癌切片中，4 例见局灶 Gram-positive cocci。由于对象不是 IM、样本少且有预选，这部分更适合作为机制假说支持。

#### 作者结论与证据强度

作者直接观察到：跨国家 IM 队列中存在复发 driver genes，且部分 driver frequencies 具有地区差异；ARID1A truncation 与 EGN 相关；KRAS/MAPK-altered IM 有相应 signature 富集和类器官药物敏感性；SBS17 见于 IM/胃癌、在外部正常胃中不典型，并与 late replication、OXPHOS/氧化损伤和吸烟相关；high CH 在本队列内与 dysplasia/EGN 独立相关，并提高内部表观 AUC。
...
|---|---|---|---|
| `P003.S0015–P003.S0029`（15/15） | 1,582 个 IM、98 个 normal、六地近期队列与 TransGCEP1000；20 对 WGS、14 人/38 份 EM-seq 及唾液微生物组。 | 02、03 | 不同分析的样本/受试者分母不同；逐人明细已从官方 Supplementary Table S1 取得，但不属于主 PDF。 |
...
审计结论：主 Results 连续范围 `P003.S0015–P016.S0015` 的 521/521 个 ID 已完成来源区间归类，未归类 ID：无。Manifest 自动标为 `results` 的 165 个 ID 均属主 Results；另有 356 个真实 Results ID 因图页、双栏和跨页续接被误标为 `supplementary`，已人工校正。这里的 521/521 是 ID inventory 完整性，不表示正文逐句复述了每个数字。
...
转录组层面使用 bulk RNA-seq 和 scRNA-seq，并复用此前数据加新样本。Organoid RNA-seq 用 SMART-Seq v4、HiSeq 4000、HISAT2/featureCounts，并与 GSE74843、GSE112369 合并后用 ComBat 校正。organoid 实验从正常胃和 IM 建立类器官，用 CDX2 immunostaining 和 RNA-seq 证明 IM identity，随后做 pyrvinium、ERK inhibitor、STAT3 inhibitor 处理和 ATP/colony assays。微生物部分结合 targeted DNA-seq、bulk RNA-seq PathSeq、bioBakery/Terra、唾液 shotgun metagenomics、Gram stain、FISH 和 Stereo-seq。
...
Logistic regression 用于 dysplasia/EGN 风险分析。输入包括临床变量和分子变量，输出 OR、P value 和多变量独立关联。本文策略是先单变量筛选，再在同一数据中多变量评估；Methods 未报告 train/test、交叉验证、bootstrap、校准或决策曲线。26/9 个 EGN 事件下存在明显过拟合风险，所报 AUC 应视为未验证的表观区分度。
...
独立 48 个 IM 再次检出 35/47（74.5%）driver genes，另有 37/47（78.7%）出现在正常胃、TCGA pan-GI cancer 或 IntOGen 胃癌资源至少之一；前者不是显著性、频率或结局的独立重验证。KRAS/MAPK RNA-seq 结果通过 1,000 次 down-sampling 控制组间样本数不平衡。CH 风险结果通过排除测序深度偏离均值 ±1 SD 的样本进行敏感性分析，说明 coverage 差异不是主要驱动。
...
- WGS：NEBNext Ultra II DNA Library Prep Kit 建库，NovaSeq 6000 PE150；从突变 VAF 较可观者选择 20 对 IM–normal，平均覆盖 60.5×，对齐 hs37d5 后以 MarkDuplicates 处理重复 reads。该选择不代表总体 IM，约 60× 也可能漏掉小亚克隆（`P003.S0022`、`P018.S0017–P018.S0026`）。
...
- CH/临床模型：blood/saliva 作为 Mutect2 “tumor”、matched IM 作为 control，并关闭 normal-artifact filter；至少 5 条支持 reads、blood/saliva VAF ≥2%，且至少为 matched IM VAF 的 2×。Logistic regression 用 R `glm`，单变量筛选后在同一数据拟合多变量；未报告数据拆分、重采样、校准或决策曲线。GCEP1000 取最早胃窦 biopsy，排除既往 HGD/胃癌和 OLGIM I，近期国际样本只用 baseline outcome，最终 765 人（43 dysplasia、26 EGN）（`P020.S0046–P021.S0014`）。
...
- 原位验证：IHC 的 MPO/CXCL8/CXCL2 分别为 1:10,000、1:100、1:50；Gram stain 为 crystal violet 与 safranin 各 1 分钟。FISH probe 1 μmol/L，88°C 3 分钟预热、42°C overnight；Stereo-seq 用 5 μm FFPE、DNBSEQ-T7 和 SAW `--detect-microorganism`/Kraken2，在 bin50 中去除 bacterial counts <5，并过滤 genes <3、total counts <20 或 mitochondrial transcripts >5% 的 bins；StereoMap/Stereopy 进行 total-count normalization、`log1p` 与 Leiden clustering（`P021.S0036–P022.S0028`）。

### Methods 证据覆盖审计

| 原文句子 ID | 忠实中文含义 | 方法学解释 | 复现注意点 |
|---|---|---|---|
| `P018.S0004–P018.S0008`（5/5） | 六地伦理审批、书面知情同意和 Helsinki 声明。 | 界定多中心人体样本合规性。 | 逐中心明细可查官方 Supplementary Table S1；主文没有统一展开。 |
...
| `P019.S0022–P020.S0007`（34/34） | Organoid 培养、RNA-seq、batch correction、药物/colony assay、western blot 与 CDX2/EPCAM staining。 | 在正常与 IM 来源类器官中验证状态和药物敏感性。 | Supplementary Fig. S1 已从官方包复核；关键 panel 仅 3 次技术重复且未说明独立生物学重复，遗传扰动缺失。 |
...
审计结论：主 Methods 连续范围 `P018.S0004–P022.S0028` 的 226/226 个 ID 已完成来源区间归类，未归类 ID：无；数据可用性 `P022.S0029–P022.S0033` 的 5/5 个 ID 同样已归类。Manifest 自动标为 `methods` 的 115 个 ID 均属主 Methods；后 111 个真实 Methods ID 被误标为 `supplementary`，已人工校正。这里的 226/226 是 ID inventory 完整性，不表示每个参数都在笔记中逐句重述。

### 生物学与临床意义

这篇文章把 IM 风险从“病理分期 + Hp 暴露”推进到“上皮克隆演化 + 突变过程 + 系统性造血克隆 + 黏膜免疫微生物生态”。ARID1A truncation、mutation count 和 high CH 是值得进入独立纵向验证的候选风险变量；SBS17 则是过程/暴露候选，本文没有证明它预测 EGN。任何一项目前都不能用于识别单个患者的真实进展概率。
...
- Methods `P019.S0013` 报 bulk RNA-seq 总数 104；Results/Fig. 6F `P015.S0024`、`P015.S0042` 却是 20 high CH + 94 non/low CH = 114。Fig. 6G 在排除 10 份 bacterial reads <0.02% 样本后为 104，不能消解 Methods 对 bulk 总数的冲突。
...
可迁移到胃癌精准预防队列的框架是：OLGIM/病理分层 + high-depth epithelial mutation panel + blood CH panel + smoking/Hp history + oral/gastric microbiome + longitudinal EGN endpoint。若样本量允许，可预先验证 ARID1A truncation、mutation burden 和 high CH；SBS17 exposure 与 PIGR status 应先作为探索性机制变量，不能假定具有进展预测价值。
...
可迁移到干预研究的方向包括两类：一类是 KRAS/MEK/ERK/OXPHOS 相关上皮干预，另一类是非 Hp 口腔菌和黏膜免疫屏障干预。原文只提出 non-Hp antibiotic 的可能性；probiotic、oral hygiene 与目标试验模拟是编者延伸，需另行建立证据与安全边界。
